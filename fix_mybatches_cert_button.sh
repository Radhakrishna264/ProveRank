  ...
  return (
    <div style={{width:'100%'}}>
      <div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}>
        <span style={{fontSize:10,color:'rgba(var(--pr-sub-rgb),0.76)'}}>Progress</span>
      ...
      <div style={{display:'flex',justifyContent:'space-between',marginTop:4}}>
        <span style={{fontSize:9,color:'rgba(var(--pr-sub-rgb),0.68)'}}>0 tests</span>
        <span style={{fontSize:9,color:'rgba(var(--pr-sub-rgb),0.68)'}}>Goal: 100%</span>
      </div>
    </div>
  )
}
      ...
      <div style={{position:'relative',zIndex:2,padding:'14px 14px 80px',maxWidth:900,margin:'0 auto'}}>
        ...
        {lastAccessed&&(
          <div style={{background:CARD,border:`1px solid ${ECOLS[lastAccessed.examType]||'#4D9FFF'}28`,borderRadius:18,padding:'16px',marginBottom:14,backdropFilter:'blur(16px)',animation:'slideUp 0.35s ease'}}>
            <div style={{fontSize:10,fontWeight:700,color:'rgba(var(--pr-sub-rgb),0.84)',textTransform:'uppercase',letterSpacing:1,marginBottom:10}}>▶️ Continue Where You Left Off</div>
                  ...
                  <div style={{marginTop:(b.isExpired||b.daysLeft<=7||b.isCompleted)?22:0}}>
                    ...
                    <div style={{display:'flex',gap:8,marginTop:12,flexWrap:'wrap'}}>
                      {tab==='wishlist'?(
                        ...
                        <button disabled
                          style={{flex:1,padding:'9px',background:'linear-gradient(135deg,#FFD700,#FFA000)',border:'none',borderRadius:11,color:'#000',fontWeight:700,cursor:'default',fontSize:11,opacity:0.85}}>🏆 Completed</button>
                      ):b.isExpired?(
