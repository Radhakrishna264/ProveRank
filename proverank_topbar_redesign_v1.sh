#!/bin/bash
# ProveRank — Student Panel Topbar: Ultra Premium redesign
# (grouped action capsule, gradient accent hairline, refined logo lockup,
# crisp SVG hamburger/logout icons instead of plain bordered squares).
# Same file as the earlier background fix — this includes both changes.
# Zero logic/handler/href changes — theme toggle, lang toggle, bell,
# notifications count, logout, sidebar drawer — all identical behavior.
set -e
...
cat > "$SHELLFILE" << 'EOF_STUDENTSHELL2'
'use client'
...
echo ""
echo "🎨 Topbar redesigned:"
echo "   - Right side actions grouped into one premium glass capsule (theme/lang/bell/logout)"
echo "   - Gradient blue→cyan accent hairline under the header"
echo "   - Hamburger + logout now crisp SVG icons in soft gradient tiles"
echo "   - Logo gets a subtle glow in dark mode, badge lockup refined"
echo "   - Works in both Light and Dark mode"
echo ""
echo "▶ Next: cd ~/workspace/frontend && npm run dev   (check any Student Panel page, both themes)"
