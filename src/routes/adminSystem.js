...
router.get('/global-search', verifyToken, isSuperAdmin, async (req, res) => {
  try {
    const { q = '' } = req.query
    if (!q || q.length < 2) return res.json({ success: true, results: { students: [], admins: [], exams: [], questions: [] } })
    const rx = new RegExp(q, 'i')
    const db = req.app.locals.db || router.db
    const col = (name) => require('mongoose').connection.db.collection(name)
    const [students, admins, exams, questions] = await Promise.all([
    ...
    res.json({ success: true, results: { students, admins, exams, questions } })
