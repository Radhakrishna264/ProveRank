...
router.put('/:id', auth, isAdmin, async (req, res) => {
  try {
    ...
    const editable = ['name', 'description', 'examType', 'thumbnail', 'colorIcon', 'teacherAssigned',
      'startDate', 'endDate', 'visibility', 'seatLimit', 'enrollmentRule', 'accessPolicy', 'joinCode', 'autoArchiveAfterEnd', 'totalTests'];
...
function buildBannerSyncFields(series) {
  // 🔧 CHANGE (Validity removed) — always derive from real Start/End Date, no more
  // fallback 'validity days' number. dateRange is the single source of truth now.
  let dateRange = 'Dates not set';
  if (series.startDate && series.endDate) {
    dateRange = new Date(series.startDate).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'}) + ' → ' + new Date(series.endDate).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'});
  }
...
router.put('/:id/banner', auth, isAdmin, async (req, res) => {
  try {
    ...
    const editable = ['title', 'tagline', 'examType', 'price', 'totalTests', 'duration', 'validity', 'highlights', 'ctaText', 'ctaShape', 'badge', 'template', 'primaryColor', 'secondaryColor', 'textColor', 'accentColor', 'fontStyle', 'bgImage', 'layers', 'textStyleOverride', 'textAlign', 'sectionVisibility', 'sectionLock', 'badgeStyle', 'cardStyle', 'gradientAngle', 'spacing'];
...
router.post('/:id/banner/duplicate-version/:vIdx', auth, isAdmin, async (req, res) => {
  try {
    ...
    await banner.save();
    await logBannerAudit({ bannerId: banner._id, action: 'version_duplicated', newValue: { fromVIdx: req.params.vIdx }, linkedType: 'series', linkedBatchId: req.params.id, performedBy: req.user.id, performedByName: req.user.name });
    res.json({ success: true, banner });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
