#!/bin/bash
# ProveRank — Completely remove the reward system (XP + Pathfinder badge +
# Getting Started checklist widget) from backend + frontend.
# Files touched:
#   1) src/routes/auth.js        — removed 3 routes: GET /checklist,
#      POST /checklist/mark, POST /checklist/complete (XP + badge award
#      logic). Also removed `badges:[]` init from /register.
#      (Builds on top of the earlier registration-flag fix — both intact.)
#   2) src/models/User.js        — removed `xp` and `checklist` schema
#      fields (F37 block). `badges` was referenced by old code but was
#      never actually in the schema, so nothing to remove there.
#   3) app/dashboard/page.tsx    — removed the entire "Getting Started"
#      ChecklistWidget component + its render call, and removed the
#      separate "Badges" stat card from Stats Row 2.
# Nothing else touched — streak, loginHistory, all other dashboard
# sections untouched.
set -e
...
DASHPAGE=$(grep -rl "F37 — Getting Started Checklist\|ChecklistWidget" --include="*.tsx" . 2>/dev/null | head -1)

echo "auth.js       : ${AUTHJS:-NOT FOUND}"
echo "User.js       : ${USERJS:-NOT FOUND}"
echo "dashboard page: ${DASHPAGE:-NOT FOUND}"

if [ -z "$AUTHJS" ] || [ -z "$USERJS" ] || [ -z "$DASHPAGE" ]; then
...
cp "$DASHPAGE" "${DASHPAGE}.bak_${TS}"
echo "✅ Backups created (.bak_${TS})"

# ── 1) auth.js ────────────────────────────────────────────────────
cat > "$AUTHJS" << 'EOF_AUTHJS2'
...
EOF_DASHBOARD
echo "✅ Dashboard page updated: $DASHPAGE"

echo ""
echo "🧹 Reward system (XP + badges + Getting Started checklist) fully removed."
echo "   Backend routes gone, DB schema fields gone, dashboard card + stat gone."
echo ""
echo "▶ Restart backend:  cd ~/workspace && node src/index.js"
echo "▶ Then check:       cd ~/workspace/frontend && npm run dev   (Dashboard page)"
