...
function SolarSystem() {
  ...
  return (
    <div style={{ position: 'fixed', top: '42%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 1, pointerEvents: 'none', width: 0, height: 0 }}>
      <div style={{ position: 'absolute', width: 24, height: 24, marginLeft: -12, marginTop: -12, borderRadius: '50%', background: 'radial-gradient(circle at 40% 40%,#FFF9C4,#FFD600,#FF8F00)', boxShadow: '0 0 34px rgba(255,200,0,0.5),0 0 65px rgba(255,150,0,0.22)' }} />
      {planets.map((p, i) => (
        <div key={i} style={{ position: 'absolute', width: p.o * 2, height: p.o * 2, marginLeft: -p.o, marginTop: -p.o, borderRadius: '50%', border: '1px solid rgba(77,159,255,0.05)', animation: `orb ${p.dur}s linear infinite`, animationDelay: `${p.dl}s` }}>
          <div style={{ position: 'absolute', top: -p.sz / 2, left: '50%', marginLeft: -p.sz / 2, width: p.sz, height: p.sz, borderRadius: '50%', background: p.col, boxShadow: `0 0 ${p.sz * 2}px rgba(77,159,255,0.16)` }} />
        </div>
      ))}
...
function Stars({ r }: { r: number }) {
  return <span>{[1, 2, 3, 4, 5].map(i => <span key={i} style={{ color: i <= Math.round(r) ? '#FFD700' : 'rgba(255,215,0,0.15)', fontSize: 11 }}>★</span>)}<span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginLeft: 3 }}>{r.toFixed(1)}</span></span>
}
...
function BatchCard({ b, tok, onUpdate }: { b: Batch; tok: string | null; onUpdate: () => void }) {
  ...
  const enroll = async () => {
    ...
    setLoading(true)
    try { const res = await fetch(`${API}/api/student/batches/${b._id}/enroll`, { method: 'POST', headers: { Authorization: `Bearer ${tok}` } }); const d = await res.json(); if (d.success) onUpdate(); else alert(d.error || 'Error') }
    finally { setLoading(false) }
  }
  const toggleWish = async () => { if (!tok) return alert('Please login'); await fetch(`${API}/api/student/batches/${b._id}/wishlist`, { method: 'POST', headers: { Authorization: `Bearer ${tok}` } }); onUpdate() }
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ padding: '13px 14px 15px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, margin: '9px 0 11px' }}>
          {b.isFree ? <span style={{ fontSize: 21, fontWeight: 900, color: '#27AE60', fontFamily: 'Playfair Display,serif' }}>FREE</span>
        ...
        {b.isEnrolled ? <button style={{ width: '100%', padding: '10px', background: `linear-gradient(135deg,${ec}20,${ec}10)`, border: `1px solid ${ec}40`, borderRadius: 11, color: ec, fontWeight: 700, cursor: 'pointer', fontSize: 11 }}>Continue Learning →</button>
          : b.isFree ? <button onClick={enroll} disabled={loading} style={{ width: '100%', padding: '10px', background: 'linear-gradient(135deg,#27AE60,#1E8449)', border: 'none', borderRadius: 11, color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 11, boxShadow: '0 4px 16px rgba(39,174,96,0.28)' }}>{loading ? 'Enrolling...' : '🚀 Enroll Free'}</button>
          : b.allowFreeTrial ? <button onClick={enroll} disabled={loading} style={{ width: '100%', padding: '10px', background: `linear-gradient(135deg,${ec},${ec}BB)`, border: 'none', borderRadius: 11, color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 11 }}>{loading ? 'Starting...' : '🎯 Free Trial'}</button>
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  {f.opts.map(o => { const active = (filters as Record<string, string>)[f.key] === o.v; return <button key={o.v} onClick={() => setFilters(prev => ({ ...prev, [f.key]: o.v }))} style={{ padding: '4px 9px', borderRadius: 20, fontSize: 10, cursor: 'pointer', background: active ? 'rgba(77,159,255,0.18)' : 'rgba(77,159,255,0.05)', border: `1px solid ${active ? 'rgba(77,159,255,0.42)' : 'rgba(77,159,255,0.1)'}`, color: active ? '#4D9FFF' : 'rgba(160,200,240,0.42)' }}>{o.l}</button> })}
                </div>
              </div>
            ))}
          </div>
