                ...
                <div>
                  ...
                  <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:7}}>
                    {[{k:'all',l:'All',col:'#A78BFA'},{k:'Physics',l:'⚛️ Phy',col:'#60A5FA'},{k:'Chemistry',l:'🧪 Chem',col:'#F472B6'},{k:'Biology',l:'🧬 Bio',col:'#34D399'},{k:'Math',l:'📐 Math',col:'#FBBF24'},{k:'Other',l:'📚 Other',col:'#94A3B8'}].map(function(x){
                      ...
                      const isA=qSec===x.k
                      return(<button key={x.k} onClick={function(){setQSec(x.k);setQBioSub('all');setQPage(1)}} style={{padding:'4px 10px',borderRadius:16,border:'1.5px solid '+(isA?x.col:x.col+'22'),background:isA?x.col+'18':'transparent',color:isA?x.col:'#64748B',fontSize:10,fontWeight:isA?700:400,cursor:'pointer'}}>{x.l} ({cnt})</button>)
                    })}
                  </div>
...
<div style={{padding:'10px 0 14px',marginBottom:8,borderBottom:'1px solid rgba(255,255,255,0.08)'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8,flexWrap:'wrap',gap:4}}><span style={{fontSize:11,color:'#94a3b8',fontWeight:500,letterSpacing:0.2}}>{_fQsSorted.length>0?((_qPg-1)*25+1)+'-'+Math.min(_qPg*25,_fQsSorted.length)+' of '+_fQsSorted.length+' Questions':''}</span><span style={{fontSize:11,color:'#4D9FFF',fontWeight:700,background:'rgba(77,159,255,0.1)',padding:'2px 8px',borderRadius:10}}>{'Page '+_qPg+' / '+_qTP}</span></div><div style={{display:'flex',justifyContent:'center',alignItems:'center',gap:3,flexWrap:'wrap'}}><button onClick={()=>setQPage(1)} disabled={_qPg<=1} style={{fontSize:11,color:'#4D9FFF',background:'rgba(77,159,255,0.08)',border:'1px solid rgba(77,159,255,0.2)',borderRadius:6,padding:'5px 8px',cursor:'pointer',opacity:_qPg<=1?0.3:1,fontWeight:700}}>{'«'}</button><button onClick={()=>setQPage(p=>Math.max(1,p-1))} disabled={_qPg<=1} style={{fontSize:11,color:'#4D9FFF',background:'rgba(77,159,255,0.1)',border:'1px solid rgba(77,159,255,0.25)',borderRadius:6,padding:'5px 11px',cursor:'pointer',opacity:_qPg<=1?0.3:1,fontWeight:600}}>{'← Prev'}</button>{Array.from({length:_qTP},function(_el,i){return i+1}).filter(function(p){return p===1||p===_qTP||Math.abs(p-_qPg)<=1}).reduce(function(acc,p,i,arr){if(i>0&&p-arr[i-1]>1)acc.push('…');acc.push(p);return acc;},[]).map(function(p,idx){return typeof p==='string'?<span key={'d'+idx} style={{color:'#475569',fontSize:12,padding:'0 2px',fontWeight:500}}>{'…'}</span>:<button key={p} onClick={function(){setQPage(p)}} style={{fontSize:12,color:_qPg===p?'#fff':'#4D9FFF',background:_qPg===p?'linear-gradient(135deg,#4D9FFF,#a78bfa)':'rgba(77,159,255,0.08)',border:'1px solid '+(_qPg===p?'transparent':'rgba(77,159,255,0.25)'),borderRadius:6,padding:'5px 8px',cursor:'pointer',fontWeight:_qPg===p?800:500,minWidth:32,textAlign:'center',boxShadow:_qPg===p?'0 2px 10px rgba(77,159,255,0.5)':'none'}}>{p}</button>})}<button onClick={()=>setQPage(p=>Math.min(_qTP,p+1))} disabled={_qPg>=_qTP} style={{fontSize:11,color:'#4D9FFF',background:'rgba(77,159,255,0.1)',border:'1px solid rgba(77,159,255,0.25)',borderRadius:6,padding:'5px 11px',cursor:'pointer',opacity:_qPg>=_qTP?0.3:1,fontWeight:600}}>{'Next →'}</button><button onClick={()=>setQPage(_qTP)} disabled={_qPg>=_qTP} style={{fontSize:11,color:'#4D9FFF',background:'rgba(77,159,255,0.08)',border:'1px solid rgba(77,159,255,0.2)',borderRadius:6,padding:'5px 8px',cursor:'pointer',opacity:_qPg>=_qTP?0.3:1,fontWeight:700}}>{'»'}</button></div></div>
{fQs.length===0
