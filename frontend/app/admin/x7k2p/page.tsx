                    ...
                    <div key={i} style={{...cs,marginBottom:8}}>
                      ...
                      {q.options&&<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4,marginBottom:6}}>
                        {(Array.isArray(q.options)?q.options:[q.optionA,q.optionB,q.optionC,q.optionD].filter(Boolean)).map((o:string,j:number)=>{const _hasT=!!(o&&o.trim()),_hasI=!!(q.optionImages&&q.optionImages[j]&&String(q.optionImages[j]).trim());if(!_hasT&&!_hasI)return null;return(
                          <div key={j} style={{padding:'3px 0'}}><span style={{fontSize:11,color:'#6EE7B7',fontWeight:600}}>{String.fromCharCode(65+j)})</span>{o&&<span style={{fontSize:11,color:DIM,marginLeft:3}}>{o}</span>}{q.optionImages?.[j]&&<img src={q.optionImages[j] as string} alt='' style={{height:52,width:'100%',objectFit:'cover',borderRadius:4,marginTop:4,cursor:'pointer',border:'1px solid rgba(99,102,241,0.3)',display:'block'}} onClick={()=>setLbImg(String(q.optionImages?.[j]||''))} onError={(e:any)=>{e.currentTarget.style.display='none'}}/>}{q.optionImages?.[j]&&<img src={q.optionImages[j] as string} alt='' style={{display:'block',maxWidth:'100%',maxHeight:60,borderRadius:3,marginTop:o?3:0,border:'1px solid rgba(255,255,255,0.1)'}} onError={(e:any)=>{e.currentTarget.style.display='none'}}/>}</div>
                        )
            })}
                      </div>}
