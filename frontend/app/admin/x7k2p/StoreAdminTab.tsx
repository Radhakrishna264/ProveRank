...
function Products({ T }: { T:(m:string,t?:string)=>void }) {
  ...
  const save = async () => {
    const payload = { ...form, price:parseFloat(form.price), originalPrice:parseFloat(form.originalPrice), stock:form.stock===''? 0 : parseInt(form.stock), pages:form.pages===''? undefined : parseInt(form.pages), weight:form.weight===''? undefined : parseInt(form.weight), deliveryCharge:form.deliveryCharge===''||form.deliveryCharge===undefined ? 49 : parseFloat(form.deliveryCharge), freeDeliveryAbove:form.freeDeliveryAbove===''||form.freeDeliveryAbove===undefined ? 499 : parseFloat(form.freeDeliveryAbove), tags:form.tags.split(',').map((t:string)=>t.trim()).filter(Boolean), features:form.features.filter((f:string)=>f.trim()), images:form.images.filter((img:any)=>img.url) };
