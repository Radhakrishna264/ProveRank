...
export default function ExamAttemptPage() {
  ...
  // Feature 32 — Time Extension Socket Listener
  useEffect(() => {
    // ── Feature 32: Per-Student Time Extension ──────────────────
    if (socket) {
      // Join personal room so admin can target this student
      const studentId = attempt?.studentId || user?.id || user?._id;
      const attemptId = attempt?._id || params?.attemptId || attemptIdRef.current;
      if (studentId) socket.emit('join', `student:${studentId}`);
      if (attemptId) socket.emit('join', `attempt:${attemptId}`);

      socket.on('time:extend', (data) => {
