...
export const C={primary:'#4D9FFF',card:'rgba(0,22,40,0.82)',cardL:'rgba(255,255,255,0.92)',border:'rgba(77,159,255,0.22)',borderL:'rgba(77,159,255,0.4)',text:'#E8F4FF',textL:'#0F172A',sub:'#6B8FAF',subL:'#475569',success:'#00C48C',danger:'#FF4D4D',gold:'#FFD700',warn:'#FFB84D',purple:'#A78BFA',pink:'#FF6B9D'}
...
function GalaxyBg(){
  ...
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      frame++;stars.forEach(s=>{s.ph+=s.tw;const op=s.op*(0.55+0.45*Math.sin(s.ph));ctx.beginPath();ctx.arc(s.x,s.y,s.r,0,Math.PI*2);ctx.fillStyle=s.col+op+')';ctx.fill()})
...
const NAV=[{id:'dashboard',icon:'📊',en:'Dashboard',hi:'डैशबोर्ड',href:'/dashboard'},{id:'my-exams',icon:'📝',en:'My Exams',hi:'मेरी परीक्षाएं',href:'/my-exams'},{id:'results',icon:'📈',en:'Results',hi:'परिणाम',href:'/results'},{id:'analytics',icon:'📉',en:'Analytics',hi:'विश्लेषण',href:'/analytics'},{id:'leaderboard',icon:'🏆',en:'Leaderboard',hi:'लीडरबोर्ड',href:'/leaderboard'},{id:'certificate',icon:'🎖️',en:'Certificates',hi:'प्रमाणपत्र',href:'/certificate'},{id:'admit-card',icon:'🪪',en:'Admit Card',hi:'प्रवेश पत्र',href:'/admit-card'},{id:'pyq-bank',icon:'📚',en:'PYQ Bank',hi:'पिछले वर्ष के प्रश्न',href:'/pyq-bank'},{id:'mini-tests',icon:'⚡',en:'Mini Tests',hi:'मिनी टेस्ट',href:'/mini-tests'},{id:'attempt-history',icon:'🕐',en:'Attempt History',hi:'परीक्षा इतिहास',href:'/attempt-history'},{id:'revision',icon:'🧠',en:'Smart Revision',hi:'स्मार्ट रिवीजन',href:'/revision'},{id:'goals',icon:'🎯',en:'My Goals',hi:'मेरे लक्ष्य',href:'/goals'},{id:'compare',icon:'⚖️',en:'Compare',hi:'तुलना करें',href:'/dashboard/compare'},{id:'batch-compare',icon:'📊',en:'Batch Compare',hi:'बैच तुलना',href:'/dashboard/batch-compare'},{id:'announcements',icon:'📢',en:'Announcements',hi:'घोषणाएं',href:'/announcements'},{id:'doubt',icon:'💬',en:'Doubt & Query',hi:'संदेह और प्रश्न',href:'/doubt'},{id:'my-batches',icon:'📚',en:'My Batches',hi:'मेरे बैच',href:'/dashboard/my-batches'},{id:'store',icon:'🛒',en:'Store',hi:'स्टोर',href:'/dashboard/store'},{id:'test-series',icon:'📚',en:'Test Series',hi:'टेस्ट सीरीज',href:'/dashboard/test-series'},{id:'parent-portal',icon:'👨‍👩‍👧',en:'Parent Portal',hi:'अभिभावक पोर्टल',href:'/parent-portal'},{id:'support',icon:'🛟',en:'Support',hi:'सहायता',href:'/support'},{id:'settings',icon:'⚙️',en:'Settings',hi:'सेटिंग्स',href:'/settings'},{id:'profile',icon:'👤',en:'Profile',hi:'प्रोफ़ाइल',href:'/profile'}]
export default function StudentShell({pageKey,children}:{pageKey:string;children:ReactNode}){
  ...
  return(
    <ShellCtx.Provider value={{lang,darkMode:dm,colorTheme:_isTS?'dark':colorTheme,theme:th,setColorTheme,user,toast,token,role}}>
      <div data-color-theme={_isTS?'dark':colorTheme} style={{minHeight:'100vh',background:th.shellBg,color:txt,fontFamily:'Inter,sans-serif',position:'relative',overflowX:'hidden'}}>
        ...
        <div style={{position:'fixed',top:0,left:0,width:272,height:'100vh',background:th.sidebarBg,borderRight:`1px solid ${bdr}`,zIndex:50,overflowY:'auto',display:'flex',flexDirection:'column',transform:side?'translateX(0)':'translateX(-100%)',transition:'transform .28s cubic-bezier(.4,0,.2,1)',backdropFilter:'blur(24px)',boxShadow:'5px 0 32px rgba(0,0,0,.6)'}}>
          <div style={{padding:'18px 18px 14px',borderBottom:`1px solid ${bdr}`,position:'sticky',top:0,background:th.sidebarBg,flexShrink:0}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <PRLogo size={38}/>
              <div><div style={{fontFamily:'Playfair Display,serif',fontSize:18,fontWeight:700,background:'linear-gradient(90deg,#4D9FFF 0%,#fff 60%,#4D9FFF 100%)',backgroundSize:'200% 100%',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',animation:'gradMove 4s ease infinite'}}>ProveRank</div><div style={{fontSize:10,color:'#B8C8D8',fontWeight:600,marginTop:1}}>{role==='parent'?(lang==='en'?'Parent':'अभिभावक'):(lang==='en'?'Student':'छात्र')}</div></div>
            </div>
          ...
          <div style={{padding:'8px',flex:1,overflowY:'auto'}}>
            {NAV.map(n=>(<a key={n.id} href={n.href} className="nav-lnk" onClick={()=>setSide(false)} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 12px',borderRadius:9,textDecoration:'none',color:pageKey===n.id?th.primary:sub,background:pageKey===n.id?th.navActive:'transparent',fontWeight:pageKey===n.id?700:400,fontSize:13,borderLeft:pageKey===n.id?('3px solid '+th.primary):'3px solid transparent',marginBottom:1,transition:'all .2s'}}><span style={{fontSize:15,width:20,textAlign:'center'}}>{n.icon}</span><span>{lang==='en'?n.en:n.hi}</span>{pageKey===n.id&&<span style={{marginLeft:'auto',width:6,height:6,borderRadius:'50%',background:th.primary,flexShrink:0}}/>}</a>))}
          </div>
          <div style={{padding:'12px 14px',borderTop:`1px solid ${bdr}`,flexShrink:0}}>
            <div style={{padding:'12px',background:'rgba(77,159,255,.06)',borderRadius:12,border:`1px solid ${bdr}`,marginBottom:10,textAlign:'center'}}><div style={{fontSize:11,color:sub,marginBottom:4}}>{lang==='en'?'Powered by AI Proctoring':'AI प्रॉक्टरिंग द्वारा संचालित'}</div><div style={{fontSize:10,color:C.success,fontWeight:600}}>🟢 {lang==='en'?'All Systems Live':'सभी सिस्टम लाइव'}</div></div>
            
          </div>
        </div>
        <div style={{position:'sticky',top:0,zIndex:40,background:th.headerBg,backdropFilter:'blur(22px)',borderBottom:`1px solid ${bdr}`,height:58,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 16px',boxShadow:'0 2px 24px rgba(0,0,0,.3)'}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:8}}><PRLogo size={30}/><div><div style={{fontFamily:'Playfair Display,serif',fontWeight:700,fontSize:15,background:`linear-gradient(90deg,${C.primary},#fff)`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',lineHeight:1}}>ProveRank</div><div style={{fontSize:9,color:'#B8C8D8',fontWeight:600,letterSpacing:.6}}>{lang==='en'?'STUDENT':'छात्र'}</div></div></div>
          </div>
          ...
          <button
            ...
            style={{background:'none',border:'1px solid rgba(79,195,247,0.3)',borderRadius:8,width:34,height:34,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'#4FC3F7',flexShrink:0,transition:'all .2s'}}
        ...
        <div style={{position:'relative',zIndex:2,padding:'0 0 56px',width:'100%',maxWidth:'100vw',margin:'0',overflow:'hidden visible',animation:'fadeIn .4s ease',background:'transparent'}}>{children}</div>
      </div>
    </ShellCtx.Provider>
  )
}
