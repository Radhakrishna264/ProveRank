...
const ShellCtx=createContext<ShellCtx>({lang:'en',darkMode:true,user:null,toast:()=>{},token:'',role:'student'})
      ...
      <div style={{minHeight:'100vh',background:'#020816',color:txt,fontFamily:'Inter,sans-serif',position:'relative',overflowX:'hidden'}}>
        ...
        <div style={{position:'fixed',top:0,left:0,width:272,height:'100vh',background:'rgba(0,5,18,.97)',borderRight:`1px solid ${bdr}`,zIndex:50,overflowY:'auto',display:'flex',flexDirection:'column',transform:side?'translateX(0)':'translateX(-100%)',transition:'transform .28s cubic-bezier(.4,0,.2,1)',backdropFilter:'blur(24px)',boxShadow:'5px 0 32px rgba(0,0,0,.6)'}}>
          ...
          <div style={{padding:'8px',flex:1,overflowY:'auto'}}>
            {NAV.map(n=>(<a key={n.id} href={n.href} className="nav-lnk" onClick={()=>setSide(false)} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 12px',borderRadius:9,textDecoration:'none',color:pageKey===n.id?'#4D9FFF':sub,background:pageKey===n.id?'rgba(77,159,255,.16)':'transparent',fontWeight:pageKey===n.id?700:400,fontSize:13,borderLeft:pageKey===n.id?'3px solid #4D9FFF':'3px solid transparent',marginBottom:1,transition:'all .2s'}}><span style={{fontSize:15,width:20,textAlign:'center'}}>{n.icon}</span><span>{lang==='en'?n.en:n.hi}</span>{pageKey===n.id&&<span style={{marginLeft:'auto',width:6,height:6,borderRadius:'50%',background:'#4D9FFF',flexShrink:0}}/>}</a>))}
          </div>
          <div style={{padding:'12px 14px',borderTop:`1px solid ${bdr}`,flexShrink:0}}>
            ...
            <button onClick={logout} style={{width:'100%',padding:'11px',background:'rgba(255,77,77,.1)',color:'#FF4D4D',border:'1px solid rgba(255,77,77,.28)',borderRadius:10,cursor:'pointer',fontWeight:700,fontSize:13,fontFamily:'Inter,sans-serif',display:'flex',alignItems:'center',justifyContent:'center',gap:8,transition:'all .2s'}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" stroke="#FF4D4D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>{lang==='en'?'Logout':'लॉगआउट'}</button>
          </div>
        </div>
