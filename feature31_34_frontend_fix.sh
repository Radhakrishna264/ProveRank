...
export default function AllExams({ token, API, T, onCreateNew }: Props) {
  ...
  const [cloneForm, setCloneForm] = useState<any>({newTitle:'',startTime:'',endTime:'',targetBatch:''})
  ...
  const openCloneDialog = (exam:any) => {
    ...
    setCloneForm({ newTitle: `Copy of ${exam.title}`, startTime:'', endTime:'', targetBatch: exam.batch||'' })
  }
  ...
  const executePermanentDelete = async (exam:any, payload:any) => {
    try {
      ...
      const d = await r.json()
      if (d.success) { T(`Exam delete ho gaya — ${d.deletedAttempts||0} attempts, ${d.deletedResults||0} results bhi cascade delete hue`); setExams(p=>p.filter(x=>x._id!==exam._id)) }
  ...
  const cancelUndo = () => {
    ...
    setUndoToast(null)
    T('Delete cancel ho gaya — exam safe hai ✅')
  }

  const confirmPermanentDelete = () => {
    if (!deleteImpact) return
    const { exam, data } = deleteImpact
    if (data?.requiresTypeToConfirm && confirmTitleInput !== exam.title) { T('Exam ka exact title type karo confirm karne ke liye','e'); return }
    if (data?.requiresReason && !deleteReasonInput.trim()) { T('Completed exam delete karne ke liye reason dena mandatory hai','e'); return }
  ...
  const restoreExam = async (exam:any) => {
    ...
    try {
      ...
      const d = await r.json()
      if (d.success) { T('Exam restore ho gaya ✅'); fetchTrash(); fetchExams() } else T(d.message||'Failed','e')
    } catch { T('Network error','e') }
    setBusyId(null)
  }

  const purgeFromTrash = async (exam:any) => {
    if (!window.confirm(`"${exam.title}" ko HAMESHA ke liye delete karna hai? Ye undo nahi ho sakta.`)) return
    setBusyId(exam._id)
    try {
      ...
      const d = await r.json()
      if (d.success) { T('Permanently delete ho gaya'); setTrashExams(p=>p.filter(x=>x._id!==exam._id)) } else T(d.message||'Failed','e')
    } catch { T('Network error','e') }
    setBusyId(null)
  }
    ...
    <div style={{fontFamily:'Inter,sans-serif'}}>
      ...
      {trashView && (
        <div style={cs}>
          ...
          <div style={{fontSize:11,color:DIM,marginBottom:14}}>Archived exams — 30 din ke baad automatically permanently delete ho jaate hain.</div>
          {trashLoading ? (
            <div style={{textAlign:'center' as any,padding:'40px 0',color:DIM}}><span className="ae-spin" style={{fontSize:22}}>⟳</span></div>
          ) : trashExams.length===0 ? (
            <div style={{textAlign:'center' as any,padding:'30px 0',color:DIM,fontSize:12}}>Recycle Bin khaali hai.</div>
          ) : (
                ...
                return (
                  <div key={e._id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10,padding:'10px 14px',background:'rgba(255,255,255,0.03)',borderRadius:10,border:`1px solid ${BOR}`,flexWrap:'wrap' as any}}>
                    <div>
                      ...
                      <div style={{fontSize:10,color:WRN,marginTop:2}}>⏳ {daysLeft} din baad auto-delete</div>
                    </div>
            ...
            return (
              <div key={e._id} className="ae-card" style={{position:'relative' as any,overflow:'hidden',borderRadius:16,background:CRD,border:`1px solid ${BOR}`,borderLeft:`4px solid ${meta.color}`,backdropFilter:'blur(14px)',boxShadow:'0 4px 18px rgba(0,0,0,0.3)'}}>
                ...
                <div style={{display:'flex',gap:6,padding:'10px 14px',overflowX:'auto' as any}}>
                  ...
                  {e.status==='live' ? (
                    <button title="Live exam — pehle end/unpublish karo delete/archive karne ke liye" style={{...bg_,flexShrink:0,padding:'7px 11px',fontSize:13,color:DNG,opacity:0.6,cursor:'not-allowed'}}>🔒</button>
                  ) : (
      ...
      {deleteImpact && (
        <div style={{position:'fixed' as any,inset:0,background:'rgba(0,4,14,0.75)',backdropFilter:'blur(4px)',zIndex:210,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'20px 14px',overflowY:'auto' as any}} onClick={()=>setDeleteImpact(null)}>
          <div onClick={e=>e.stopPropagation()} style={{...cs,width:'100%',maxWidth:420,marginTop:10,border:`1.5px solid ${DNG}44`}}>
            ...
            {deleteImpactLoading ? (
              ...
              <div style={{padding:'14px',background:`${DNG}14`,border:`1px solid ${DNG}44`,borderRadius:10,fontSize:12,color:DNG,marginBottom:14}}>
                🔒 Ye exam abhi LIVE/Active hai — delete ya archive nahi kar sakte. Pehle isse end/unpublish karo (📢/↩️ button se), phir wapas try karo.
              </div>
            ) : (
              <>
                <div style={{display:'flex',gap:6,flexWrap:'wrap' as any,marginBottom:14}}>
                  <span style={{fontSize:11,fontWeight:700,color:WRN,background:`${WRN}18`,border:`1px solid ${WRN}44`,borderRadius:8,padding:'5px 10px'}}>👥 {deleteImpact.data.studentsAttempted} students ne attempt kiya</span>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  {deleteCascadeOpen && (
                    <div style={{marginTop:8,padding:'10px 12px',background:'rgba(255,255,255,0.03)',borderRadius:8,fontSize:11,color:DIM,lineHeight:1.7}}>
                      ...
                      • Future certificates (agar feature ban'a) bhi automatically invalid ho jaayenge<br/>
                      • Ye action sirf "Permanent Delete" se hota hai — "Archive" se data safe rehta hai
                    </div>
                  )}
                </div>
                ...
                {deleteMode==='archive' && (
                  <div style={{fontSize:11,color:DIM,marginBottom:14,lineHeight:1.5}}>Exam "Recycle Bin" me chala jaayega — 30 din tak restore kar sakte ho, uske baad auto-delete ho jaayega.</div>
                )}

                {deleteMode==='permanent' && (
                  <>
                    <div style={{fontSize:11,color:DNG,marginBottom:10,fontWeight:600}}>⚠️ Ye action undo nahi ho sakta (10 second ka grace period milega).</div>
                    ...
                    {deleteImpact.data.requiresTypeToConfirm && (
                      <div style={{marginBottom:10}}>
                        <label style={lbl}>Confirm karne ke liye exam ka title type karo: <b style={{color:TS}}>{deleteImpact.exam.title}</b></label>
                        <input value={confirmTitleInput} onChange={e=>setConfirmTitleInput(e.target.value)} style={inp} placeholder="Exam title yahan type karo"/>
                      </div>
                    )}
                  </>
                )}
      ...
      {undoToast && (
        <div style={{position:'fixed' as any,left:16,bottom:16,zIndex:260,background:CRD,border:`1px solid ${DNG}44`,borderRadius:14,padding:'14px 16px',minWidth:260,maxWidth:340,boxShadow:'0 8px 28px rgba(0,0,0,0.5)'}}>
          <div style={{fontSize:12,color:TS,marginBottom:8}}>🗑 "{undoToast.exam.title}" delete ho raha hai... ({undoToast.secondsLeft}s)</div>
      ...
      {cloneDialogExam && (
        <div style={{position:'fixed' as any,inset:0,background:'rgba(0,4,14,0.7)',backdropFilter:'blur(4px)',zIndex:210,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'20px 14px',overflowY:'auto' as any}} onClick={()=>setCloneDialogExam(null)}>
          <div onClick={e=>e.stopPropagation()} style={{...cs,width:'100%',maxWidth:400,marginTop:10}}>
            ...
            <div style={{fontSize:11,color:DIM,marginBottom:14}}>Same questions, settings, marking scheme copy ho jaayenge. Naya exam Draft me banega.</div>
            <div style={{display:'flex',flexDirection:'column' as any,gap:12}}>
              <div>
              ...
              <div>
                <label style={lbl}>Batch (alag batch me clone karna ho to change karo)</label>
                <input value={cloneForm.targetBatch} onChange={e=>setCloneForm((f:any)=>({...f,targetBatch:e.target.value}))} style={inp}/>
              </div>
            </div>
      ...
      {cloneSuccessToast && (
        <div style={{position:'fixed' as any,inset:0,zIndex:270,display:'flex',alignItems:'center',justifyContent:'center',pointerEvents:'none' as any}}>
      ...
      {cloneInfoExam && (
        <div style={{position:'fixed' as any,inset:0,background:'rgba(0,4,14,0.7)',backdropFilter:'blur(4px)',zIndex:210,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:'20px 14px',overflowY:'auto' as any}} onClick={()=>{setCloneInfoExam(null);setCompareData(null)}}>
              ...
              <div style={{display:'flex',flexDirection:'column' as any,gap:12}}>
                ...
                <div>
                  ...
                  {(!cloneInfoData.children || cloneInfoData.children.length===0) ? (
                    <div style={{fontSize:11,color:DIM}}>Abhi tak iska koi clone nahi banaya gaya.</div>
                  ) : (
          ...
          <div onClick={e=>e.stopPropagation()} style={{...cs,width:'100%',maxWidth:480,marginTop:10}}>
            ...
            {editExam.clonedFrom && (
              <div style={{fontSize:11,color:PRP,background:`${PRP}14`,border:`1px solid ${PRP}33`,borderRadius:8,padding:'8px 10px',marginBottom:14}}>
                🔗 Ye ek cloned exam hai (draft) — publish karne se pehle settings change karo. Questions replace/edit karne ke liye "Add Questions" / Question Bank flow use karo (yahan se settings hi edit hote hain).
              </div>
            )}
            <div style={{display:'flex',flexDirection:'column' as any,gap:12}}>
              <div>
