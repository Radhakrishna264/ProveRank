...
function ProfileContent() {
  const { lang, darkMode:dm, user, toast, token, theme } = useShell()
  ...
  useEffect(() => {
    ...
    setTimezone(me.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || 'Asia/Kolkata')
    ...
    initial.current = {
      name:me.name||'', phone:me.phone||'', dob:me.dob||'', city:me.city||'', state:me.state||'', gender:me.gender||'', bio:me.bio||'', avatar:me.avatar||'', timezone:me.timezone||'',
  ...
  useEffect(()=>{ const i=initial.current; setDirtyPersonal(name!==i.name||phone!==i.phone||dob!==i.dob||city!==i.city||state!==i.state||gender!==i.gender||bio!==i.bio||avatar!==i.avatar||timezone!==i.timezone) },[name,phone,dob,city,state,gender,bio,avatar,timezone])
  useEffect(()=>{ const i=initial.current; setDirtyAcademic(targetExam!==i.targetExam||targetYear!==i.targetYear||yearAppearing!==i.yearAppearing||board!==i.board||school!==i.school||medium!==i.medium||coaching!==i.coaching) },[targetExam,targetYear,yearAppearing,board,school,medium,coaching])
  useEffect(()=>{ const i=initial.current; setDirtyPrefs(notifEmail!==i.notifEmail||notifSms!==i.notifSms||notifStudy!==i.notifStudy) },[notifEmail,notifSms,notifStudy])
  ...
  const savePersonal = async () => {
    ...
    await saveSection({name,phone,dob,city,state,gender,bio,avatar,timezone}, 'personal')
    setSavingPersonal(false)
  }
  const saveAcademic = async () => {
    setSavingAcademic(true)
    await saveSection({targetExam,targetYear,yearOfAppearing:yearAppearing,board,school,medium,coachingInstitute:coaching}, 'academic')
    setSavingAcademic(false)
  }
  const savePrefs = async () => {
    setSavingPrefs(true)
    await saveSection({preferences:{emailNotif:notifEmail,smsNotif:notifSms,studyReminder:notifStudy}}, 'preferences')
  ...
  const PreferencesSection = () => (
    <SectionCard theme={theme}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 4px',borderBottom:`1px solid ${bdr}`}}>
        ...
        <a href="/settings" style={{fontSize:12,color:prim,fontWeight:700,textDecoration:'none'}}>{dm?t('Dark','डार्क'):t('Light','लाइट')} → {t('Change','बदलें')}</a>
      </div>
