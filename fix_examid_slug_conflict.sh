...
set -e
# ════════════════════════════════════════════════════════════════
# FIX: Next.js dynamic-slug conflict — /exam/[id] vs /exam/[examId]
# Error: "You cannot use different slug names for the same dynamic
#         path ('examId' !== 'id')."
#
# Root cause: two sibling dynamic folders under frontend/app/exam/
#   [id]      -> old single-page all-in-one exam flow (page.tsx)
#   [examId]  -> new multi-step flow (attempt/instructions/result/
#                waiting/webcam sub-folders, no page.tsx of its own)
# Both are actively linked to by different pages, so neither can be
# deleted outright. Fix: merge [id]/page.tsx INTO [examId]/ (its one
# param reference already used the variable name "examId" internally
# -- only the params?.id -> params?.examId line needed to change).
# ════════════════════════════════════════════════════════════════
cd ~/workspace
echo "🔧 Fixing /exam/[id] vs /exam/[examId] slug conflict..."

# ── Step 0: Backup ──
mkdir -p .backups_examid_fix
cp -r "frontend/app/exam/[id]" ".backups_examid_fix/[id]_$(date +%s)" 2>/dev/null || true
echo "✅ Backup saved to ~/workspace/.backups_examid_fix/"

# ── Step 1: Confirm [examId]/page.tsx does NOT already exist (safety) ──
if [ -f "frontend/app/exam/[examId]/page.tsx" ]; then
  echo "❌ frontend/app/exam/[examId]/page.tsx already exists — aborting to avoid overwrite."
  echo "   Please check manually before re-running."
  exit 1
fi

# ── Step 2: Write the merged page (old [id] content, 1 line fixed) ──
mkdir -p "frontend/app/exam/[examId]"
cat > "frontend/app/exam/[examId]/page.tsx" << 'EXAMPAGEEOF'
'use client'
...
EXAMPAGEEOF
echo "✅ Created frontend/app/exam/[examId]/page.tsx"

# ── Step 3: Remove the old conflicting [id] folder ──
rm -rf "frontend/app/exam/[id]"
echo "✅ Removed frontend/app/exam/[id]/"

# ── Step 4: Verify — should now show only ONE dynamic slug under exam/ ──
echo ""
echo "🔎 Verifying..."
find frontend/app/exam -maxdepth 1 -type d
if [ -d "frontend/app/exam/[id]" ]; then echo "❌ [id] still exists — something went wrong"; exit 1; fi
if [ ! -f "frontend/app/exam/[examId]/page.tsx" ]; then echo "❌ New page.tsx missing"; exit 1; fi
echo "✅ Only [examId] remains — conflict resolved."

echo ""
echo "════════════════════════════════════════════════════════════════"
echo "✅ FIX COMPLETE"
echo "════════════════════════════════════════════════════════════════"
echo "No other files needed changes — dashboard/page.tsx, admit-card/page.tsx,"
echo "and mini-tests/page.tsx link via href={\`/exam/\${id}\`} which works"
echo "regardless of the internal param name."
echo ""
echo "NEXT STEPS:"
echo "  1. rm -f frontend/.next/dev.lock 2>/dev/null; pkill -f next 2>/dev/null"
echo "  2. cd frontend && npm run dev   (or redeploy to Vercel)"
echo "  3. Test: My Batches -> click Continue on any batch/test series"
echo "  4. Also retest old entry points: Dashboard Start button, Admit Card,"
echo "     Mini Tests Start -> (they use the same [examId] route now)"
echo "  5. git add -A && git commit -m \"fix: merge /exam/[id] into /exam/[examId] - resolve Next.js slug conflict\" && git push"
