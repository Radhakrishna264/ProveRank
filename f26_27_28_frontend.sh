#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║  ProveRank — Features 26+27+28: Create Exam Wizard (Frontend)  ║
# ╚══════════════════════════════════════════════════════════════════╝
set -e
FE=/home/runner/workspace/frontend/app/admin/x7k2p
echo "🚀 Create Exam Wizard Frontend..."

cat > $FE/CreateExamWizard.tsx << 'ENDOFFILE'
'use client'
...
interface Props { token:string; API:string; T:(m:string,t?:'s'|'e'|'w')=>void; fetchAll:()=>void; batches:any[]; exams:any[]; questions:any[] }
...
export default function CreateExamWizard({ token, API, T, fetchAll, batches, exams, questions }:Props) {
...
ENDOFFILE
echo "✅ CreateExamWizard.tsx created ($(wc -l < $FE/CreateExamWizard.tsx) lines)"

# ── Patch page.tsx: import + replace create_exam tab ─────────────────────────
node << 'EOF'
const fs = require('fs');
...
// 1. Add import
if (!c.includes('CreateExamWizard')) {
  c = c.replace("'use client'", "'use client'\nimport CreateExamWizard from './CreateExamWizard'");
  console.log('✅ CreateExamWizard imported');
} else { console.log('✅ Import present'); }

// 2. Replace create_exam tab content
const START = "          {tab==='create_exam'&&(\n            <div>";
const END   = "            </div>\n          )}\n\n          {/* ══ EXAM TEMPLATES ══ */}";
const si = c.indexOf(START), ei = c.indexOf(END);

if (si === -1 || ei === -1) {
  console.log('Start found:', si !== -1, '| End found:', ei !== -1);
  // Try alternative end marker
  const END2 = "            </div>\n          )}\n\n          {/* ══ BULK EXAM";
  const ei2 = c.indexOf(END2);
  if (si !== -1 && ei2 !== -1) {
    const NEW = `          {tab==='create_exam'&&(
            <CreateExamWizard
          ...
          {/* ══ BULK EXAM`;
    c = c.substring(0, si) + NEW + c.substring(ei2 + END2.length);
    console.log('✅ create_exam tab replaced (alt marker)');
    fs.writeFileSync(f, c); process.exit(0);
  }
  console.log('❌ Could not find tab boundaries'); process.exit(1);
}
...
c = c.substring(0, si) + NEW + c.substring(ei + END.length);
fs.writeFileSync(f, c);
console.log('✅ create_exam tab replaced with CreateExamWizard');
EOF

# ── Verification ──────────────────────────────────────────────────────────────
echo ""
echo "═══════════════════════════════════════════════════════════════"
echo "  Features 26+27+28 Frontend — Verification"
echo "═══════════════════════════════════════════════════════════════"
F=$FE/CreateExamWizard.tsx
chk(){ grep -q "$2" "$F" 2>/dev/null && echo "  ✅ $3" || echo "  ❌ $3"; }

chk "applyTemplate"            "26    Quick Templates"
chk "title.*required\|formErrors.title" "26.1.1 Exam Title required"
chk "setSubject\|SUBJECTS"     "26.1.2 Subject selector"
chk "setCategory\|CATEGORIES" "26.1.3 Category selector"
chk "setTotalQs\|totalQs"      "26.1.4 Total Question Count"
chk "subjectQs\|subject-wise"  "26.1.5 Subject-wise Qs count"
chk "setExamType\|EXAM_TYPES"  "26.1.6 Exam Type"
chk "setDuration\|duration"    "26.1.7 Duration (minutes)"
chk "totalMarks.*totalQs.*correctMarks\|auto" "26.1.8 Auto-calculate total marks"
chk "correctMarks\|negativeMarks" "26.1.9 Marking scheme +/−"
chk "startDate\|datetime-local" "26.1.10 Start date & time"
chk "endDate"                  "26.1.11 End date & time"
chk "instructions"             "26.1.12 Custom instructions"
chk "passwordEnabled\|password" "26.1.13 Password toggle"
chk "whitelist"                "26.1.14 Whitelist toggle"
chk "waitingRoom\|waitingMins" "26.1.15 Waiting room toggle"
chk "reattempt\|reattemptUnlimited" "26.1.16 Re-attempt control"
chk "reviewWindow"             "26.1.17 Review window toggle"
chk "sectionWise"              "26.1.18 Section-wise toggle"
chk "watermark"                "26.1.19 Watermark toggle"
chk "liveQsRange"              "26.1.21 Live Qs range"
chk "assignType\|setAssignType" "26.2.1 Assignment type"
chk "batchId\|setBatchId"       "26.2.2 Batch dropdown"
chk "testSeriesId"             "26.2.3 Test Series dropdown"
chk "multiBatches"             "26.2.4 Multi-batch"
chk "step-enter\|cubic-bezier\|slideIn" "26.3/26.7 Step indicator + animated transition"
chk "datetime-local"           "26.4 Date-time picker"
chk "type.*range.*duration\|duration.*slider" "26.5 Duration slider"
chk "SUC.*correctMarks\|DNG.*negativeMarks" "26.6 Marking scheme colored badges"
chk "formErrors\|shake-anim"   "26.8 Required fields glow + shake"
chk "Live Preview"             "26.9 Real-time exam card preview"
chk "qMethod.*bank\|setQMethod.*bank" "27.1 QB Bank import"
chk "isPYQ\|PYQ Bank"          "27.1 PYQ Bank import"
chk "smart.*suggest\|smart-select\|qMethod.*smart" "27.2/27.14 Smart AI select"
chk "copypaste"                "27.3 Copy-paste"
chk "qMethod.*excel\|qMethod.*pdf" "27.4 Excel/PDF upload"
chk "selectedBySubj\|totalSelected" "27.5 Selected Q count by subject"
chk "q.text.*overflow\|WebkitLineClamp" "27.6 Question preview"
chk "onDragStart\|onDrop"      "27.7 Drag-drop reorder"
chk "filter.*_id.*!==.*q._id\|setSelectedQs.*filter" "27.8 Remove individual Q"
chk "subjectQs.*balanceOk\|balanceOk" "27.10/27.12 Balance check"
chk "bankFilter"               "27.11 Filter while selecting"
chk "easyPct\|medPct\|hardPct" "27.13 Difficulty distribution %"
chk "duplicate.*check\|isDup"  "27.16 Duplicate detector"
chk "q.usageCount\|usageCount" "27.17 Previously used indicator"
chk "generateSets\|multiSet\|setCount" "27.18/27.19 Multi-set generate"
chk "chapSet\|chapSet.size"    "27.20 Chapter coverage"
chk "imgCount\|image Qs"       "27.21 Image Q indicator"
chk "selectedBySubj.*v.*===\|balanceOk" "27.22 Min/Max validator"
chk "flex.*1 1 260px\|left.*filter.*right.*tray" "27.23 Split layout"
chk "sticky.*top.*bar\|totalSelected.*totalTarget" "27.24 Sticky count bar"
chk "cursor.*grab\|onDragStart" "27.25 Drag-drop grip"
chk "easyPct.*SUC\|hardPct.*DNG" "27.26 Difficulty pie mini"
chk "Smart.*Select\|smart-select\|PRP.*glow" "27.27 Smart Select button"
chk "dashed.*border\|border.*dashed\|excel.*pdf" "27.28 Upload zone dashed"
chk "shake-anim\|shakeNext"    "27.29 Shake on wrong count"
chk "checklist.*ok\|checklistOk" "28.9 Pre-publish checklist"
chk "Estimated Difficulty\|rating.*Hard\|rating.*Medium" "28.10 Difficulty rating"
chk "Clone.*Exam\|doClone"     "28.11 Clone option"
chk "Password Reminder"        "28.12 Password reminder"
chk "studentCount"             "28.13 Student count preview"
chk "Summary\|summaryCard\|Exam Summary" "28.1 Exam summary"
chk "Marking.*review\|correctMarks.*negativeMarks.*summary" "28.2 Marking review"
chk "questionsExpanded\|accordion\|Questions accordion" "28.3/28.31 Questions accordion"
chk "Back.*Step\|goStep.*1.*left" "28.4 Edit/back button"
chk "copyLink\|Exam Link\|copy.*link" "28.5/28.30 Exam link copy"
chk "Publish Now\|doPublish"   "28.8.2 Publish Now"
chk "Save.*Draft\|doPublish.*true" "28.8.3 Save as Draft"
chk "Save.*Template\|exam-wizard/templates" "28.8.4 Save as template"
chk "copyLink"                 "28.8.5 Exam link copy"
chk "notifyStudents\|Notify Students" "28.8.6 Notify toggle"
chk "schedule\|doSchedule"     "28.8.1 Schedule auto-publish"
chk "confetti\|publishSuccess\|🎉" "28.15 Confetti on publish"
chk "boxShadow.*PRP\|glow.*PRP" "28.18 Publish button glow"
chk "CreateExamWizard" "$FE/page.tsx" && echo "  ✅ Component wired in page.tsx" || echo "  ❌ Not in page.tsx"

echo ""
echo "═══════════════════════════════════════════════════════════════"
echo "🎉 git add . && git commit -m 'feat: Create Exam Wizard F26+27+28' && git push"
echo "═══════════════════════════════════════════════════════════════"
