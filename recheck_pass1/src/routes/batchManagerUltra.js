...
router.post('/', auth, isAdmin, async (req, res) => {
  try {
    ...
    if (body.batchCode && String(body.batchCode).trim()) dupOr.push({ batchCode: String(body.batchCode).trim() });
    const dup = await Batch.findOne({ $or: dupOr });
...
router.post('/:id/banner/duplicate-version/:vIdx', auth, isAdmin, async (req, res) => {
  try {
    ...
    await banner.save();
    await logBannerAudit({ bannerId: banner._id, action: 'version_duplicated', newValue: { fromVIdx: req.params.vIdx }, linkedType: 'batch', linkedBatchId: req.params.id, performedBy: req.user.id, performedByName: req.user.name });
    res.json({ success: true, banner });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
function applyPublishState(batch, isPublished) {
  ...
  batch.lifecycleStatus = isPublished ? 'active' : (batch.lifecycleStatus === 'archived' ? 'archived' : 'paused');
}
