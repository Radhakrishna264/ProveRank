...
export default function AdminPanel() {
  ...
  useEffect(()=>{
    ...
    s.id=id;
    s.textContent='@media(min-width:768px){#pr-top-nav{height:68px!important;}#pr-top-nav>div:first-child{gap:20px!important;}#pr-top-nav>div:first-child>button:first-child{width:44px!important;height:44px!important;}#pr-top-nav>div:last-child{gap:14px!important;}#pr-top-nav>div:last-child button{min-width:42px!important;height:42px!important;border-radius:10px!important;}#pr-top-nav .pr-brand-name{font-size:16px!important;}#pr-top-nav .pr-role-tag{font-size:11px!important;letter-spacing:1.5px!important;}#pr-top-nav .pr-brand{position:absolute!important;left:50%!important;top:50%!important;transform:translate(-50%,-50%)!important;z-index:1;}}';
...
const confirmAndAdd=useCallback(async()=>{
  ...
  const NAV=[
    ...
    {id:'batches',ico:'📦',lbl:'Batch Management',grp:'Students'},
  ...
  const PERM_TO_NAV:{[k:string]:string[]}={
    ...
    batch_transfer:['batches','test_series'],
        ...
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div className="pr-brand" style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <div style={{display:'flex',flexDirection:'column',alignItems:'flex-start',gap:3}}>
              ...
              {/* Role chip — solid surface, gold for SuperAdmin, blue for Admin */}
              <div className="pr-role-tag" style={{display:'inline-flex',alignItems:'center',gap:4,padding:'2.5px 9px 2.5px 7px',borderRadius:20,whiteSpace:'nowrap',background:role==='superadmin'?'linear-gradient(135deg,#B8860B,#FFD700)':'linear-gradient(135deg,#2E5FA3,#4D9FFF)',boxShadow:role==='superadmin'?'0 2px 10px rgba(255,215,0,.35)':'0 2px 10px rgba(77,159,255,.35)'}}>
                <span style={{fontSize:9}}>⚡</span>
                <span style={{fontSize:9,fontWeight:800,letterSpacing:1,color:role==='superadmin'?'#241a00':'#fff'}}>{role.toUpperCase()}</span>
              </div>
            </div>
          </div>
        </div>
...
{delConfirmId&&(
          ...
          {/* == BATCHES == (FPR1 Ultra SaaS Upgrade) */}
          {tab==='batches'&&(<BatchManagerUltra token={token} API={API}/>)}
