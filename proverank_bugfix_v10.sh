...
export default function AdminPanel() {
  ...
  const uploadQuestions = async()=>{
    ...
    try{
      ...
      if(qUploadMethod==='copypaste'||qUploadMethod==='manual'){
        if(!manualQText.trim()){showToast('Paste question text first','error');setUploadingQ(false);return}
  ...
  const renderCreateExam = ()=>(
    <div>
      ...
      {examStep===2 && (<React.Fragment key='step2-stable'>
          ...
          <div style={{padding:'20px'}}>
            ...
            <div style={{display:qUploadMethod==='pdf'?'block':'none'}}>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  <textarea value={answerKeyText} onChange={e=>{e.stopPropagation();setAnswerKeyText(e.target.value)}} rows={4} placeholder="1-B, 2-A, 3-D ..." style={{width:'100%',padding:'10px 13px',borderRadius:9,border:`1.5px solid ${iBrd}`,background:iBg,color:tm,fontSize:12,fontFamily:'monospace',resize:'none',outline:'none',boxSizing:'border-box'}}/>
                </div>
            </div>
