    ...
    <div style={{fontFamily:'Inter,sans-serif'}}>
      ...
      {previewExam && (
        <div style={{position:'fixed' as any,inset:0,background:'rgba(0,4,14,0.85)',backdropFilter:'blur(4px)',zIndex:230,display:'flex',flexDirection:'column' as any}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'14px 18px',borderBottom:`1px solid ${BOR}`,background:CRD,flexShrink:0}}>
            <div>
              <div style={{fontFamily:'Playfair Display,serif',fontWeight:700,fontSize:15,color:TS}}>👁 {previewExam.title}</div>
              <div style={{fontSize:10,color:DIM,marginTop:2}}>Student jaisa hi content view — read-only, koi attempt record nahi banta</div>
            </div>
            <button onClick={()=>setPreviewExam(null)} style={{background:'transparent',border:'none',color:DIM,fontSize:20,cursor:'pointer',flexShrink:0}}>✕</button>
          </div>
            ...
            {previewLoading ? (
              ...
              <div style={{display:'flex',flexDirection:'column' as any,gap:12,maxWidth:680,margin:'0 auto'}}>
                {previewQuestions.map((q:any,qi:number)=>{
                  const qText = q.questionText || q.text || q.question || q.title || '(Question text missing — render issue!)'
                  ...
                  return (
                    <div key={q._id||qi} style={{...cs,padding:'14px 16px',borderLeft:`3px solid ${missingOpts?DNG:ACC}`}}>
                      ...
                      <div style={{fontSize:13,color:TS,lineHeight:1.6,marginBottom:10}}>{qText}</div>
                      {!missingOpts && (
