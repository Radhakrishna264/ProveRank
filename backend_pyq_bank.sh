...
# ║  BACKEND SCRIPT — Models + Routes + Index Mount          ║
# ╚══════════════════════════════════════════════════════════╝
set -e
WS=~/workspace
echo "🚀 Starting PYQ Bank Backend setup..."

# ─── 1. PYQExamCard Model ─────────────────────────────────────────────────────
cat > $WS/src/models/PYQExamCard.js << 'ENDOFFILE'
...
ENDOFFILE
echo "✅ PYQExamCard.js model created"

# ─── 2. PYQYearCard Model ─────────────────────────────────────────────────────
cat > $WS/src/models/PYQYearCard.js << 'ENDOFFILE'
const mongoose = require('mongoose');
const schema = new mongoose.Schema({
...
ENDOFFILE
echo "✅ PYQYearCard.js model created"

# ─── 3. Add pyqExam field to Question model (safe patch) ─────────────────────
cat > /tmp/patch_question_model.js << 'ENDOFFILE'
const fs = require('fs');
const path = require('path');
const file = path.join(require('os').homedir(), 'workspace/src/models/Question.js');
if (!fs.existsSync(file)) { console.log('⚠️  Question.js not found at', file); process.exit(0); }
let c = fs.readFileSync(file, 'utf8');
if (c.includes('pyqExam')) {
  console.log('✅ pyqExam field already present in Question model');
  process.exit(0);
}
// Insert pyqExam right after pyqYear
c = c.replace(
  'pyqYear: { type: Number, default: null }',
  'pyqYear: { type: Number, default: null },\n  pyqExam: { type: String, default: \'NEET\' },\n  pyqYearCardId: { type: mongoose.Schema.Types.ObjectId, ref: \'PYQYearCard\', default: null }'
);
fs.writeFileSync(file, c);
console.log('✅ pyqExam + pyqYearCardId fields added to Question model');
ENDOFFILE
cd $WS && node /tmp/patch_question_model.js

# ─── 4. Main PYQ Bank Admin Route ─────────────────────────────────────────────
cat > $WS/src/routes/pyqBankAdmin.js << 'ENDOFFILE'
...
ENDOFFILE
echo "✅ pyqBankAdmin.js route file created"

# ─── 5. Mount route in index.js ────────────────────────────────────────────────
cat > /tmp/mount_pyq_route.js << 'ENDOFFILE'
const fs   = require('fs');
const path = require('path');
const file = path.join(require('os').homedir(), 'workspace/src/index.js');
if (!fs.existsSync(file)) {
  // Try alternate path
  const alt = path.join(require('os').homedir(), 'workspace/index.js');
  if (!fs.existsSync(alt)) { console.log('❌ index.js not found — mount manually:'); console.log("  const pyqBankAdminRoutes = require('./routes/pyqBankAdmin');"); console.log("  app.use('/api/pyq-bank', pyqBankAdminRoutes);"); process.exit(0); }
  file = alt;
}
let c = fs.readFileSync(file, 'utf8');
if (c.includes('pyqBankAdmin') || c.includes('pyq-bank')) {
  console.log('✅ PYQ Bank route already mounted in index.js');
  process.exit(0);
}
// Find a good insertion point — after another route require
const markers = ["const adminSystemRoutes", "const adminManagementRoutes", "const questionFeaturesRoutes", "app.use('/api"];
let insertAfter = null;
for (const m of markers) {
  if (c.includes(m)) { insertAfter = m; break; }
}
if (!insertAfter) { console.log('⚠️  Could not auto-mount. Mount manually:\n  const pyqBankAdminRoutes = require(\'./routes/pyqBankAdmin\');\n  app.use(\'/api/pyq-bank\', pyqBankAdminRoutes);'); process.exit(0); }

// Add require line near other requires
const requireLine = "const pyqBankAdminRoutes = require('./routes/pyqBankAdmin');\n";
const mountLine   = "app.use('/api/pyq-bank', pyqBankAdminRoutes);\n";

// Find last require statement position
const lastReqIdx = c.lastIndexOf("const ") ;
const requireSection = c.indexOf(insertAfter);
if (requireSection === -1) { console.log('⚠️  Insertion marker not found'); process.exit(0); }
// Insert require before marker
c = c.substring(0, requireSection) + requireLine + c.substring(requireSection);

// Find the app.use for the marker route and add mount after it
const appUseMatch = c.match(/app\.use\(['"]\/api\/[^'"]+['"],\s*\w+Routes\)/g);
if (appUseMatch) {
  const lastMount = appUseMatch[appUseMatch.length - 1];
  const mountPos = c.lastIndexOf(lastMount) + lastMount.length;
  c = c.substring(0, mountPos) + '\n' + mountLine + c.substring(mountPos);
} else {
  // Just append before exports or end
  c += '\n' + mountLine;
}
fs.writeFileSync(file, c);
console.log('✅ PYQ Bank route mounted in index.js at /api/pyq-bank');
ENDOFFILE
cd $WS && node /tmp/mount_pyq_route.js

# ─── 6. Verification ──────────────────────────────────────────────────────────
echo ""
echo "═══════════════════════════════════════════════════════"
echo "  PYQ Bank Backend — Verification"
echo "═══════════════════════════════════════════════════════"

check_file() {
  if [ -f "$1" ]; then echo "  ✅ $2"; else echo "  ❌ $2 — NOT FOUND: $1"; fi
}
check_content() {
  if grep -q "$2" "$1" 2>/dev/null; then echo "  ✅ $3"; else echo "  ❌ $3 — not found in $1"; fi
}

check_file "$WS/src/models/PYQExamCard.js"          "PYQExamCard.js model"
check_file "$WS/src/models/PYQYearCard.js"          "PYQYearCard.js model"
check_file "$WS/src/routes/pyqBankAdmin.js"         "pyqBankAdmin.js route"
check_content "$WS/src/models/Question.js"  "pyqExam"      "pyqExam field in Question model"
check_content "$WS/src/index.js" "pyq-bank"              "Route mounted in index.js"
check_content "$WS/src/routes/pyqBankAdmin.js" "exam-cards"    "Exam cards routes present"
check_content "$WS/src/routes/pyqBankAdmin.js" "PYQYearCard"   "Year card routes present"
check_content "$WS/src/routes/pyqBankAdmin.js" "copypaste"     "Copy-paste upload route"
check_content "$WS/src/routes/pyqBankAdmin.js" "upload/excel"  "Excel upload route"
check_content "$WS/src/routes/pyqBankAdmin.js" "upload/pdf"    "PDF upload route"
check_content "$WS/src/routes/pyqBankAdmin.js" "DEFAULT_CARDS" "9 Default exam cards seeded"
check_content "$WS/src/routes/pyqBankAdmin.js" "getExamStats"  "Stats helper function"

echo ""
echo "  API Routes Available:"
echo "  ✅ GET  /api/pyq-bank/exam-cards           — List all exam cards + stats"
echo "  ✅ POST /api/pyq-bank/exam-cards           — Create new exam card"
echo "  ✅ PUT  /api/pyq-bank/exam-cards/:id       — Update exam card"
echo "  ✅ DEL  /api/pyq-bank/exam-cards/:id       — Delete exam card"
echo "  ✅ GET  /api/pyq-bank/exam-cards/:n/stats  — Exam stats"
echo "  ✅ GET  /api/pyq-bank/exam-cards/:n/years  — Year cards list"
echo "  ✅ POST /api/pyq-bank/exam-cards/:n/years  — Create year card"
echo "  ✅ PUT  /api/pyq-bank/years/:id            — Update year card"
echo "  ✅ DEL  /api/pyq-bank/years/:id            — Delete year card + questions"
echo "  ✅ GET  /api/pyq-bank/years/:id/questions  — Get questions (filters)"
echo "  ✅ POST /api/pyq-bank/years/:id/questions  — Manual add question"
echo "  ✅ POST /api/pyq-bank/years/:id/upload/copypaste — Copy-paste upload"
echo "  ✅ POST /api/pyq-bank/years/:id/upload/excel     — Excel upload"
echo "  ✅ POST /api/pyq-bank/years/:id/upload/pdf       — PDF upload"
echo ""
echo "═══════════════════════════════════════════════════════"
echo "🎉 Backend setup COMPLETE — push to GitHub & redeploy Render"
echo "═══════════════════════════════════════════════════════"
