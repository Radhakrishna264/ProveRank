...
WIZARD_FILE="$DIR/CreateExamWizard.tsx"
TEMPLATES_FILE="$DIR/ExamTemplates.tsx"
echo "✓ Admin folder mila: $DIR"

if [ ! -f "$WIZARD_FILE" ]; then
  echo "❌ CreateExamWizard.tsx expected yahan tha par nahi mila: $WIZARD_FILE"
  exit 1
fi

# ── backup before any overwrite ───────────────────────────────────────────────
cp "$PAGE_FILE" "$PAGE_FILE.bak_feat29"
cp "$WIZARD_FILE" "$WIZARD_FILE.bak_feat29"
echo "✓ Backups bana diye (.bak_feat29)"
echo ""
# ── 1) ExamTemplates.tsx — NEW FILE (Templates Manager page, 29.1-29.16) ─────
echo "→ Writing $TEMPLATES_FILE ..."
cat > "$TEMPLATES_FILE" << '__PRRANK_EOF_TPL__'
'use client'
...
__PRRANK_EOF_WIZARD__

# ── 3) page.tsx — FULL REWRITE (Templates tab replaced + wizard props wired) ─
...
echo ""
echo "── Feature 29 verification (Frontend) ──"
pass=0; total=0
chk(){ total=$((total+1)); if grep -q "$1" "$2" 2>/dev/null; then echo "✅ $3"; pass=$((pass+1)); else echo "❌ $3"; fi }

chk "import ExamTemplates from './ExamTemplates'" "$PAGE_FILE"   "29     Templates Manager wired into admin panel"
chk "templates.map(t=>{"                          "$TEMPLATES_FILE" "29.1   Templates list page/section"
chk "titleFormat"                                 "$TEMPLATES_FILE" "29.2a  New Template — title format stored"
chk "subjectQs"                                   "$TEMPLATES_FILE" "29.2b  New Template — sections/marks/marking stored"
chk "fetchCategories"                             "$TEMPLATES_FILE" "29.3   Template categories (NEET/JEE/CUET/Custom)"
chk "usageCount"                                  "$TEMPLATES_FILE" "29.4   Usage count badge"
chk "duplicateTemplate"                           "$TEMPLATES_FILE" "29.5   Duplicate template"
chk "lastUsedAt"                                  "$TEMPLATES_FILE" "29.6   Last-used timestamp"
chk "setPreviewT"                                 "$TEMPLATES_FILE" "29.7   Template preview"
chk "togglePin"                                   "$TEMPLATES_FILE" "29.8   Pin favourite template"
chk "openVersions"                                "$TEMPLATES_FILE" "29.9a  Version history list"
...
chk "tpl-flash"                                   "$WIZARD_FILE"    "29.13c Subtle fill animation on Step 1"
chk "onApply="                                    "$PAGE_FILE"      "29.13d Templates page → Wizard Step 1 wiring"
chk "No templates yet"                            "$TEMPLATES_FILE" "29.14  Empty state"
