#!/bin/bash
set -e
echo "=== CORRECTED Fix: Warnings only log/track for proctoring — NEVER auto-submit. Only time expiry auto-submits. ==="
cat > ~/workspace/src/routes/antiCheatRoutes.js << 'FILEEOF1'
...
FILEEOF1
echo "antiCheatRoutes.js updated ✅"

cat > ~/workspace/frontend/app/exam/\[examId\]/attempt/page.tsx << 'FILEEOF2'
'use client'
...
export default function ExamAttempt() {
  ...
  const initAttempt = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const fallbackTotal = (qd.duration || 200) * 60
        const total = tr.ok && typeof td.totalDurationSec === 'number' ? td.totalDurationSec : fallbackTotal
        const remaining = tr.ok && typeof td.remainingSec === 'number' ? td.remainingSec : total
        // FIX — a FRESH attempt's remaining time should be ~equal to the total
        // duration. If it comes back near-zero, the exam would auto-submit
        // within 1 second with zero answers (the exact bug reported). Catch
        // it here and show an error instead of silently burning the attempt.
        if (isFreshAttempt && remaining < total * 0.9) {
          setErrorState({ code: 'TIMER_INVALID', message: t.errTitle + ' (timer returned ' + remaining + 's of ' + total + 's — please contact support, do not retry blindly as it may consume an attempt)' })
          setInitializing(false)
          return
        }
        // Resumed attempt whose time genuinely ran out while away — this is a
        // legitimate outcome, but it must be a clear, visible transition, not
        // a silent instant auto-submit.
        if (!isFreshAttempt && remaining <= 2) {
  ...
  useEffect(() => {
    ...
    // FIX — mirrors the backend's 4s grace: mobile browsers commonly fire a
    // spurious blur/visibility/fullscreen-exit burst together right at mount
    // while negotiating fullscreen (address bar, camera-permission dialog).
    // That is not real cheating, so we don't even show the warning UI for it.
    const inStartupGrace = () => Date.now() - mountedAt < 4000
    const logWarning = (type: string) => setWarningHistory(h => [{ type, at: new Date().toLocaleTimeString() }, ...h].slice(0, 15))

    const onVis = () => {
      if (document.hidden && attempt) {
        if (inStartupGrace()) return // FIX — ignore startup-glitch burst
        ...
        if (user && attempt?._id) {
          fetch(`${API}/api/anticheat/tab-switch`, {
            ...
            // NOTE — warnings are recorded for the proctoring report only.
            // They never auto-submit the exam; only time expiry does that.
          }).catch(() => {})
        }
      }
    }
    ...
    const onFsChange = () => {
      ...
      setFocusLocked(isFs)
      if (!isFs && attempt) {
        if (inStartupGrace()) return // FIX — mobile fullscreen negotiation glitch right at mount, not real cheating
  ...
  // FIX — exit fullscreen only on TRUE unmount (leaving the attempt page for
  // good), not on every re-run of the anti-cheat effect above (which would
  // otherwise flicker fullscreen off/on mid-exam whenever `attempt` or
  // `examMeta.fullscreenForce` update during normal hydration).
  useEffect(() => {
  ...
  const autoSubmit = async (isAuto: boolean = false) => {
    ...
    try {
      ...
      const d = await r.json()
      // FIX — fullscreen must not persist onto the Result page; only the
      // attempt screen should be fullscreen, and it must auto-exit on submit.
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#000A18', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div style={{ maxWidth: 420, textAlign: 'center', color: '#E8F4FF', fontFamily: 'Inter,sans-serif' }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 8 }}>{lang === 'hi' ? 'Time Khatam Ho Gaya' : "Time's Up"}</div>
          <div style={{ fontSize: 13, color: '#6B8BAF' }}>
            {lang === 'hi' ? 'Aapka pichla attempt ka time khatam ho chuka tha. Submit ho raha hai...' : 'Your previous attempt had already run out of time. Submitting now...'}
          </div>
        </div>
      </div>
    )
...
cd ~/workspace
git add -A
git commit -m "fix: warnings (tab-switch/blur/fullscreen-exit) now only logged for proctoring, never auto-submit — only time expiry auto-submits"
git push
