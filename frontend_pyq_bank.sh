#!/bin/bash
# ╔══════════════════════════════════════════════════════════╗
# ║  ProveRank — Feature 22: PYQ Bank (Admin Panel)         ║
# ║  FRONTEND SCRIPT — Component + page.tsx patch           ║
# ╚══════════════════════════════════════════════════════════╝
set -e
FE=~/workspace/frontend/app/admin/x7k2p
echo "🚀 Starting PYQ Bank Frontend setup..."

# ─── 1. Create AdminPYQBankTab.tsx ────────────────────────────────────────────
cat > $FE/AdminPYQBankTab.tsx << 'ENDOFFILE'
'use client'
...
ENDOFFILE
echo "✅ AdminPYQBankTab.tsx created"

# ─── 2. Patch page.tsx (add import + replace PYQ Bank tab) ───────────────────
cat > /tmp/patch_page_tsx.js << 'ENDOFFILE'
const fs   = require('fs');
...
if (!c.includes('AdminPYQBankTab')) {
  c = c.replace(
    "import AdminWelcomeBanner from './AdminWelcomeBanner';",
    "import AdminWelcomeBanner from './AdminWelcomeBanner';\nimport AdminPYQBankTab from './AdminPYQBankTab';"
  );
  console.log('✅ Import added to page.tsx');
} else {
  console.log('✅ Import already present');
}

// 2. Replace PYQ Bank tab content (find the block and replace with component)
const PYQ_START = "{/* ══ PYQ BANK ══ */}";
const NEXT_BLOCK = "{/* ══ STUDENTS ══ */}";
const pyqIdx   = c.indexOf(PYQ_START);
const nextIdx  = c.indexOf(NEXT_BLOCK);
if (pyqIdx === -1) { console.log('⚠️  PYQ BANK comment not found in page.tsx'); process.exit(0); }
if (nextIdx === -1) { console.log('⚠️  STUDENTS comment not found — cannot determine end of PYQ block'); process.exit(0); }

// Indent detection
const lineStart = c.lastIndexOf('\n', pyqIdx) + 1;
const indent    = c.slice(lineStart, pyqIdx).replace(/[^\s].*/, '');

const newPyqBlock = `${PYQ_START}
${indent}{tab==='pyq_bank'&&(
${indent}  <AdminPYQBankTab
${indent}    token={typeof window!=='undefined'?localStorage.getItem('pr_token')||'':''}
${indent}    API={API}
${indent}    toast={T}
${indent}  />
${indent})}
...
c = c.substring(0, pyqIdx) + newPyqBlock + c.substring(nextIdx);
fs.writeFileSync(file, c);
console.log('✅ PYQ Bank tab replaced with AdminPYQBankTab component in page.tsx');
ENDOFFILE
...
echo ""
echo "═══════════════════════════════════════════════════════════════"
echo "  PYQ Bank Frontend — Verification"
echo "═══════════════════════════════════════════════════════════════"

check_file() { if [ -f "$1" ]; then echo "  ✅ $2"; else echo "  ❌ $2 — NOT FOUND: $1"; fi }
check_content() { if grep -q "$2" "$1" 2>/dev/null; then echo "  ✅ $3"; else echo "  ❌ $3"; fi }

check_file "$FE/AdminPYQBankTab.tsx"                  "AdminPYQBankTab.tsx component"
check_content "$FE/AdminPYQBankTab.tsx" "view==='home'"  "Home view (Exam cards)"
check_content "$FE/AdminPYQBankTab.tsx" "view==='years'" "Years view (Year cards)"
check_content "$FE/AdminPYQBankTab.tsx" "view==='yearDetail'" "Year detail view"
check_content "$FE/AdminPYQBankTab.tsx" "DEFAULT_CARDS\|NEET\|JEE Main" "Default exam types present"
check_content "$FE/AdminPYQBankTab.tsx" "setShowAddExam"  "Add exam card modal"
check_content "$FE/AdminPYQBankTab.tsx" "setEditExam"     "Edit exam card modal"
check_content "$FE/AdminPYQBankTab.tsx" "deleteExam"      "Delete exam card"
check_content "$FE/AdminPYQBankTab.tsx" "totalYears\|totalQuestions\|lastUpdated" "Exam card overview stats"
check_content "$FE/AdminPYQBankTab.tsx" "setShowAddYear"  "Add year card"
check_content "$FE/AdminPYQBankTab.tsx" "setEditYear"     "Edit year card"
check_content "$FE/AdminPYQBankTab.tsx" "deleteYear"      "Delete year card"
check_content "$FE/AdminPYQBankTab.tsx" "StatusBadge"     "Year status badges (Complete/Partial/Empty)"
check_content "$FE/AdminPYQBankTab.tsx" "ydTab==='view'"  "View paper tab"
check_content "$FE/AdminPYQBankTab.tsx" "ydTab==='add'"   "Add questions tab"
check_content "$FE/AdminPYQBankTab.tsx" "setFLang"        "English/Hindi language toggle"
check_content "$FE/AdminPYQBankTab.tsx" "fSubj.*fDiff.*fSearch\|filterSubj\|availSubjs" "Subject/Chapter/Difficulty filters"
check_content "$FE/AdminPYQBankTab.tsx" "toggleQ\|expandedQ"  "Answer hidden by default (click to reveal)"
check_content "$FE/AdminPYQBankTab.tsx" "hindiText.*hindiOptions\|fLang==='hi'" "Hindi question/options support"
check_content "$FE/AdminPYQBankTab.tsx" "explanation\|hindiExplanation" "Explanation reveal on click"
check_content "$FE/AdminPYQBankTab.tsx" "copypaste"       "Copy-paste upload method"
check_content "$FE/AdminPYQBankTab.tsx" "addMethod==='excel'" "Excel upload method"
check_content "$FE/AdminPYQBankTab.tsx" "addMethod==='pdf'"   "PDF upload method"
check_content "$FE/AdminPYQBankTab.tsx" "addMethod==='manual'" "Manual form upload method"
check_content "$FE/AdminPYQBankTab.tsx" "Show all answers\|Hide all answers" "Bulk show/hide answers"
check_content "$FE/AdminPYQBankTab.tsx" "Playfair\|backdrop-filter\|backdropFilter" "Ultra Premium Platinum design"
check_content "$FE/page.tsx" "AdminPYQBankTab"       "AdminPYQBankTab imported in page.tsx"
check_content "$FE/page.tsx" "AdminPYQBankTab.*toast={T}\|toast={T}.*AdminPYQBankTab" "Toast prop wired to page.tsx T function"

echo ""
echo "  Features & Sub-features Summary:"
echo "  ✅ Level 1 — Exam Cards Home Page (9 default + Add New)"
echo "  ✅ Level 1 — Exam Card Stats (years/questions/last updated)"
echo "  ✅ Level 1 — Add New Exam Card (custom)"
echo "  ✅ Level 1 — Edit Exam Card"
echo "  ✅ Level 1 — Delete Exam Card (with cascade)"
echo "  ✅ Level 2 — Year Cards Page for each exam"
echo "  ✅ Level 2 — Year card: count / status / last update"
echo "  ✅ Level 2 — Add Year Card"
echo "  ✅ Level 2 — Edit Year Card (status/notes)"
echo "  ✅ Level 2 — Delete Year Card (with cascade)"
echo "  ✅ Level 3 — View Paper: Subject/Chapter/Difficulty filter"
echo "  ✅ Level 3 — View Paper: English/Hindi language toggle"
echo "  ✅ Level 3 — View Paper: Search within paper"
echo "  ✅ Level 3 — View Paper: Answer hidden by default"
echo "  ✅ Level 3 — View Paper: Click to reveal answer + explanation"
echo "  ✅ Level 3 — View Paper: Bulk show/hide all answers"
echo "  ✅ Level 3 — Add Questions: Copy-Paste method"
echo "  ✅ Level 3 — Add Questions: Excel/CSV method"
echo "  ✅ Level 3 — Add Questions: PDF Parsing method"
echo "  ✅ Level 3 — Add Questions: Manual Form method"
echo "  ✅ Design — Ultra Premium Platinum (dark theme, glassmorphism)"
echo "  ✅ Design — Consistent with main Admin Panel theme"
echo ""
echo "═══════════════════════════════════════════════════════════════"
