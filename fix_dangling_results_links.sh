#!/bin/bash
set -e
echo "=== Fix: 2 dangling /results links found in survivor pages ==="
cat > ~/workspace/frontend/app/exam-review/\[id\]/page.tsx << 'FILEEOF1'
'use client'
...
function ExamReviewContent() {
  ...
  return (
    <div style={{animation:'fadeIn .4s ease'}}>
      <div style={{marginBottom:16}}><a href="/attempt-history" style={{fontSize:12,color:C.primary,textDecoration:'none',display:'flex',alignItems:'center',gap:5}}>← {t('Back to Attempt History','परीक्षा इतिहास पर वापस')}</a></div>
...
FILEEOF1
echo "exam-review/[id]/page.tsx updated ✅"

cat > ~/workspace/frontend/app/attempt-history/page.tsx << 'FILEEOF2'
'use client'
import { useState, useEffect } from 'react'
import StudentShell, { useShell, C } from '@/src/components/StudentShell'
const API = process.env.NEXT_PUBLIC_API_URL || 'https://proverank.onrender.com'
...
FILEEOF2
echo "attempt-history/page.tsx updated ✅"

cd ~/workspace
git add -A
git commit -m "fix: 2 dangling /results links (exam-review Back button, attempt-history View button) after Results page deletion"
git push
