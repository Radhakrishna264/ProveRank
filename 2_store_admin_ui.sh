...
BASE=~/workspace
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🛒  ProveRank Store — ADMIN STORE TAB UI"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

# ─────────────────────────────────────────
# CREATE: StoreAdminTab.tsx
# ─────────────────────────────────────────
cat > $BASE/frontend/app/admin/x7k2p/StoreAdminTab.tsx << 'ENDOFFILE'
'use client';
...
ENDOFFILE
echo "✅ StoreAdminTab.tsx created"

# ─────────────────────────────────────────
# PATCH: Add Store tab to Admin page.tsx
# ─────────────────────────────────────────
node << 'ENDOFFILE'
const fs   = require('fs');
...
// 1. Add import
if (!content.includes('StoreAdminTab')) {
  content = content.replace("'use client';", "'use client';\nimport StoreAdminTab from './StoreAdminTab';");
  changed = true;
  console.log('✅ StoreAdminTab import added');
}

// 2. Add nav item — look for a pattern like 'changelog' or 'backup' or 'logout' in nav
const storeNavItem = `{ id:'store', label:'Store', icon:'🛒' },`;
if (!content.includes("'store'") && !content.includes('"store"')) {
  // Try to find nav array items
  const patterns = [
    "{ id:'logout'",    '{ id:"logout"',
    "{ id:'backup'",    '{ id:"backup"',
  ...
  for (const pat of patterns) {
    if (content.includes(pat)) {
      content = content.replace(pat, storeNavItem + '\n          ' + pat);
      inserted = true;
      console.log('✅ Store nav item added before:', pat);
      break;
    }
  }
  if (!inserted) {
    // Fallback: add before first closing bracket of a nav array
    // Look for icon pattern common in nav items
    const navPattern = /(\{[^}]*icon:[^}]*\}[,\s]*\n)(\s*\])/;
    if (navPattern.test(content)) {
      content = content.replace(navPattern, '$1' + "          " + storeNavItem + '\n$2');
      inserted = true;
      console.log('✅ Store nav item added (fallback pattern)');
    }
  }
  if (inserted) changed = true;
  else console.log('⚠️  Could not auto-insert nav item — add manually: ' + storeNavItem);
}

// 3. Add tab content render — look for the last {activeTab === '...' && ...} pattern
const storeTabRender = `\n        {activeTab === 'store' && <StoreAdminTab />}`;
if (!content.includes("activeTab === 'store'") && !content.includes('activeTab === "store"')) {
  const patterns2 = [
    "{activeTab === 'logout'", '{activeTab === "logout"',
    "{activeTab === 'backup'", '{activeTab === "backup"',
    "{activeTab === 'changelog'", '{activeTab === "changelog"',
    "{activeTab === 'audit'", '{activeTab === "audit"',
  ];
  let rendered = false;
  for (const pat of patterns2) {
    if (content.includes(pat)) {
      content = content.replace(pat, storeTabRender + '\n        ' + pat);
      rendered = true;
      console.log('✅ Store tab render added before:', pat);
      break;
    }
  }
  if (!rendered) {
    // Generic fallback: add before </main> or </div> near the end
    const mainClose = content.lastIndexOf('</main>');
    if (mainClose !== -1) {
      content = content.slice(0, mainClose) + storeTabRender + '\n      ' + content.slice(mainClose);
      rendered = true;
      console.log('✅ Store tab render added before </main>');
    }
  }
  if (rendered) changed = true;
  else console.log('⚠️  Could not auto-insert tab render — add manually in the tab content section');
}
...
if (changed) {
  ...
  console.log('ℹ️  page.tsx already has store tab');
}
ENDOFFILE

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "✅  ADMIN STORE TAB SETUP COMPLETE!"
echo ""
echo "📄 Files Created:"
echo "   frontend/app/admin/x7k2p/StoreAdminTab.tsx"
echo ""
echo "🔧 Admin page.tsx patched to include:"
echo "   - import StoreAdminTab"
echo "   - 🛒 Store nav item in sidebar"
echo "   - Store tab content render"
echo ""
echo "⚠️  If page.tsx patch failed, manually add:"
echo "   1. import StoreAdminTab from './StoreAdminTab';"
echo "   2. Nav item: { id:'store', label:'Store', icon:'🛒' }"
echo "   3. Render: {activeTab === 'store' && <StoreAdminTab />}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
