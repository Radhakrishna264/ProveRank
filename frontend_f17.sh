...
# FRONTEND SCRIPT — Run on Replit Shell
# Creates: SmartPaperGen.tsx + updates page.tsx
# ═══════════════════════════════════════════════════════════════

set -e
echo "🚀 ProveRank Feature 17 — Frontend Deploy Starting..."

# ════════════════════════════════════════════════════════════════
# 1. CREATE SmartPaperGen.tsx
# ════════════════════════════════════════════════════════════════
cat > ~/workspace/frontend/app/admin/x7k2p/SmartPaperGen.tsx << 'COMP_EOF'
'use client';
...
echo "✅ SmartPaperGen.tsx created"

# ════════════════════════════════════════════════════════════════
# 2. UPDATE page.tsx — add import + replace smart_gen section
# ════════════════════════════════════════════════════════════════
node << 'NODE_EOF'
const fs   = require('fs');
const path = require('path');

const fpath = path.join(process.env.HOME, 'workspace/frontend/app/admin/x7k2p/page.tsx');
if (!fs.existsSync(fpath)) { console.error('❌ page.tsx not found at:', fpath); process.exit(1); }

let content = fs.readFileSync(fpath, 'utf8');

// Add import (only once)
if (!content.includes("import SmartPaperGen")) {
...
// Replace smart_gen section (between the two markers)
const M1 = '{/* \u2550\u2550 SMART GENERATOR \u2550\u2550 */}';
const M2 = '{/* \u2550\u2550 PYQ BANK \u2550\u2550 */}';
const i1 = content.indexOf(M1);
const i2 = content.indexOf(M2);

if (i1 === -1) { console.error('❌ SMART GENERATOR marker not found'); process.exit(1); }
if (i2 === -1) { console.error('❌ PYQ BANK marker not found'); process.exit(1); }
if (i1 >= i2)  { console.error('❌ Markers in wrong order'); process.exit(1); }

const API_PROP = 'API={API} token={typeof window!==\'undefined\'?localStorage.getItem(\'pr_token\')||\'\':\'\'}';
const newSection =
`{/* \u2550\u2550 SMART GENERATOR \u2550\u2550 */}
          {tab==='smart_gen'&&(
            <SmartPaperGen ${API_PROP} />
          )}

          `;

content = content.slice(0, i1) + newSection + content.slice(i2);
fs.writeFileSync(fpath, content, 'utf8');
console.log('  ✅ smart_gen tab section replaced with <SmartPaperGen />');
...
# ════════════════════════════════════════════════════════════════
# 3. VERIFICATION — Feature 17 to 17.26
# ════════════════════════════════════════════════════════════════
node << 'VERIFY_EOF'
const fs   = require('fs');
const path = require('path');

const compPath = path.join(process.env.HOME, 'workspace/frontend/app/admin/x7k2p/SmartPaperGen.tsx');
const pagePath = path.join(process.env.HOME, 'workspace/frontend/app/admin/x7k2p/page.tsx');

if (!fs.existsSync(compPath)) { console.error('❌ SmartPaperGen.tsx NOT FOUND'); process.exit(1); }
const comp = fs.readFileSync(compPath, 'utf8');
const page = fs.existsSync(pagePath) ? fs.readFileSync(pagePath, 'utf8') : '';

const has = (pattern) => {
  if (typeof pattern === 'string') return comp.includes(pattern);
  return pattern.test(comp);
};

const checks = [
  ['17    ', 'Smart Paper Generator component',     () => comp.includes('SmartPaperGen')],
  ['17.1  ', 'Subject Multi-Select (17.1)',          () => has('ALL_SUBJECTS') && has('toggleSubject') && has('selSubjects')],
  ['17.2  ', 'Chapter Multi-Select (17.2)',          () => has('chapters') && has('chapter')],
  ['17.3  ', 'Difficulty Mix % input (17.3)',        () => has('difficultyMix') && has('easy') && has('medium') && has('hard')],
  ['17.4  ', 'Marking Scheme input (17.4)',          () => has('marking') && has('correct') && has('incorrect') && has('unattempted')],
  ['17.5  ', 'Subject-wise Counts preview (17.5)',   () => has('totalCountPreview') && has('subjectConfig')],
  ['17.6  ', 'Exam Template auto-fill (17.6)',       () => has("mode === 'neet'") && has("mode === 'jee'") && has("mode === 'cuet'")],
  ['17.7  ', 'S75 Template Sync (17.7)',             () => has('/api/exams/templates')],
  ['17.8  ', 'AI Smart Select from QB (17.8)',       () => has('/api/paper/generate') && has('smartSelect')],
  ['17.9  ', 'Multiple Sets A/B/C (17.9)',           () => has('setsCount') && has("Set A") && has("Set B")],
  ['17.10 ', 'Preview generated paper (17.10)',      () => has('activeSetData') && has('activeSet')],
  ['17.11 ', 'Use as Exam + batch (17.11)',          () => has('useAsExam') && has('/api/paper/use-as-exam') && has('uaeBatch')],
  ['17.12 ', 'Exclude Used Qs toggle+% (17.12)',    () => has('exUsed') && has('exUsedPct') && has('excludeUsed')],
  ['17.13 ', 'Exclude PYQ toggle+% (17.13)',        () => has('exPYQ') && has('exPYQPct') && has('excludePYQ')],
  ['17.14 ', 'answerKey + explanation DB (17.14)',   () => has('answerKey') && has('explanation') && has('correct')],
  ['17.15 ', 'Surprise Me mode (17.15)',             () => has('surpriseMode') && has('Surprise Me')],
  ['17.16 ', 'Difficulty auto-balance (17.16)',      () => has('auto-balanced') || has('auto_balance') || has('autoBalanced')],
  ['17.17 ', 'Set comparison overlap (17.17)',       () => has('setComparison') && has('overlap') && has('showComp')],
  ['17.18 ', 'Save as template (17.18)',             () => has('saveTemplate') && has('savedTmpls') && has('Save Template')],
  ['17.19 ', 'Export PDF + Excel (17.19)',           () => has("handleExport('pdf')") && has("handleExport('excel')")],
  ['17.20 ', '2-column wizard layout (17.20)',       () => has("gridTemplateColumns:'1fr 1fr'")],
  ['17.21 ', 'Animated progress bar (17.21)',        () => has('progress') && has('AI Selection in Progress')],
  ['17.22 ', 'Set A/B/C tabs + shuffle (17.22)',     () => has('setLabel') && has('🔀')],
  ['17.23 ', 'Criteria chips/badges (17.23)',        () => has('S.chip') && has('Active Criteria')],
  ['17.24 ', 'Green checkmark on confirm (17.24)',   () => has("'✅ Create Exam'") || has('checkmark') || has('uaeSuccess')],
  ['17.25 ', 'Ultra premium design (17.25)',         () => has('gradient') && has('backdropFilter') && has('glassmorphism') || (has('linear-gradient') && has('backdropFilter'))],
  ['17.26 ', 'Question Formats multi-select+% (17.26)', () => has('ALL_FORMATS') && has('fmtConfs') && has('Assertion_Reason') && has('Match_Column')],
];

// page.tsx checks
const pageChecks = [
  ['page  ', 'SmartPaperGen imported in page.tsx', () => page.includes("import SmartPaperGen")],
  ['page  ', '<SmartPaperGen /> in smart_gen tab', () => page.includes('<SmartPaperGen')],
];

console.log('\n' + '═'.repeat(60));
console.log('  ProveRank — Feature 17 VERIFICATION REPORT');
console.log('═'.repeat(60));

let allPass = true;
[...checks, ...pageChecks].forEach(([code, label, fn]) => {
  let ok = false;
  try { ok = fn(); } catch(e) { ok = false; }
  if (!ok) allPass = false;
  const icon = ok ? '✅' : '❌';
  console.log(`  ${icon}  Feature ${code}— ${label}`);
});

console.log('═'.repeat(60));
if (allPass) {
  console.log('  🎉 ALL FEATURES 17 → 17.26 IMPLEMENTED SUCCESSFULLY!');
} else {
  console.log('  ⚠️  Some features need attention (see ❌ above)');
}
console.log('═'.repeat(60) + '\n');
VERIFY_EOF
