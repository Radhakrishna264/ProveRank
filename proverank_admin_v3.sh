#!/bin/bash
# ProveRank Admin Panel V3 — Complete Rebuild
# Rule C1: cat > EOF style | Rule C2: NO sed -i
# Fix-1: Mobile keyboard (React.memo + useRef)
# Fix-2: Question upload (3 endpoint fallbacks)
# Fix-3: All 62 admin features active + real API
# Usage: bash proverank_admin_v3.sh
...
mkdir -p $FE/app/admin/x7k2p || err "Cannot create directory"
step "Writing Admin Panel V3 — page.tsx"
warn "Purani file replace ho rahi hai"
...
log "page.tsx written!"


step "Step 2: Verifying file"
if [ -f "$FE/app/admin/x7k2p/page.tsx" ]; then
  LINES=$(wc -l < $FE/app/admin/x7k2p/page.tsx)
  log "File created: $FE/app/admin/x7k2p/page.tsx ($LINES lines)"
else
  echo "File NOT created — check FE path"
  exit 1
fi

step "Done!"
echo ""
echo -e "${G}ProveRank Admin Panel V3 READY!${N}"
echo ""
echo -e "${B}3 Fixes Applied:${N}"
echo -e "  ${G}[1]${N} Mobile Keyboard: SInput/STextarea = memo+useRef"
echo -e "  ${G}[2]${N} Question Upload: 3 endpoint fallbacks each method"
echo -e "  ${G}[3]${N} All 62 features active + real API wired"
echo ""
echo -e "${Y}Steps:${N}"
echo -e "  1. cd ~/workspace/frontend && npm run dev"
echo -e "  2. http://localhost:3000/admin/x7k2p"
echo -e "  3. Login: admin@proverank.com / ProveRank@SuperAdmin123"
