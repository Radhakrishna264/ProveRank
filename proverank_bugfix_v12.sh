...
export default function AdminPanel() {
  ...
  // ── Textarea refs (uncontrolled) — fixes mobile keyboard disappear bug ──
  ...
  const manualQLive = React.useRef('')
  const answerKeyLive = React.useRef('')
  ...
  const uploadQuestions = async()=>{
    ...
    try{
      ...
      if(qUploadMethod==='copypaste'||qUploadMethod==='manual'){
        const liveQText=manualQLive.current||manualQText;const liveAKey=answerKeyLive.current||answerKeyText;if(!liveQText.trim()){showToast('Paste question text first','error');setUploadingQ(false);return}
        res = await fetch(`${API}/api/upload/copypaste/questions`,{
          method:'POST',
          headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},
          body:JSON.stringify({examId:createdExamId, text:liveQText, answerKey:liveAKey})
        })
  ...
  const sendAnnounce = async()=>{
    const announceVal=announceRef.current?.value||'';if(!announceVal){showToast('Write announcement first','error');return}
    try{
      let res = await fetch(`${API}/api/admin/announce`,{
        method:'POST', headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},
        body:JSON.stringify({message:announceVal,batch:announceBatch})
      })
      if(!res.ok){
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(320px,1fr))',gap:16,marginBottom:16}}>
          <Card>
            ...
            <div style={{padding:'16px 20px'}}>
              <textarea ref={announceRef} rows={3} placeholder="Type announcement..." style={{width:'100%',padding:'10px 13px',borderRadius:9,border:`1.5px solid ${iBrd}`,background:iBg,color:tm,fontSize:13,fontFamily:'Inter,sans-serif',resize:'none',outline:'none',boxSizing:'border-box'}}/>
          ...
          <Card>
            ...
            <div style={{padding:'12px 16px'}}>
              <div style={{display:'flex',gap:8,marginBottom:10}}>
                <input defaultValue={todoInput} onInput={e=>setTodoInput((e.target as HTMLInputElement).value)} onKeyDown={e=>e.key==='Enter'&&addTodo()} placeholder="Add task..." style={{flex:1,padding:'7px 11px',borderRadius:8,border:`1px solid ${iBrd}`,background:iBg,color:tm,fontSize:12,fontFamily:'Inter,sans-serif',outline:'none'}}/>
    ...
    <Card>
      <CardHeader title={`📋 All Exams (${exams.length})`} action={
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <input defaultValue={examSearchFilter} onInput={e=>setExamSearchFilter((e.target as HTMLInputElement).value)} placeholder="Search exams..." style={{padding:'7px 12px',borderRadius:8,border:`1px solid ${iBrd}`,background:iBg,color:tm,fontSize:12,fontFamily:'Inter,sans-serif',outline:'none',width:180}}/>
      ...
      {examStep===2 && (
        <Card style={{maxWidth:700}}>
          ...
          <div style={{padding:'20px'}}>
            ...
            <div style={{display:(qUploadMethod==='manual'||qUploadMethod==='copypaste')?'block':'none'}}>
            ...
            <div style={{display:qUploadMethod==='excel'?'block':'none'}}>
                <div style={{padding:'14px',background:'rgba(0,196,140,0.04)',borderRadius:10,border:'1px solid rgba(0,196,140,0.2)',marginBottom:16}}>
                  <div style={{fontSize:12,color:'#00C48C',fontWeight:700,marginBottom:6}}>📊 Excel Format (POST /api/excel/questions)</div>
            ...
            <div style={{display:qUploadMethod==='pdf'?'block':'none'}}>
                <div style={{padding:'14px',background:'rgba(168,85,247,0.04)',borderRadius:10,border:'1px solid rgba(168,85,247,0.2)',marginBottom:16}}>
                  <div style={{fontSize:12,color:'#A855F7',fontWeight:700,marginBottom:6}}>📄 PDF Parser (POST /api/upload/pdf/questions)</div>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  <textarea value={answerKeyText} onChange={e=>{e.stopPropagation();answerKeyLive.current=e.target.value;setAnswerKeyText(e.target.value)}} rows={4} placeholder="1-B, 2-A, 3-D ..." style={{width:'100%',padding:'10px 13px',borderRadius:9,border:`1.5px solid ${iBrd}`,background:iBg,color:tm,fontSize:12,fontFamily:'monospace',resize:'none',outline:'none',boxSizing:'border-box'}}/>
                </div>
            </div>
  ...
  const renderAllStudents = ()=>(
    <Card>
      <CardHeader title={`👥 All Students (${students.length})`} action={
        <input defaultValue={searchQuery} onInput={e=>setSearchQuery((e.target as HTMLInputElement).value)} placeholder="Search..." style={{padding:'7px 12px',borderRadius:8,border:`1px solid ${iBrd}`,background:iBg,color:tm,fontSize:12,fontFamily:'Inter,sans-serif',outline:'none',width:180}}/>
      }/>
      ...
      <Card>
        ...
        <div style={{padding:'20px'}}>
          <div style={{marginBottom:14}}>
            ...
            <textarea ref={announceRef} rows={5} placeholder="Write your announcement..." style={{width:'100%',padding:'10px 13px',borderRadius:9,border:`1.5px solid ${iBrd}`,background:iBg,color:tm,fontSize:13,fontFamily:'Inter,sans-serif',resize:'none',outline:'none',boxSizing:'border-box'}}/>
          </div>
    ...
    <Card style={{maxWidth:560}}>
      ...
      <div style={{padding:'24px'}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <textarea ref={seoDescRef} rows={3} style={{width:'100%',padding:'10px 13px',borderRadius:9,border:`1.5px solid ${iBrd}`,background:iBg,color:tm,fontSize:13,fontFamily:'Inter,sans-serif',resize:'none',outline:'none',boxSizing:'border-box'}}/>
    ...
    <Card>
      ...
      <div style={{padding:'20px'}}>
        <div style={{display:'flex',gap:8,marginBottom:16}}>
          <input defaultValue={todoInput} onInput={e=>setTodoInput((e.target as HTMLInputElement).value)} onKeyDown={e=>e.key==='Enter'&&addTodo()} placeholder="Add new task..." style={{flex:1,padding:'10px 13px',borderRadius:9,border:`1.5px solid ${iBrd}`,background:iBg,color:tm,fontSize:13,fontFamily:'Inter,sans-serif',outline:'none'}}/>
