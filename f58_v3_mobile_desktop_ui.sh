      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          {isInteger ? (
            <div style={{ marginBottom: 28 }}>
              ...
              <input
                ...
                style={{ width: '100%', maxWidth: isMobile ? '100%' : 260, padding: isMobile ? '16px 18px' : '12px 16px', borderRadius: 10, border: `1.5px solid ${bord}`, background: dark ? 'rgba(0,22,40,0.5)' : '#fff', color: tm, fontSize: 17 }}
              />
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 10 : 12, marginBottom: 28 }}>
              {opts.map(opt => {
                ...
                return (
                  <div key={opt} onClick={() => q && selectOption(q, opt)}
                    style={{ display: 'flex', alignItems: 'center', gap: 14, padding: isMobile ? '18px 18px' : '14px 20px', minHeight: isMobile ? 30 : undefined, borderRadius: 12, border: `1.5px solid ${isSelected ? '#4D9FFF' : bord}`, background: isSelected ? 'rgba(77,159,255,0.1)' : (dark ? 'rgba(0,22,40,0.5)' : '#fff'), cursor: 'pointer', transition: 'all .2s' }}>
        ...
        {isMobile && q && (
          <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: headerBg, borderTop: `1px solid ${bord}`, display: 'flex', alignItems: 'center', padding: '10px 12px', gap: 8, zIndex: 130, boxShadow: '0 -8px 24px rgba(0,0,0,0.35)' }}>
            <button onClick={() => toggleFlag(q)} style={{ width: 46, height: 46, borderRadius: 12, border: `1.5px solid rgba(168,85,247,0.5)`, background: flagged.has(q._id) ? 'rgba(168,85,247,0.2)' : 'transparent', color: '#A855F7', fontSize: 18, flexShrink: 0 }}>🔖</button>
            <button onClick={() => clearResponse(q)} style={{ width: 46, height: 46, borderRadius: 12, border: `1.5px solid ${bord}`, background: 'transparent', color: tm, fontSize: 17, flexShrink: 0 }}>🗑</button>
            ...
            <button onClick={() => goRelative(1)} style={{ flex: 1.6, height: 46, borderRadius: 12, border: 'none', background: 'linear-gradient(135deg,#4D9FFF,#0055CC)', color: '#fff', fontWeight: 700, fontSize: 13 }}>{t.saveNext} →</button>
          </div>
        )}
      </div>
