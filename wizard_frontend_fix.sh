#!/bin/bash
# ============================================================================
# ProveRank — Create Batch / Create Test Series Wizard Overhaul (FRONTEND)
# - Removes Batch/Series Code field, Cover Icon field
# - Updates Exam dropdown to new exam list
# - Removes Lifecycle Mode + Enrollment Rule Builder step
# - Removes Discount Price / Free Trial / Bundle Pricing (Base Price stays)
# - Removes Default Controls step (Spotlight / Auto-Archive)
# - Renames final CTA "Publish Batch/Series" -> "Confirm" (creates as draft,
#   hidden from students until manually launched from the card)
# Run from frontend project ROOT on Replit.
# ============================================================================
set -e
echo "🚀 Create Wizard Overhaul — Frontend Install Starting..."

BATCH_MGR=$(grep -rl "function CreateBatchWizard" --include="*.tsx" . 2>/dev/null | head -1)
SERIES_MGR=$(grep -rl "function CreateSeriesWizard" --include="*.tsx" . 2>/dev/null | head -1)

if [ -z "$BATCH_MGR" ]; then echo "❌ BatchManagerUltra.tsx not found (searched for 'CreateBatchWizard')."; exit 1; fi
if [ -z "$SERIES_MGR" ]; then echo "❌ TestSeriesManagerUltra.tsx not found (searched for 'CreateSeriesWizard')."; exit 1; fi
echo "📍 $BATCH_MGR"
echo "📍 $SERIES_MGR"

cp "$BATCH_MGR" "$BATCH_MGR.pre-wizardfix-bak"
cp "$SERIES_MGR" "$SERIES_MGR.pre-wizardfix-bak"
echo "📦 Backups created (.pre-wizardfix-bak)"

# NOTE: full-file overwrite (matches your exact current file content, tested + tsc-validated)
cat > "$BATCH_MGR" << 'WZFIX_BATCH_EOF'
'use client'
// ══════════════════════════════════════════════════════════════════
...
WZFIX_BATCH_EOF
echo "✅ BatchManagerUltra.tsx updated ($(wc -l < "$BATCH_MGR") lines)"

cat > "$SERIES_MGR" << 'WZFIX_SERIES_EOF'
'use client'
// ══════════════════════════════════════════════════════════════════
  ...
  return (
    <Modal onClose={onClose} width={640}>
      ...
      {step === 3 && (
        <div>
          <div style={{ ...cs, marginBottom: 0 }}>
            <div style={{ fontWeight: 700, color: '#93C5FD', fontSize: 14 }}>{form.name || '(Unnamed Series)'}</div>
            <div style={{ fontSize: 11, color: DIM, marginTop: 4 }}>{form.examType} · Seat Limit: {form.seatLimit || 'Unlimited'} · {form.visibility}</div>
            <div style={{ fontSize: 11, color: DIM, marginTop: 4 }}>Price: ₹{form.price}</div>
          </div>
          {dupWarn && (
...
WZFIX_SERIES_EOF
echo "✅ TestSeriesManagerUltra.tsx updated ($(wc -l < "$SERIES_MGR") lines)"
...
if command -v tsc >/dev/null 2>&1 || command -v npx >/dev/null 2>&1; then
  ...
  $TSC_CMD --noEmit --jsx preserve --target es2020 --module esnext --moduleResolution bundler --allowJs --skipLibCheck --esModuleInterop "$BATCH_MGR" "$SERIES_MGR" > /tmp/wzfix_tsc.txt 2>&1 || true
  ERRS=$(grep -E "error TS1[0-9]{3}:" /tmp/wzfix_tsc.txt || true)
  if [ -n "$ERRS" ]; then
    echo "❌ SYNTAX ERRORS — restoring backups:"
    echo "$ERRS"
    cp "$BATCH_MGR.pre-wizardfix-bak" "$BATCH_MGR"
    cp "$SERIES_MGR.pre-wizardfix-bak" "$SERIES_MGR"
    exit 1
  else
...
PASS=0; FAIL2=0
check(){ D="$1"; F="$2"; P="$3"
  if grep -q -- "$P" "$F" 2>/dev/null; then echo "✅ $D"; PASS=$((PASS+1))
  else echo "❌ $D"; FAIL2=$((FAIL2+1)); fi
}
for LABEL in "Batch:$BATCH_MGR:CreateBatchWizard" "Series:$SERIES_MGR:CreateSeriesWizard"; do
  NAME=$(echo "$LABEL" | cut -d: -f1); F=$(echo "$LABEL" | cut -d: -f2); FN=$(echo "$LABEL" | cut -d: -f3)
  WIZARD_BODY=$(sed -n "/^function $FN/,/^}/p" "$F")
  echo "── $NAME wizard (create-flow only — Pricing/Controls tabs on the detail page are untouched by design) ──"
  ! grep -q "Batch Code\|Series Code" <<< "$WIZARD_BODY" && echo "✅ 1) Batch/Series Code field removed" && PASS=$((PASS+1)) || { echo "❌ 1) Code field still present"; FAIL2=$((FAIL2+1)); }
  ! grep -q "Cover Icon" <<< "$WIZARD_BODY" && echo "✅ 2) Cover Icon field removed" && PASS=$((PASS+1)) || { echo "❌ 2) Cover Icon field still present"; FAIL2=$((FAIL2+1)); }
  grep -q "NEET UG', 'NEET PG" <<< "$WIZARD_BODY" && echo "✅ 3) New exam list present" && PASS=$((PASS+1)) || { echo "❌ 3) New exam list missing"; FAIL2=$((FAIL2+1)); }
  ! grep -q "Lifecycle Mode\|Enrollment Rule Builder" <<< "$WIZARD_BODY" && echo "✅ 4) Lifecycle Mode + Enrollment Rule Builder removed" && PASS=$((PASS+1)) || { echo "❌ 4) Lifecycle/Enrollment step still present"; FAIL2=$((FAIL2+1)); }
  grep -q "Base Price" <<< "$WIZARD_BODY" && echo "✅ 5a) Base Price field still present" && PASS=$((PASS+1)) || { echo "❌ 5a) Base Price field missing"; FAIL2=$((FAIL2+1)); }
  ! grep -q "Discount Price\|Enable Free Trial\|Bundle Pricing" <<< "$WIZARD_BODY" && echo "✅ 5b) Discount/Trial/Bundle fields removed from wizard" && PASS=$((PASS+1)) || { echo "❌ 5b) Discount/Trial/Bundle fields still present in wizard"; FAIL2=$((FAIL2+1)); }
  ! grep -q "Spotlight (Featured)\|Auto-Archive After End Date" <<< "$WIZARD_BODY" && echo "✅ 6) Default Controls step removed from wizard" && PASS=$((PASS+1)) || { echo "❌ 6) Default Controls step still present in wizard"; FAIL2=$((FAIL2+1)); }
  grep -q "✅ Confirm" <<< "$WIZARD_BODY" && echo "✅ 7a) Final CTA is Confirm" && PASS=$((PASS+1)) || { echo "❌ 7a) Confirm CTA missing"; FAIL2=$((FAIL2+1)); }
  ! grep -q "Publish Batch\|Publish Series" <<< "$WIZARD_BODY" && echo "✅ 7b) No 'Publish' CTA text remains in wizard" && PASS=$((PASS+1)) || { echo "❌ 7b) 'Publish' CTA text still present in wizard"; FAIL2=$((FAIL2+1)); }
done

echo "═══════════════════════════════════════════════════════"
echo "RESULT: $PASS PASSED / $((PASS+FAIL2)) TOTAL"
if [ "$FAIL2" -eq 0 ]; then
  echo "🎉 ALL 7 REQUESTED CHANGES VERIFIED ON BOTH WIZARDS ✅"
else
  echo "⚠️  $FAIL2 check(s) need attention — see ❌ above"
fi
echo "═══════════════════════════════════════════════════════"
echo ""
echo "⚠️  Restart your Next.js dev server."
echo "📦 Rollback: cp \"\$BATCH_MGR.pre-wizardfix-bak\" \"\$BATCH_MGR\" && cp \"\$SERIES_MGR.pre-wizardfix-bak\" \"\$SERIES_MGR\""
