#!/bin/bash
set -e
echo "=== Fix 1: Update questions.js with auto-translation ==="

cat > ~/workspace/src/routes/questions.js << 'EOF'
...
echo "✅ questions.js updated"

echo ""
echo "=== Fix 2: Upgrade /translate route to use AI (not dictionary) ==="

# Patch only the /:id/translate route in questionFeaturesRoutes.js using Node.js
cat > /tmp/fix_translate_route.js << 'JSEOF'
const fs = require('fs');
const path = require('path');

const filePath = path.join(process.env.HOME, 'workspace/src/routes/questionFeaturesRoutes.js');
let code = fs.readFileSync(filePath, 'utf8');

// Replace the dictionary-based translate route with AI-based
const OLD = `// ✅ STEP 19 - AI-8: Hindi-English Auto Translator
...
const NEW_START = `// ✅ STEP 19 - AI-8: Hindi-English Auto Translator (AI-powered)
router.post('/:id/translate', verifyToken, isAdmin, async (req, res) => {
  try {
    const question = await Question.findById(req.params.id);
    if (!question) return res.status(404).json({ success: false, message: 'Question not found' });`;

if (!code.includes(OLD.slice(0, 100))) {
  console.log('⚠️  translate route pattern not found — skipping (may already be updated)');
  process.exit(0);
}

// Find start and end of the old translate route
const routeStart = code.indexOf('// ✅ STEP 19 - AI-8: Hindi-English Auto Translator');
const routeEnd   = code.indexOf('\n// Bulk translate all', routeStart);

if (routeStart === -1 || routeEnd === -1) {
  console.log('⚠️  Could not locate route boundaries — skipping');
  process.exit(0);
}

const newRoute = `// ✅ STEP 19 - AI-8: Hindi-English Auto Translator (AI-powered via 20-Layer Fallback)
router.post('/:id/translate', verifyToken, isAdmin, async (req, res) => {
  try {
    const question = await Question.findById(req.params.id);
    if (!question) return res.status(404).json({ success: false, message: 'Question not found' });
    ...
    res.json({
      success: true,
      message: 'AI Hindi translation complete!',
...
code = code.slice(0, routeStart) + newRoute + code.slice(routeEnd);

// Also upgrade bulk translate
code = code.replace(
  "q.hindiText = words.map(w => enToHi[w] || w).filter(w => w).join(' ');\n        q.translatedBy = 'AI-8-Bulk';",
  `const { translateQuestionToHindi } = require('../services/aiTranslationService');
      try {
        const r = await translateQuestionToHindi(q.text, q.options || [], q.explanation || '');
        q.hindiText        = r.hindiText        || '';
        q.hindiOptions     = r.hindiOptions     || [];
        q.hindiExplanation = r.hindiExplanation || '';
        q.translatedBy     = 'AI-8-Bulk-Groq';
      } catch(te) {
        console.log('[BulkTranslate] ⚠️ ' + q._id + ': ' + te.message);
        q.hindiText    = q.text;
        q.translatedBy = 'AI-8-Bulk-Fallback';
      }`
);

fs.writeFileSync(filePath, code, 'utf8');
console.log('✅ questionFeaturesRoutes.js translate route upgraded to AI');
JSEOF

node /tmp/fix_translate_route.js

echo ""
echo "=== Fix 3: Ensure Question model has hindiOptions + hindiExplanation fields ==="

cat > /tmp/fix_question_model.js << 'JSEOF'
const fs = require('fs');
const path = require('path');

// Find Question model file
const candidates = [
  path.join(process.env.HOME, 'workspace/src/models/Question.js'),
  path.join(process.env.HOME, 'workspace/src/models/question.js'),
];
...
if (!filePath) {
  console.log('⚠️  Question model not found at expected paths — check manually');
  process.exit(0);
}

let code = fs.readFileSync(filePath, 'utf8');

// Check if hindiOptions already exists
if (code.includes('hindiOptions')) {
  console.log('✅ Question model already has hindiOptions — no change needed');
  process.exit(0);
}

// Add hindiOptions + hindiExplanation after hindiText field
const insertAfter = 'hindiText';
// Find the hindiText line and add fields after it
code = code.replace(
  /hindiText\s*:\s*\{[^}]*\}/,
  (match) => match + `,\n  hindiOptions:     { type: [String], default: [] },\n  hindiExplanation: { type: String, default: '' },\n  translatedBy:     { type: String },\n  translatedAt:     { type: Date }`
);

fs.writeFileSync(filePath, code, 'utf8');
console.log('✅ Question model updated: hindiOptions + hindiExplanation added');
JSEOF

node /tmp/fix_question_model.js

echo ""
echo "=== All backend fixes done! ==="
echo "Verify by running: node ~/workspace/src/index.js"
