#!/bin/bash
# ProveRank — Restore "My Test Series" (formerly My Batches) — backend,
# frontend page, and sidebar nav link. Deep-dive analysis confirmed this
# page (progress tracking, streak, renewals, certificates, activity feed,
# leaderboard, compare) was genuinely shared Batch+TestSeries functionality
# that Pass 1 removed. Restored here Series-only, same endpoint paths.
# NOTE: studentBatchWorkspace.js is NOT restored here — nothing in the
# current frontend calls it (the "Continue" button on this page was
# already just a "Coming Soon" alert stub before Pass 1), so it's not
# urgent. Can be reviewed separately if you want that deeper feature built.
set -e
cd ~/workspace
...
cp src/index.js ~/workspace/.pre_batch_removal_backup/index_before_mybatches_restore_$ts.js
cp frontend/src/components/StudentShell.tsx ~/workspace/.pre_batch_removal_backup/StudentShell_before_mybatches_restore_$ts.tsx
echo "Backup saved."

echo "═══════════════════════════════════════════"
echo "STEP 1 — Write restored backend: src/routes/myBatches.js"
echo "═══════════════════════════════════════════"
cat > src/routes/myBatches.js << 'FILEEOF'
...
async function findEnrolledDoc(id){
  const doc=await TestSeries.findById(id).lean();
...
// GET /api/my-batches — all enrolled series with meta (+ search/filter/sort)
...
router.post('/:id/renew',auth,async(req,res)=>{
  try{
    ...
    let meta=user?.enrolledBatchesMeta||[];
    const idx=meta.findIndex(m=>m.batchId?.toString()===req.params.id);
    if(idx<0)return res.status(404).json({error:'You are not enrolled in this series — enroll first before renewing'});
    const now=new Date();
...
// GET /api/my-batches/compare?ids=a,b — compare 2 enrolled series
...
// GET /api/my-batches/activity-feed — combined activity feed across all enrolled series
...
// GET /api/my-batches/:batchId/leaderboard — series-specific leaderboard
...
FILEEOF
echo "OK — wrote src/routes/myBatches.js"

echo "═══════════════════════════════════════════"
echo "STEP 2 — Write restored backend: src/routes/batchActivityRoutes.js"
echo "═══════════════════════════════════════════"
cat > src/routes/batchActivityRoutes.js << 'FILEEOF'
const express  = require('express');
const router   = express.Router();
const jwt      = require('jsonwebtoken');
const mongoose = require('mongoose');
...
// GET /api/batch-activity/:batchId — get activities for a series
...
module.exports = router;
FILEEOF
echo "OK — wrote src/routes/batchActivityRoutes.js"

echo "═══════════════════════════════════════════"
echo "STEP 3 — Write restored frontend page (My Test Series)"
echo "═══════════════════════════════════════════"
mkdir -p frontend/app/dashboard/my-batches
cat > frontend/app/dashboard/my-batches/page.tsx << 'FILEEOF'
'use client'
  ...
  const requestNotifPermission=async()=>{
    ...
    if(perm==='granted'){
      new Notification('✅ ProveRank Notifications Enabled!',{
        body:'You will now receive streak reminders and test series updates.',
        icon:'/favicon.ico'
      })
    }
  }
      ...
      <div style={{position:'relative',zIndex:2,padding:'20px 16px 80px',maxWidth:880,margin:'0 auto'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',gap:16,marginBottom:28,animation:'fadeIn 0.5s ease'}}>
          <div>
            ...
            <div style={{fontFamily:'Playfair Display,serif',fontSize:'clamp(24px,6vw,32px)',fontWeight:800,color:TEXT,lineHeight:1.1}}>My Test Series</div>
          </div>
        ...
        <div style={{marginBottom:18}}>
          <div style={{display:'flex',gap:8}}>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search test series, exam, subject…" style={{...inp,flex:1}} />
        ...
        {loading?(
          ...
          <div style={{textAlign:'center',padding:'56px 16px',background:CARD,border:`1px solid ${BORDER}`,borderRadius:20}}>
            ...
            <div style={{fontFamily:'Playfair Display,serif',fontSize:19,fontWeight:700,color:TEXT,marginBottom:8}}>{tab==='wishlist'?'No saved test series yet':tab==='completed'?'No completed test series yet':'No active test series yet'}</div>
            <div style={{fontSize:12,color:SUB,marginBottom:24}}>{tab==='wishlist'?'Save test series you like from the marketplace':'Enroll in a test series to start your journey'}</div>
            <button onClick={()=>router.push('/dashboard/test-series')} style={{background:'linear-gradient(135deg,#4D9FFF,#00D4FF)',border:'none',borderRadius:12,padding:'12px 28px',color:'#031018',fontWeight:800,cursor:'pointer',fontSize:13}}>Explore Test Series →</button>
          </div>
        ):(
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            {filtered.map((b,i)=>{
              ...
              return (
                <div key={b._id} style={{
                  ...
                  {b.isExpired&&<div style={{position:'absolute',top:0,left:0,right:0,background:'rgba(231,76,60,0.15)',padding:'6px 14px',fontSize:10,fontWeight:700,color:'#E74C3C',textAlign:'center',letterSpacing:0.3}}>Series expired — renew to continue</div>}
                  ...
                  {b.isCompleted&&<div style={{position:'absolute',top:0,left:0,right:0,background:'rgba(255,215,0,0.12)',padding:'6px 14px',fontSize:10,fontWeight:700,color:'#FFD700',textAlign:'center',letterSpacing:0.3}}>Series completed</div>}
                  ...
                  <div style={{marginTop:(b.isExpired||b.daysLeft<=7||b.isCompleted)?20:0}}>
                    ...
                    <div style={{display:'flex',gap:8,marginTop:16}}>
                      {tab==='wishlist'?(
                        ...
                        <button onClick={()=>{accessBatch(b._id);alert('Test Series Workspace Coming Soon')}}
...
FILEEOF
echo "OK — wrote frontend/app/dashboard/my-batches/page.tsx"

echo "═══════════════════════════════════════════"
echo "STEP 4 — Re-add nav link in StudentShell.tsx"
echo "═══════════════════════════════════════════"
node << 'NODEEOF'
const fs = require('fs');
...
if (content.includes("id:'my-batches'")) {
  console.log('SKIP — nav link already present.');
} else {
  const anchor = "{id:'test-series',icon:'📚',en:'Test Series',hi:'टेस्ट सीरीज',href:'/dashboard/test-series'},";
  if (!content.includes(anchor)) {
    console.error('ABORT — expected test-series nav anchor not found. No changes made.');
    process.exit(1);
  }
  const insertion = "{id:'my-batches',icon:'📚',en:'My Test Series',hi:'मेरी टेस्ट सीरीज',href:'/dashboard/my-batches'},\n    " + anchor;
  content = content.replace(anchor, insertion);
  fs.writeFileSync(path, content);
  console.log('OK — StudentShell.tsx: re-added My Test Series nav link.');
}
NODEEOF

echo "═══════════════════════════════════════════"
echo "STEP 5 — Re-mount routes in index.js"
echo "═══════════════════════════════════════════"
node << 'NODEEOF'
const fs = require('fs');
...
if (content.includes("require('./routes/myBatches')")) {
  console.log('SKIP — index.js already mounts these routes.');
} else {
  ...
  const insertion =
`// ── Restore: My Test Series (formerly My Batches) ──
...
`;
  ...
  fs.writeFileSync(path, content);
  console.log('OK — index.js: re-mounted myBatches + batchActivity routes.');
}
NODEEOF
...
echo "═══════════════════════════════════════════"
for f in src/routes/myBatches.js src/routes/batchActivityRoutes.js src/index.js; do
  echo "-- node -c $f --"
  node -c "$f" && echo "OK"
done
...
echo "3. Test in browser: sidebar → My Test Series"
echo "4. Only after both pass — git add, commit, push"
echo "NOT restored: studentBatchWorkspace.js (not called by any live page"
echo "currently — the 'Continue' button was already just a stub alert)."
