...
export default function TestSeriesPage() {
  ...
  return (
    <div style={{ minHeight:'100vh',color:'var(--pr-text)',fontFamily:'Inter,sans-serif',position:'relative',overflowX:'hidden',background:'var(--pr-bg)', ...(vars as any) }}>
      ...
      {/* STICKY TOP BAR */}
      <div style={{ position:'sticky',top:0,zIndex:50,background:'rgba(var(--pr-header-rgb),0.94)',backdropFilter:'blur(22px)',borderBottom:'1px solid rgba(77,159,255,0.1)',padding:'10px 14px',display:'flex',alignItems:'center',gap:10 }}>
        ...
        <PRLogo size={32} />
        <div>
          <div style={{ fontFamily:'Playfair Display,serif',fontSize:14,fontWeight:700,background:'linear-gradient(90deg,#4D9FFF,#00D4FF)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent' }}>Batches & Test Series</div>
          <div style={{ fontSize:10,color:'rgba(var(--pr-sub-rgb),0.42)' }}>NEET / JEE / CUET</div>
        </div>
        <div style={{ flex:1 }} />
            ...
            {FACTS.map((f,i)=>(
              <div key={i} style={{ display:'flex',gap:13,alignItems:'flex-start',animation:`slideUp ${1.1+i*0.12}s ease` }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:11,color:'rgba(var(--pr-sub-rgb),0.58)',lineHeight:1.7 }}>{f.f}</div>
                </div>
              </div>
            ))}
          </div>
