...
if grep -q "import BatchManagerUltra from './BatchManagerUltra'" "$PAGE_FILE"; then
  ...
  if grep -q "^'use client'$" "$PAGE_FILE"; then
    sed -i "0,/^'use client'$/s//'use client'\nimport BatchManagerUltra from '.\/BatchManagerUltra'/" "$PAGE_FILE"
  else
    sed -i "1i import BatchManagerUltra from './BatchManagerUltra'" "$PAGE_FILE"
  fi
  echo "✅ Added BatchManagerUltra import"
fi

# ── 3) Replace old inline Batches tab block with <BatchManagerUltra/> (idempotent) ──
if grep -q "<BatchManagerUltra token={token} API={API}" "$PAGE_FILE"; then
  echo "⏭️  Batches tab already upgraded — skipping block replace"
else
  BSTART=$(grep -c '══ BATCHES ══' "$PAGE_FILE" || true)
  BEND=$(grep -c '══ ADMINS ══' "$PAGE_FILE" || true)
  if [ "$BSTART" != "1" ] || [ "$BEND" != "1" ]; then
    echo "⚠️  Could not safely locate unique Batches section markers (found BSTART=$BSTART, BEND=$BEND)."
    echo "⚠️  Skipping automatic tab replace — please manually render <BatchManagerUltra token={token} API={API}/> for tab==='batches' in $PAGE_FILE"
  else
    awk '
      BEGIN{skip=0}
      /\{\/\* ══ BATCHES ══ \*\/\}/{
        print "          {/* == BATCHES == (FPR1 Ultra SaaS Upgrade) */}"
        print "          {tab===\x27batches\x27&&(<BatchManagerUltra token={token} API={API}/>)}"
        skip=1
        next
      }
      /\{\/\* ══ ADMINS ══ \*\/\}/{ skip=0 }
      skip==1{next}
      {print}
    ' "$PAGE_FILE" > "$PAGE_FILE.new_fpr1"

    if grep -q "══ ADMINS ══" "$PAGE_FILE.new_fpr1" && grep -q "<BatchManagerUltra token={token} API={API}" "$PAGE_FILE.new_fpr1"; then
      mv "$PAGE_FILE.new_fpr1" "$PAGE_FILE"
      echo "✅ Replaced inline Batches tab with <BatchManagerUltra/> component"
    else
...
# ── Optional TypeScript sanity check (skipped gracefully if tsc unavailable) ──
if command -v npx >/dev/null 2>&1 && [ -f "$ADMIN_DIR/../../../../package.json" ]; then
  echo "🔎 Running optional TypeScript check (non-blocking)..."
  (cd "$ADMIN_DIR" && npx --no-install tsc --noEmit --jsx react-jsx --skipLibCheck --esModuleInterop --moduleResolution bundler BatchManagerUltra.tsx) 2>/dev/null && echo "✅ BatchManagerUltra.tsx passed TypeScript check" || echo "ℹ️  TypeScript check skipped/unavailable in this environment (not blocking — file is pre-validated)."
fi
...
check "8) Create Batch Button"                                    "Create Batch" "$CFILE"
check "9) Bulk Actions (Archive/Delete Selected)"                 "bulkAction" "$CFILE"
check "10) Batch Card Grid (premium cards)"                       "function BatchManagerUltra" "$CFILE"
check "11) Card Actions (Open/Duplicate/Archive/Delete)"          "duplicateBatch" "$CFILE"
check "12) Create Batch Wizard — Multi-Step"                       "function CreateBatchWizard" "$CFILE"
check "13) Wizard — Template Picker"                               "Batch Template Picker" "$CFILE"
...
check "20) Batch Detail Page — Tabbed Workspace"                    "function BatchDetail" "$CFILE"
...
check "23) Add Student Modal (by ID + Email dual selector)"        "function StudentAddTransferModal" "$CFILE"
check "24) Transfer Student Flow (source + target batch)"          "mode === 'transfer'" "$CFILE"
check "25) Before/After Summary in Add/Transfer Modal"              "beforeAfter" "$CFILE"
check "26) Exams Tab (assign/remove/priority/flags)"                "function ExamsTab" "$CFILE"
...
check "37) Analytics — Batch Comparison"                             "Batch Comparison" "$CFILE"
