#!/bin/bash
# ProveRank — Part-04 Phase-02
# Adds: Share as Image (html2canvas) + Public Comparison Link (guest view)
# Zero existing features touched

set -e
echo "🚀 ProveRank Part-04 Phase-02 — Starting..."

# ─────────────────────────────────────────────
# STEP 1 — Public Compare page (NEW)
# ─────────────────────────────────────────────
echo "📝 Step 1: Creating public compare page..."
mkdir -p ~/workspace/frontend/app/compare/public
cat > ~/workspace/frontend/app/compare/public/page.tsx << 'EOF'
'use client'
...
EOF
echo "✅ Public compare page created"

# ─────────────────────────────────────────────
# STEP 2 — batch-compare page — Add Share as Image + Public Link button
# ─────────────────────────────────────────────
echo "📝 Step 2: Adding Share as Image + Public Link to batch-compare page..."
cd ~/workspace && node << 'JSEOF'
const fs = require('fs');
let c = fs.readFileSync('frontend/app/dashboard/batch-compare/page.tsx', 'utf8');

if (c.includes('shareAsImage')) {
  console.log('Already added — skipping'); process.exit(0);
}

// 1. Add downloading state after copied state
c = c.replace(
  'const [copied, setCopied] = useState(false)',
  'const [copied, setCopied] = useState(false)\n  const [downloading, setDownloading] = useState(false)\n  const [publicCopied, setPublicCopied] = useState(false)'
);

// 2. Add modalRef after showModal
c = c.replace(
...
// 4. Add shareAsImage function + publicLink function after shareLink function
const oldShareLink = `  const shareLink = () => {
    const url = \`\${window.location.origin}/dashboard/batch-compare?ids=\${selected.map(b => b._id).join(',')}\`
    navigator.clipboard.writeText(url)
    setCopied(true); setTimeout(() => setCopied(false), 2500)
  }`;

const newShareLink = `  const shareLink = () => {
    const url = \`\${window.location.origin}/dashboard/batch-compare?ids=\${selected.map(b => b._id).join(',')}\`
    navigator.clipboard.writeText(url)
    setCopied(true); setTimeout(() => setCopied(false), 2500)
  }
...
c = c.replace(oldShareLink, newShareLink);

// 5. Add ref to modal div — find the modal outer div
c = c.replace(
  '      {showModal && (',
  '      {showModal && (<div ref={modalRef}>'
);
// Close the extra div before the closing of showModal
c = c.replace(
  '      )}\n\n      {/* Batch Selection',
  '      </div>)}\n\n      {/* Batch Selection'
);

// 6. Add Share as Image + Public Link buttons next to Export PDF button
const oldExportBtn = `<button onClick={() => window.print()} style={{ background: 'rgba(39,174,96,0.1)', border: '1px solid rgba(39,174,96,0.25)', borderRadius: 10, padding: '7px 13px', color: '#27AE60', cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>
                  📄 Export PDF
                </button>`;

const newExportBtn = `<button onClick={() => window.print()} style={{ background: 'rgba(39,174,96,0.1)', border: '1px solid rgba(39,174,96,0.25)', borderRadius: 10, padding: '7px 13px', color: '#27AE60', cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>
                  📄 Export PDF
                </button>
...
fs.writeFileSync('frontend/app/dashboard/batch-compare/page.tsx', c);
console.log('batch-compare page updated');
JSEOF
echo "✅ batch-compare page updated"
...
echo ""
echo "=== VERIFICATION ==="
echo "Public compare page:" && ls ~/workspace/frontend/app/compare/public/page.tsx
echo "shareAsImage fn:" && grep -c "shareAsImage" ~/workspace/frontend/app/dashboard/batch-compare/page.tsx
echo "copyPublicLink fn:" && grep -c "copyPublicLink\|Public Link" ~/workspace/frontend/app/dashboard/batch-compare/page.tsx
echo "html2canvas import:" && grep -c "html2canvas" ~/workspace/frontend/app/dashboard/batch-compare/page.tsx
echo "modalRef:" && grep -c "modalRef" ~/workspace/frontend/app/dashboard/batch-compare/page.tsx
echo "Public page guest notice:" && grep -c "Guest\|guest\|Public View\|public view" ~/workspace/frontend/app/compare/public/page.tsx
echo ""
echo "✅✅✅ Part-04 Phase-02 COMPLETE!"
