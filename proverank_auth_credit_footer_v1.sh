  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, fontFamily: 'Inter,sans-serif', position: 'relative', overflowX: 'hidden' }}>
      <style>{`
        ...
        @keyframes heartBeat{0%,100%{transform:scale(1)}25%{transform:scale(1.3)}50%{transform:scale(1)}75%{transform:scale(1.22)}}
      ...
      <div className="auth-row" style={{ display: 'flex', minHeight: '100vh', position: 'relative', zIndex: 1 }}>
        ...
        <div className="auth-form-area" style={{ flex: 1, display: 'flex', justifyContent: 'center', padding: '44px 20px', overflowY: 'auto' }}>
          <div style={{ width: '100%', maxWidth: 420, animation: 'fadeIn .5s ease' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: 30, paddingBottom: 6 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, color: T.sub, fontFamily: 'Inter,sans-serif', letterSpacing: 0.3 }}>
                <span>Crafted with</span>
                <span style={{ display: 'inline-block', color: '#FF4D6D', animation: 'heartBeat 1.6s ease-in-out infinite', filter: 'drop-shadow(0 0 4px rgba(255,77,109,0.55))' }}>♥</span>
                <span>by</span>
                <span style={{ fontWeight: 700, background: `linear-gradient(90deg,${T.pri},${T.cyan})`, backgroundSize: '200% 100%', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', animation: 'gradMove 5s ease infinite' }}>Praveen Rajput</span>
              </div>
            </div>
          </div>
        </div>
