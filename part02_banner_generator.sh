#!/bin/bash
echo "=== Part-02: Banner Generator ==="

# ── STEP 1: Backend Route ──
cat > ~/workspace/src/routes/bannerGenerator.js << 'ROUTEOF'
...
if (!c.includes('bannerGenerator')) {
  c = c.replace(
    "const brandingRoutes",
    "const bannerGeneratorRoutes = require('./routes/bannerGenerator');\nconst brandingRoutes"
  );
  c = c.replace(
    "app.use('/api/student/batches'",
    "app.use('/api/admin/banners', bannerGeneratorRoutes);\napp.use('/api/student/batches'"
  );
  fs.writeFileSync(fp, c);
  console.log('✅ Route mounted in index.js');
} else { console.log('✅ Already mounted'); }
NODEEOF

# ── STEP 3: Add Banner Generator to NAV_TABS in admin page ──
node << 'NODEEOF2'
const fs = require('fs');
const fp = process.env.HOME + '/workspace/frontend/app/admin/x7k2p/page.tsx';
let c = fs.readFileSync(fp, 'utf8');
if (!c.includes("'banner-generator'")) {
  // Add to NAV_TABS array — after branding or before last entry
  c = c.replace(
    `{id:'settings`,
    `{id:'banner-generator',label:'🎨 Banner Generator',href:'/admin/x7k2p/banner-generator'},\n    {id:'settings`
  );
  fs.writeFileSync(fp, c);
  console.log('✅ Banner Generator added to NAV_TABS');
} else { console.log('✅ Already in NAV_TABS'); }
NODEEOF2

echo "✅ NAV_TABS updated"

# ── STEP 4: Create Banner Generator Page ──
mkdir -p ~/workspace/frontend/app/admin/x7k2p/banner-generator

cat > ~/workspace/frontend/app/admin/x7k2p/banner-generator/page.tsx << 'EOF'
'use client'
...
echo "✅ Banner Generator page created"

# ── STEP 5: Git Push ──
cd ~/workspace && git add -A && git commit -m "feat: Part-02 Banner Generator — Two-panel editor, 8 templates, color picker, version history, analytics, library, publish toggle, batch link" && git push origin main
