  ...
  return (
    ...
    <div style={{ position:'fixed', inset:0, zIndex:500, background:'#000A18', display:'flex', flexDirection:'column', animation:'s360SlideIn .28s cubic-bezier(.4,0,.2,1)' }}>
      <style>{`
        ...
        @media(max-width:980px){ .s360-panel-left,.s360-panel-right,.s360-main-panel{display:none !important} .s360-panel-left.mshow,.s360-panel-right.mshow,.s360-main-panel.mshow{display:block !important} }
      `}</style>
      ...
      {data && !loading && (
        <div style={{ flex:1, overflow:'hidden', display:'flex', gap:16, padding:'16px 18px' }}>
          ...
          <div className={`s360-scroll s360-main-panel${mobileTab==='main'?' mshow':''}`} style={{ flex:1, overflowY:'auto', minWidth:0 }}>
