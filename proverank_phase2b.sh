...
export default function TestSeriesPage() {
  ...
  const FilterContent=()=>(
    <>
      ...
      {/* Reset */}
      <button onClick={()=>{setFilters({isFree:'',batchType:'',difficulty:'',subject:'',language:''});setPriceRange([0,5000]);setSort('newest')}}
    ...
    <div style={{ minHeight:'100vh',color:'#F0F8FF',fontFamily:'Inter,sans-serif',position:'relative',overflowX:'hidden',background:'transparent' }}>
      ...
      <div style={{ position:'relative',zIndex:2,padding:'14px 14px 80px',maxWidth:1300,margin:'0 auto' }}>
        ...
        <div style={{ display:isDesktop?'flex':'block',gap:22,alignItems:'flex-start' }}>
          ...
          {isDesktop&&(
            <div style={{ width:220,flexShrink:0,position:'sticky',top:70,background:'rgba(4,12,30,0.97)',border:'1px solid rgba(77,159,255,0.12)',borderRadius:18,padding:'18px 16px',backdropFilter:'blur(22px)',boxShadow:'0 10px 40px rgba(0,10,40,0.35)',animation:'slideUp 0.4s ease',maxHeight:'calc(100vh - 90px)',overflowY:'auto' }}>
              <FilterContent />
            </div>
          )}
            ...
            {recommendations.length>0&&(
              <div style={{ marginTop:40 }}>
                ...
                <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:14 }}>
                  {recommendations.map(b=><BatchCard key={b._id} b={b} tok={tok} onUpdate={fetchBatches} onBuy={handleBuy} onReview={setReviewBatch} />)}
                </div>
              </div>
            )}
          </div>
