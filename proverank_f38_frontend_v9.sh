  ...
  const onPhotoChange = (e:any) => {
    ...
    reader.onload = (ev:any) => {
      img.onload = async () => {
        ...
        // letterbox with a neutral fill so nothing gets cut off.
        const size = 300
        ...
        if (ok) { initial.current = { ...initial.current, avatar:dataUrl }; setDirtyPersonal(false) }
        setAvatarBusy(false)
      }
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.75)',zIndex:280,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setPhotoViewerOpen(false)}>
          <div onClick={e=>e.stopPropagation()} style={{maxWidth:300,width:'100%',textAlign:'center'}}>
            ...
            <div style={{display:'flex',gap:10,justifyContent:'center',flexWrap:'wrap'}}>
              <button style={btnP} disabled={avatarBusy} onClick={()=>{ setPhotoViewerOpen(false); onPickPhoto() }}>📤 {t('Upload New Photo','नई फोटो अपलोड करें')}</button>
