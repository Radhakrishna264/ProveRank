#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║  ProveRank — Features 26+27+28: Create Exam Wizard (Backend)   ║
# ╚══════════════════════════════════════════════════════════════════╝
set -e
WS=/home/runner/workspace
echo "🚀 Features 26+27+28 Backend setup..."

# ── Create examWizardRoutes.js ────────────────────────────────────────────────
cat > $WS/src/routes/examWizardRoutes.js << 'ENDOFFILE'
/**
...
ENDOFFILE
echo "✅ examWizardRoutes.js created"

# ── Mount route in index.js ───────────────────────────────────────────────────
node << 'EOF'
const fs = require('fs');
const f  = '/home/runner/workspace/src/index.js';
if (!fs.existsSync(f)) { console.log('⚠️  index.js not found'); process.exit(0); }
let c = fs.readFileSync(f, 'utf8');
if (c.includes('examWizardRoutes')) { console.log('✅ Already mounted'); process.exit(0); }
const req = "const examWizardRoutes = require('./routes/examWizardRoutes');\n";
const mnt = "app.use('/api', examWizardRoutes);\n";
const mk  = "const questionDeleteRoutes";
c = c.includes(mk) ? c.replace(mk, req + mk) : req + c;
const mm  = "app.use('/api', questionDeleteRoutes)";
c = c.includes(mm) ? c.replace(mm, mm + '\n' + mnt) : c + '\n' + mnt;
fs.writeFileSync(f, c);
console.log('✅ examWizardRoutes mounted in index.js');
EOF

# ── Verification ──────────────────────────────────────────────────────────────
echo ""
echo "════════════════════════════════════════════════════════"
echo "  Features 26+27+28 Backend — Verification"
echo "════════════════════════════════════════════════════════"
F=$WS/src/routes/examWizardRoutes.js
[ -f "$F" ] && echo "  ✅ examWizardRoutes.js created" || echo "  ❌ missing"
grep -q "exam-wizard/templates"         "$F" && echo "  ✅ 26    Templates endpoint"            || echo "  ❌ 26"
grep -q "exam-wizard/create"            "$F" && echo "  ✅ 26.1  Create exam (full payload)"    || echo "  ❌ 26.1"
grep -q "duration.*parseInt"            "$F" && echo "  ✅ 26.1.7 duration field (NOT totalDurationSec)" || echo "  ❌ 26.1.7"
grep -q "subjectQs"                     "$F" && echo "  ✅ 26.1.5 Subject-wise Qs count"        || echo "  ❌ 26.1.5"
grep -q "correctMarks\|negativeMarks"   "$F" && echo "  ✅ 26.1.9 Marking scheme"               || echo "  ❌ 26.1.9"
grep -q "passwordEnabled\|password"     "$F" && echo "  ✅ 26.1.13 Password protection"         || echo "  ❌ 26.1.13"
grep -q "whitelist"                     "$F" && echo "  ✅ 26.1.14 Whitelist toggle"             || echo "  ❌ 26.1.14"
grep -q "waitingRoom"                   "$F" && echo "  ✅ 26.1.15 Waiting room"                 || echo "  ❌ 26.1.15"
grep -q "reattempt"                     "$F" && echo "  ✅ 26.1.16 Re-attempt control"           || echo "  ❌ 26.1.16"
grep -q "reviewWindow"                  "$F" && echo "  ✅ 26.1.17 Review window"                || echo "  ❌ 26.1.17"
grep -q "sectionWise"                   "$F" && echo "  ✅ 26.1.18 Section-wise toggle"          || echo "  ❌ 26.1.18"
grep -q "watermark"                     "$F" && echo "  ✅ 26.1.19 Watermark toggle"             || echo "  ❌ 26.1.19"
grep -q "liveQsRange"                   "$F" && echo "  ✅ 26.1.21 Live Qs range"                || echo "  ❌ 26.1.21"
grep -q "assignType\|batchId\|testSeries" "$F" && echo "  ✅ 26.2  Batch/Series assignment"      || echo "  ❌ 26.2"
grep -q "questions/bank"                "$F" && echo "  ✅ 27.1  Import from QB"                || echo "  ❌ 27.1"
grep -q "copypaste"                     "$F" && echo "  ✅ 27.3  Copy-paste upload"              || echo "  ❌ 27.3"
grep -q "excel"                         "$F" && echo "  ✅ 27.4  Excel upload"                   || echo "  ❌ 27.4"
grep -q "pdf"                           "$F" && echo "  ✅ 27.4  PDF upload"                     || echo "  ❌ 27.4"
grep -q "smart-suggest"                 "$F" && echo "  ✅ 27.14 Smart suggest"                  || echo "  ❌ 27.14"
grep -q "duplicate-check"              "$F" && echo "  ✅ 27.16 Duplicate detector"             || echo "  ❌ 27.16"
grep -q "generate-sets"                 "$F" && echo "  ✅ 27.18 Multi-set generate"             || echo "  ❌ 27.18"
grep -q "reorder"                       "$F" && echo "  ✅ 27.7  Reorder questions"              || echo "  ❌ 27.7"
grep -q "exam-wizard.*review"          "$F" && echo "  ✅ 28.1  Review endpoint"               || echo "  ❌ 28.1"
grep -q "publish"                       "$F" && echo "  ✅ 28.8.2 Publish Now"                  || echo "  ❌ 28.8.2"
grep -q "draft"                         "$F" && echo "  ✅ 28.8.3 Save as Draft"                 || echo "  ❌ 28.8.3"
grep -q "schedule-publish"              "$F" && echo "  ✅ 28.8.1 Schedule auto-publish"         || echo "  ❌ 28.8.1"
grep -q "templates.*POST"              "$F" && echo "  ✅ 28.8.4 Save as template"              || echo "  ❌ 28.8.4"
grep -q "notify-students"              "$F" && echo "  ✅ 28.8.6 Notify students"              || echo "  ❌ 28.8.6"
grep -q "clone"                         "$F" && echo "  ✅ 28.11 Clone exam"                     || echo "  ❌ 28.11"
grep -q "examWizardRoutes" "$WS/src/index.js" && echo "  ✅ Route mounted in index.js"          || echo "  ❌ Not mounted"
echo ""
echo "════════════════════════════════════════════════════════"
