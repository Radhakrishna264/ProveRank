...
export default function AdminPanel() {
  ...
  const manualQRef = useRef<HTMLTextAreaElement>(null)
  ...
  const uploadQuestions = async()=>{
    ...
    try{
      ...
      if(qUploadMethod==='copypaste'||qUploadMethod==='manual'){
        const qText=manualQRef.current?.value||'';const aKey=answerKeyRef.current?.value||'';if(!qText){showToast('Paste question text first','error');setUploadingQ(false);return}
        res = await fetch(`${API}/api/upload/copypaste/questions`,{
          method:'POST',
          headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},
          body:JSON.stringify({examId:createdExamId, text:qText, answerKey:aKey})
        })
          ...
          <div style={{padding:'20px'}}>
            ...
            <div style={{display:(qUploadMethod==='manual'||qUploadMethod==='copypaste')?'block':'none'}}>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  <textarea ref={manualQRef} rows={8} placeholder="Paste questions here..." style={{width:'100%',padding:'10px 13px',borderRadius:9,border:`1.5px solid ${iBrd}`,background:iBg,color:tm,fontSize:12,fontFamily:'monospace',resize:'vertical',outline:'none',boxSizing:'border-box'}}/>
                </div>
                <div style={{marginBottom:14}}>
            ...
            <div style={{display:qUploadMethod==='pdf'?'block':'none'}}>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  <textarea ref={answerKeyRef} rows={4} placeholder="1-B, 2-A, 3-D ..." style={{width:'100%',padding:'10px 13px',borderRadius:9,border:`1.5px solid ${iBrd}`,background:iBg,color:tm,fontSize:12,fontFamily:'monospace',resize:'none',outline:'none',boxSizing:'border-box'}}/>
                </div>
            </div>
