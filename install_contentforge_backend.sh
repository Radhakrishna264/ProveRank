...
set -e
echo "=================================================="
echo "ProveRank Backend Installer — Features 19B/20/20B/21/21B"
echo "=================================================="

cd ~/workspace || { echo "ERROR: ~/workspace not found"; exit 1; }

echo "[1/4] Backing up files that will be modified..."
mkdir -p .backup_contentforge_$(date +%Y%m%d_%H%M%S)
BACKUP_DIR=$(ls -d .backup_contentforge_* | tail -1)
cp src/models/Exam.js "$BACKUP_DIR/Exam.js.bak" 2>/dev/null || true
cp src/models/Question.js "$BACKUP_DIR/Question.js.bak" 2>/dev/null || true
cp src/utils/excelParser.js "$BACKUP_DIR/excelParser.js.bak" 2>/dev/null || true
cp src/index.js "$BACKUP_DIR/index.js.bak" 2>/dev/null || true
...
echo ""
echo "[3/4] Checking / installing dependencies (PINNED versions — never blind-upgrade)..."
node -e "require('xlsx')" 2>/dev/null && echo "  -> xlsx OK (already installed)" || npm install xlsx --no-audit --no-fund
node -e "const m=require('pdf-parse'); if(typeof m!=='function') throw 0;" 2>/dev/null && echo "  -> pdf-parse OK (v1.x function API confirmed)" || (echo "  -> installing pdf-parse@1.1.1 (PINNED — v2.x has breaking API changes)"; npm install pdf-parse@1.1.1 --no-audit --no-fund)
node -e "require('multer')" 2>/dev/null && echo "  -> multer OK (already installed)" || npm install multer --no-audit --no-fund

echo ""
echo "[4/4] Mounting new routes in src/index.js (idempotent)..."
if grep -q "contentForge" src/index.js; then
  echo "  -> already mounted, skipping"
else
  cat >> src/index.js << 'IDX_EOF'
...
IDX_EOF
  echo "  -> mounted /api/content-forge"
fi
...
echo ""
echo "--- Sub-feature implementation checklist (grep-verified against shipped code) ---"

echo ""
echo ">> FEATURE 19B — Create Exam via Copy-Paste"
FEAT_OK=1
grep -rq "F19B\.5\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.5.4" || { echo "   [❌] F19B.5.4 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.5\.5\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.5.5" || { echo "   [❌] F19B.5.5 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.5\.7\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.5.7" || { echo "   [❌] F19B.5.7 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.5\.16\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.5.16" || { echo "   [❌] F19B.5.16 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.5\.21\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.5.21" || { echo "   [❌] F19B.5.21 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.6\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.6" || { echo "   [❌] F19B.6 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.6\.2\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.6.2" || { echo "   [❌] F19B.6.2 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.6\.3\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.6.3" || { echo "   [❌] F19B.6.3 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.6\.8\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.6.8" || { echo "   [❌] F19B.6.8 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.7\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.7" || { echo "   [❌] F19B.7 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.7\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.7.1" || { echo "   [❌] F19B.7.1 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.7\.2\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.7.2" || { echo "   [❌] F19B.7.2 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.7\.3\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.7.3" || { echo "   [❌] F19B.7.3 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.7\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.7.4" || { echo "   [❌] F19B.7.4 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.8\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.8.1" || { echo "   [❌] F19B.8.1 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.8\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.8.4" || { echo "   [❌] F19B.8.4 MISSING"; FEAT_OK=0; }
grep -rq "F19B\.8\.6\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F19B.8.6" || { echo "   [❌] F19B.8.6 MISSING"; FEAT_OK=0; }
if [ $FEAT_OK -eq 1 ]; then echo "   ✅ 19B backend logic — ALL PRESENT"; else echo "   ❌ 19B backend logic — INCOMPLETE"; ALL_OK=0; fi

echo ""
echo ">> FEATURE 20 — Bulk Upload via Excel/CSV (Upgraded)"
FEAT_OK=1
grep -rq "F20\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.1" || { echo "   [❌] F20.1 MISSING"; FEAT_OK=0; }
grep -rq "F20\.2\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.2" || { echo "   [❌] F20.2 MISSING"; FEAT_OK=0; }
grep -rq "F20\.2\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.2.1" || { echo "   [❌] F20.2.1 MISSING"; FEAT_OK=0; }
grep -rq "F20\.3\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.3" || { echo "   [❌] F20.3 MISSING"; FEAT_OK=0; }
grep -rq "F20\.3\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.3.4" || { echo "   [❌] F20.3.4 MISSING"; FEAT_OK=0; }
grep -rq "F20\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.4" || { echo "   [❌] F20.4 MISSING"; FEAT_OK=0; }
grep -rq "F20\.4b\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.4b" || { echo "   [❌] F20.4b MISSING"; FEAT_OK=0; }
grep -rq "F20\.6\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.6" || { echo "   [❌] F20.6 MISSING"; FEAT_OK=0; }
grep -rq "F20\.8\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.8" || { echo "   [❌] F20.8 MISSING"; FEAT_OK=0; }
grep -rq "F20\.9\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.9" || { echo "   [❌] F20.9 MISSING"; FEAT_OK=0; }
grep -rq "F20\.11\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.11" || { echo "   [❌] F20.11 MISSING"; FEAT_OK=0; }
grep -rq "F20\.12\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.12" || { echo "   [❌] F20.12 MISSING"; FEAT_OK=0; }
grep -rq "F20\.13\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.13" || { echo "   [❌] F20.13 MISSING"; FEAT_OK=0; }
grep -rq "F20\.17\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.17" || { echo "   [❌] F20.17 MISSING"; FEAT_OK=0; }
grep -rq "F20\.18\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.18" || { echo "   [❌] F20.18 MISSING"; FEAT_OK=0; }
grep -rq "F20\.23\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20.23" || { echo "   [❌] F20.23 MISSING"; FEAT_OK=0; }
if [ $FEAT_OK -eq 1 ]; then echo "   ✅ 20 backend logic — ALL PRESENT"; else echo "   ❌ 20 backend logic — INCOMPLETE"; ALL_OK=0; fi

echo ""
echo ">> FEATURE 20B — Create Exam via Excel/CSV"
FEAT_OK=1
grep -rq "F20B\.1\.2\.14\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.1.2.14" || { echo "   [❌] F20B.1.2.14 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.3\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.3" || { echo "   [❌] F20B.3 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.3\.3\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.3.3" || { echo "   [❌] F20B.3.3 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.3\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.3.4" || { echo "   [❌] F20B.3.4 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.5\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.5.4" || { echo "   [❌] F20B.5.4 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.5\.5\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.5.5" || { echo "   [❌] F20B.5.5 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.5\.16\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.5.16" || { echo "   [❌] F20B.5.16 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.5\.21\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.5.21" || { echo "   [❌] F20B.5.21 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.6\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.6" || { echo "   [❌] F20B.6 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.6\.2\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.6.2" || { echo "   [❌] F20B.6.2 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.6\.8\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.6.8" || { echo "   [❌] F20B.6.8 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.7\.2\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.7.2" || { echo "   [❌] F20B.7.2 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.8\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.8.1" || { echo "   [❌] F20B.8.1 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.8\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.8.4" || { echo "   [❌] F20B.8.4 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.8\.6\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.8.6" || { echo "   [❌] F20B.8.6 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.9\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.9" || { echo "   [❌] F20B.9 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.11\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.11" || { echo "   [❌] F20B.11 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.12\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.12" || { echo "   [❌] F20B.12 MISSING"; FEAT_OK=0; }
grep -rq "F20B\.17\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F20B.17" || { echo "   [❌] F20B.17 MISSING"; FEAT_OK=0; }
if [ $FEAT_OK -eq 1 ]; then echo "   ✅ 20B backend logic — ALL PRESENT"; else echo "   ❌ 20B backend logic — INCOMPLETE"; ALL_OK=0; fi

echo ""
echo ">> FEATURE 21 — Bulk Upload via PDF Parse (Upgraded)"
FEAT_OK=1
grep -rq "F21\.1\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.1.1" || { echo "   [❌] F21.1.1 MISSING"; FEAT_OK=0; }
grep -rq "F21\.1\.3\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.1.3" || { echo "   [❌] F21.1.3 MISSING"; FEAT_OK=0; }
grep -rq "F21\.1\.7\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.1.7" || { echo "   [❌] F21.1.7 MISSING"; FEAT_OK=0; }
grep -rq "F21\.1\.7B\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.1.7B" || { echo "   [❌] F21.1.7B MISSING"; FEAT_OK=0; }
grep -rq "F21\.1\.7C\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.1.7C" || { echo "   [❌] F21.1.7C MISSING"; FEAT_OK=0; }
grep -rq "F21\.2\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.2" || { echo "   [❌] F21.2 MISSING"; FEAT_OK=0; }
grep -rq "F21\.2\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.2.1" || { echo "   [❌] F21.2.1 MISSING"; FEAT_OK=0; }
grep -rq "F21\.2\.3\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.2.3" || { echo "   [❌] F21.2.3 MISSING"; FEAT_OK=0; }
grep -rq "F21\.3\.6\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.3.6" || { echo "   [❌] F21.3.6 MISSING"; FEAT_OK=0; }
grep -rq "F21\.3\.7\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.3.7" || { echo "   [❌] F21.3.7 MISSING"; FEAT_OK=0; }
grep -rq "F21\.3\.8\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.3.8" || { echo "   [❌] F21.3.8 MISSING"; FEAT_OK=0; }
grep -rq "F21\.5\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.5" || { echo "   [❌] F21.5 MISSING"; FEAT_OK=0; }
grep -rq "F21\.5\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.5.1" || { echo "   [❌] F21.5.1 MISSING"; FEAT_OK=0; }
grep -rq "F21\.5\.7\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.5.7" || { echo "   [❌] F21.5.7 MISSING"; FEAT_OK=0; }
grep -rq "F21\.6\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.6" || { echo "   [❌] F21.6 MISSING"; FEAT_OK=0; }
grep -rq "F21\.7\.6\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.7.6" || { echo "   [❌] F21.7.6 MISSING"; FEAT_OK=0; }
grep -rq "F21\.7\.7\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.7.7" || { echo "   [❌] F21.7.7 MISSING"; FEAT_OK=0; }
grep -rq "F21\.15\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.15" || { echo "   [❌] F21.15 MISSING"; FEAT_OK=0; }
grep -rq "F21\.16\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.16" || { echo "   [❌] F21.16 MISSING"; FEAT_OK=0; }
grep -rq "F21\.20\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21.20" || { echo "   [❌] F21.20 MISSING"; FEAT_OK=0; }
if [ $FEAT_OK -eq 1 ]; then echo "   ✅ 21 backend logic — ALL PRESENT"; else echo "   ❌ 21 backend logic — INCOMPLETE"; ALL_OK=0; fi

echo ""
echo ">> FEATURE 21B — Create Exam via PDF Parsing"
FEAT_OK=1
grep -rq "F21B\.2\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.2" || { echo "   [❌] F21B.2 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.2\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.2.1" || { echo "   [❌] F21B.2.1 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.3\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.3" || { echo "   [❌] F21B.3 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.3\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.3.1" || { echo "   [❌] F21B.3.1 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.3\.5\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.3.5" || { echo "   [❌] F21B.3.5 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.8\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.8.4" || { echo "   [❌] F21B.8.4 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.8\.5\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.8.5" || { echo "   [❌] F21B.8.5 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.8\.16\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.8.16" || { echo "   [❌] F21B.8.16 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.8\.21\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.8.21" || { echo "   [❌] F21B.8.21 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.9\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.9" || { echo "   [❌] F21B.9 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.9\.2\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.9.2" || { echo "   [❌] F21B.9.2 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.9\.7\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.9.7" || { echo "   [❌] F21B.9.7 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.11\.1\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.11.1" || { echo "   [❌] F21B.11.1 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.11\.4\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.11.4" || { echo "   [❌] F21B.11.4 MISSING"; FEAT_OK=0; }
grep -rq "F21B\.11\.6\b" src/models/Exam.js src/models/Question.js src/utils/excelParser.js src/utils/pdfQuestionParser.js src/utils/examBuilder.js src/routes/contentForge.js && echo "   [✅] F21B.11.6" || { echo "   [❌] F21B.11.6 MISSING"; FEAT_OK=0; }
if [ $FEAT_OK -eq 1 ]; then echo "   ✅ 21B backend logic — ALL PRESENT"; else echo "   ❌ 21B backend logic — INCOMPLETE"; ALL_OK=0; fi

echo ""
echo "--- New API endpoints registered ---"
node -e "
const router = require('./src/routes/contentForge');
router.stack.forEach(l => { if(l.route){ const m=Object.keys(l.route.methods).join(',').toUpperCase(); console.log('   '+m+' /api/content-forge'+l.route.path); } });
" 2>/dev/null || echo "   (run after npm install completes if this errored)"

echo ""
echo "=================================================="
if [ $ALL_OK -eq 1 ]; then
  echo "✅ BACKEND INSTALL COMPLETE — ALL CHECKS PASSED"
else
  echo "⚠️  SOME CHECKS FAILED — see [FAIL]/[❌] lines above"
fi
echo "=================================================="
echo "NOTE: Restart your server (or re-run) for changes to take effect."