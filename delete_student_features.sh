#!/bin/bash
set -e
echo "=== MEGA DELETE Part 2: Student Panel (9 routes) + Backend (2 routes) ==="

# ---- Safety check first: any other refs to these routes we might be missing? ----
echo "--- Checking for other references before deleting ---"
grep -rn "/mini-tests\|/revision\|/results\|/analytics\x27\|/compare\x27\|/leaderboard\|/certificate\|/parent-portal\|/admit-card" ~/workspace/frontend/src/components ~/workspace/frontend/app --include="*.tsx" 2>/dev/null | grep -v "node_modules\|StudentShell.tsx\|test-series/page.tsx" || echo "(none found beyond what we already fixed)"

# ---- 1) Backend: remove QB Stats + Institute Report routes ----
cat > ~/workspace/src/routes/adminDashboardRoutes.js << 'FILEEOF1'
...
FILEEOF1
echo "adminDashboardRoutes.js updated ✅"

# ---- 2) Frontend: Student sidebar — remove 10 nav items ----
...
# ---- 3) Frontend: Test Series page — remove Batch Compare tray ----
cat > ~/workspace/frontend/app/dashboard/test-series/page.tsx << 'FILEEOF3'
'use client'
...
# ---- 4) Delete the 9 student route folders ----
rm -rf ~/workspace/frontend/app/mini-tests
rm -rf ~/workspace/frontend/app/revision
rm -rf ~/workspace/frontend/app/results
rm -rf ~/workspace/frontend/app/analytics
rm -rf ~/workspace/frontend/app/compare
rm -rf ~/workspace/frontend/app/leaderboard
rm -rf ~/workspace/frontend/app/certificate
rm -rf ~/workspace/frontend/app/parent-portal
rm -rf ~/workspace/frontend/app/admit-card
echo "9 student route folders deleted ✅"

cd ~/workspace
git add -A
git commit -m "delete: 10 student-panel features (mini-tests, revision, results, analytics, compare, batch-compare, leaderboard, certificate, parent-portal, admit-card) + backend QB Stats/Institute Report routes"
git push
