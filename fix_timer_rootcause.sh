#!/bin/bash
set -e
echo "=== Root-cause fix: auto-close expired attempts server-side + clear expiry messaging ==="
...
cd ~/workspace
git add -A
git commit -m "fix: auto-close expired attempts server-side (stops stale attempts resurfacing as active), extra-time consistency in submit route, clear expiry message instead of silent instant auto-submit"
git push
