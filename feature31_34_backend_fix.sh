  ...
  try {
    ...
    res.json({
      ...
      message: skipped.length
        ? `${eligible.length} draft exam(s) delete ho gaye. ${skipped.length} skip ho gaye (draft nahi ya attempts hain) — unhe Archive karo.`
        : `${eligible.length} exam(s) delete ho gaye`,
...
router.post('/:id/clone-advanced', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    if (!original) return res.status(404).json({ success: false, message: 'Exam nahi mila' })
...
router.get('/compare', verifyToken, isAdmin, async (req, res) => {
  try {
    const { a, b } = req.query
    if (!a || !b) return res.status(400).json({ success: false, message: 'a aur b dono exam id chahiye' })
    const [examA, examB] = await Promise.all([
      Exam.findById(a).select('-questions').lean(),
      Exam.findById(b).select('-questions').lean()
    ])
    if (!examA || !examB) return res.status(404).json({ success: false, message: 'Ek ya dono exam nahi mile' })
...
router.patch('/:id/archive', verifyToken, isAdmin, async (req, res) => {
  try {
    const exam = await Exam.findById(req.params.id)
    if (!exam) return res.status(404).json({ success: false, message: 'Exam nahi mila' })
    if (exam.status === 'live') return res.status(403).json({ success: false, message: '⚠️ Live exam archive nahi kar sakte — pehle ise end/unpublish karo.' }) // 34.4
    if (req.user.role !== 'superadmin' && exam.status !== 'draft') return res.status(403).json({ success: false, message: 'Sirf Superadmin hi non-draft exam archive kar sakta hai.' }) // 34.15
    ...
    res.json({ success: true, message: 'Exam archive ho gaya — Recycle Bin me 30 din tak rahega' })
  } catch (err) { res.status(500).json({ success: false, message: err.message }) }
})
...
router.patch('/:id/restore', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Exam restore ho gaya ✅' })
  } catch (err) { res.status(500).json({ success: false, message: err.message }) }
})
...
router.delete('/:id/permanent', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    if (exam.status === 'live') return res.status(403).json({ success: false, message: '⚠️ Live exam delete nahi kar sakte — pehle ise end/unpublish karo.' }) // 34.4

    if (req.user.role !== 'superadmin' && exam.status !== 'draft') {
      return res.status(403).json({ success: false, message: 'Sirf Superadmin hi non-draft exam permanently delete kar sakta hai.' }) // 34.15
    }
    ...
    if ((notDraft || attemptCount > 0) && req.body.confirmTitle !== exam.title) { // 34.17
      return res.status(400).json({ success: false, message: 'Confirm karne ke liye exam ka exact title type karo.', requiresTypeToConfirm: true })
    }
    if (exam.status === 'ended' && !(req.body.reason && req.body.reason.trim())) { // 34.14
      return res.status(400).json({ success: false, message: 'Completed exam delete karne ke liye reason dena mandatory hai.', requiresReason: true })
    }
    ...
    res.json({ success: true, message: 'Exam permanently delete ho gaya', deletedAttempts: attemptCount, deletedResults: resultCount })
  } catch (err) { res.status(500).json({ success: false, message: err.message }) }
})
...
router.post('/bulk-archive', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: `${result.modifiedCount} exam(s) archive ho gaye`, archived: result.modifiedCount })
  } catch (err) { res.status(500).json({ success: false, message: err.message }) }
})
