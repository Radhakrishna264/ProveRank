#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║  ProveRank Sub-Admin Panel V9 — Permission-Based Dynamic Panel  ║
# ║  Rule C1: cat > EOF ONLY | Rule C2: NO sed -i | NO Python      ║
# ║  Fix: Correct PR Split Block Logo + Permission-based sidebar    ║
# ║  New file: /app/admin/panel/page.tsx (SuperAdmin untouched)     ║
# ╚══════════════════════════════════════════════════════════════════╝
...
if [ ! -d "$FE" ]; then
  err "Frontend not found at $FE. Check your path."
  exit 1
fi

mkdir -p $FE/app/admin/panel
log "Directory created: $FE/app/admin/panel"

# ══════════════════════════════════════════════════════════════════
# STEP 1 — Sub-Admin Panel Page (Permission-Based)
# ══════════════════════════════════════════════════════════════════
step "1 — Writing Sub-Admin Panel (Permission-Based)"

cat > $FE/app/admin/panel/page.tsx << 'ENDOFFILE'
'use client'
...
log "Sub-Admin Panel created: $FE/app/admin/panel/page.tsx"

# ══════════════════════════════════════════════════════════════════
# STEP 2 — Update middleware.ts to route admins to /admin/panel
# ══════════════════════════════════════════════════════════════════
step "2 — Updating middleware.ts"

# Check middleware exists
MWARE="$FE/middleware.ts"
if [ ! -f "$MWARE" ]; then
  MWARE="$FE/src/middleware.ts"
fi

if [ ! -f "$MWARE" ]; then
  warn "middleware.ts not found — creating it"
  cat > $FE/middleware.ts << 'MEOF'
...
export function middleware(request: NextRequest) {
  ...
  const token = request.cookies.get('pr_token')?.value ||
                request.headers.get('x-pr-token') || ''

  // Block /admin exact path (but not /admin/x7k2p or /admin/panel)
  if (pathname === '/admin') {
...
export const config = {
  matcher: ['/admin', '/admin/panel/:path*', '/admin/x7k2p/:path*'],
}
MEOF
  log "middleware.ts created"
else
  log "middleware.ts found at: $MWARE"
  warn "middleware.ts already exists — NOT overwriting to avoid breaking existing logic"
  warn "Manually check that /admin/panel is accessible (not blocked)"
fi
...
if [ -f "$FE/app/admin/panel/page.tsx" ]; then
  LINES=$(wc -l < $FE/app/admin/panel/page.tsx)
  log "Sub-Admin Panel: $LINES lines ✅"
else
  err "Sub-Admin Panel NOT created! Check errors above."
fi

echo ""
echo -e "${G}════════════════════════════════════════${N}"
echo -e "${G}  ProveRank Sub-Admin Panel V9 DONE ✅  ${N}"
echo -e "${G}════════════════════════════════════════${N}"
echo ""
...
echo ""
echo "2. If build OK: git add -A && git commit -m 'Admin Panel V9: Permission-based sub-admin panel, correct PR logo' && git push"
echo ""
echo "3. Test at: https://prove-rank.vercel.app/admin/panel"
echo "   → Login as test admin → Check sidebar shows only permitted features"
echo ""
echo -e "${Y}SuperAdmin Panel (x7k2p) is UNTOUCHED ✅${N}"
echo -e "${Y}Correct PR Split Block Logo in: Loading screen + Navbar ✅${N}"
echo -e "${Y}Permission-based: Only allowed features auto-appear ✅${N}"
MAINEOF

chmod +x ~/fix_admin_panel.sh 2>/dev/null || true
