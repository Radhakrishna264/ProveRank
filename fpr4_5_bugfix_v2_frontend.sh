#!/bin/bash
# ══════════════════════════════════════════════════════════════════
# ProveRank — FPR4+FPR5 BUGFIX v2 — FRONTEND
# Fixes:
#  1) Desktop filter sidebar on Batches & Test Series now hidden by default
#  2) Removed duplicate custom header/back-button on both pages — now use
#     the same shared top bar (theme/language toggle) as every other page
#  3) Removed DNA Replication / ATP Synthesis science-facts block
#  4) Sidebar labels: "Test Series" -> "Batches & Test Series",
#     "My Batches" -> "My Batches & Test Series"
# Run from your project ROOT on Replit:  bash fpr4_5_bugfix_v2_frontend.sh
# Safe to re-run (idempotent).
# ══════════════════════════════════════════════════════════════════
set -e
echo "🚀 ProveRank FPR4+FPR5 Bugfix v2 — FRONTEND install starting..."
...
if [ -z "$SHELL_FILE" ]; then echo "❌ StudentShell.tsx not found — aborting."; exit 1; fi
...
if [ -z "$MB_DIR" ]; then MB_DIR="./frontend/app/dashboard/my-batches"; echo "⚠️  My Batches dir not auto-detected — defaulting to $MB_DIR"; fi

echo "📁 StudentShell.tsx : $SHELL_FILE"
echo "📁 Test Series dir  : $TS_DIR"
echo "📁 My Batches dir   : $MB_DIR"

# ── 1) Overwrite Batches & Test Series page.tsx (bugfixed) ──
cp "$TS_DIR/page.tsx" "$TS_DIR/page.tsx.bak_bugfixv2" 2>/dev/null || true
    ...
    <div style={{ position:'fixed',inset:0,zIndex:1000,background:'rgba(0,0,0,0.88)',display:'flex',alignItems:'center',justifyContent:'center',padding:16 }}>
      <div style={{ background:'rgba(var(--pr-card-rgb),0.99)',border:'1px solid rgba(77,159,255,0.25)',borderRadius:22,padding:26,maxWidth:380,width:'100%',backdropFilter:'blur(30px)',boxShadow:'0 30px 80px rgba(0,0,0,0.6)' }}>
        <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18 }}>
          <div style={{ fontFamily:'Playfair Display,serif',fontSize:17,fontWeight:700,color:'var(--pr-text)' }}>💳 Payment Options</div>
        ...
        {batch.allowEMI&&(
          <>
            <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:14 }}>
              ...
              <span style={{ fontSize:10,color:'rgba(var(--pr-sub-rgb),0.4)',fontWeight:600,textTransform:'uppercase',letterSpacing:1 }}>or pay in EMI</span>
              <div style={{ flex:1,height:1,background:'rgba(77,159,255,0.15)' }} />
            </div>
            <div style={{ display:'flex',gap:8,marginBottom:14 }}>
              {plans.map(p=>(
                <button key={p.months} onClick={()=>setPlan(p.months)}
                  ...
                  <div style={{ fontSize:11,fontWeight:700,color:plan===p.months?'#00D4FF':'rgba(var(--pr-sub-rgb),0.5)' }}>{p.label}</div>
                  <div style={{ fontSize:13,fontWeight:900,color:plan===p.months?'var(--pr-text)':'rgba(var(--pr-sub-rgb),0.4)',marginTop:2 }}>₹{p.monthly}<span style={{ fontSize:9 }}>/mo</span></div>
                </button>
              ))}
            </div>
...
export default function TestSeriesPage() {
  ...
  const [filters,setFilters]=useState({ isFree:'', batchType:'', difficulty:'', subject:'', language:'', offerType:'', flashSaleActive:'', emiEligible:'', enrollmentState:'' })
  ...
  const fetchBatches=useCallback(async()=>{
    ...
    try{
      ...
      if(filters.emiEligible)p.set('emiEligible',filters.emiEligible)
  ...
  const FilterContent=()=>(
    <>
      ...
      {/* Quick Toggles: Flash Sale Active / EMI Eligible (FPR4) */}
      <div style={{ marginBottom:18,display:'flex',gap:8,flexWrap:'wrap' }}>
        ...
        <button onClick={()=>setFilters(prev=>({...prev,emiEligible:prev.emiEligible?'':'true'}))}
          style={{ padding:'6px 11px',borderRadius:20,fontSize:10,cursor:'pointer',background:filters.emiEligible?'rgba(0,212,255,0.15)':'rgba(77,159,255,0.05)',border:`1px solid ${filters.emiEligible?'rgba(0,212,255,0.4)':'rgba(77,159,255,0.1)'}`,color:filters.emiEligible?'#00D4FF':'rgba(var(--pr-sub-rgb),0.42)' }}>💳 EMI Eligible</button>
      </div>
      ...
      <div style={{ marginBottom:18 }}>
        ...
        <div style={{ display:'flex',gap:5,flexWrap:'wrap' }}>
          {[{v:'',l:'Any'},{v:'trial',l:'🎯 Free Trial'},{v:'bundle',l:'📦 Bundle'},{v:'spotlight',l:'⭐ Spotlight'},{v:'emi',l:'💳 EMI'},{v:'flashsale',l:'⚡ Flash Sale'}].map(o=>{
      ...
      {/* Reset */}
      <button onClick={()=>{setFilters({isFree:'',batchType:'',difficulty:'',subject:'',language:'',offerType:'',flashSaleActive:'',emiEligible:'',enrollmentState:''});setPriceRange([0,5000]);setSort('newest')}}
            ...
            {recommendations.length>0&&(
              <div style={{ marginTop:40 }}>
                <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:14 }}>
                  ...
                  <span style={{ fontFamily:'Playfair Display,serif',fontSize:15,fontWeight:700,color:'var(--pr-text)' }}>Recommended For You</span>
                </div>
                <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))',gap:14 }}>
                  {recommendations.map(b=><BatchCard key={b._id} b={b} tok={tok} onUpdate={fetchBatches} onBuy={handleBuy} onReview={setReviewBatch} onPreview={x=>setPreviewBatchId(x._id)} />)}
                </div>
              </div>
            )}
          </div>
...
PRVRNK_EOF_MARKER
echo "✅ Updated Batches & Test Series page.tsx"

# ── 2) Overwrite My Batches page.tsx (bugfixed) ──
cp "$MB_DIR/page.tsx" "$MB_DIR/page.tsx.bak_bugfixv2" 2>/dev/null || true
  ...
  return (
    <div style={{minHeight:'100vh',color:TEXT,fontFamily:'Inter,sans-serif',position:'relative',overflowX:'hidden',background:BG, ...(vars as any)}}>
      ...
      <div style={{position:'relative',zIndex:2,padding:'14px 14px 80px',maxWidth:900,margin:'0 auto'}}>
        {/* Page toolbar (progress view toggle + explore) */}
        <div style={{display:'flex',justifyContent:'flex-end',gap:7,alignItems:'center',marginBottom:10}}>
...
PRVRNK_EOF_MARKER
echo "✅ Updated My Batches page.tsx"

# ── 3) Patch StudentShell.tsx sidebar labels (idempotent) ──
if grep -q "en:'My Batches & Test Series'" "$SHELL_FILE"; then
  echo "⏭️  StudentShell.tsx labels already patched — skipping"
else
  cp "$SHELL_FILE" "$SHELL_FILE.bak_bugfixv2"
  if grep -q "{id:'my-batches',icon:'📚',en:'My Batches',hi:'मेरे बैच',href:'/dashboard/my-batches'}," "$SHELL_FILE"; then
    sed -i "s#{id:'my-batches',icon:'📚',en:'My Batches',hi:'मेरे बैच',href:'/dashboard/my-batches'},#{id:'my-batches',icon:'📚',en:'My Batches \& Test Series',hi:'मेरे बैच और टेस्ट सीरीज',href:'/dashboard/my-batches'},#" "$SHELL_FILE"
    echo "✅ Renamed 'My Batches' -> 'My Batches & Test Series'"
  else
    echo "ℹ️  my-batches nav anchor not found in expected form — please verify manually"
  fi
  if grep -q "{id:'test-series',icon:'📚',en:'Test Series',hi:'टेस्ट सीरीज',href:'/dashboard/test-series'}," "$SHELL_FILE"; then
    sed -i "s#{id:'test-series',icon:'📚',en:'Test Series',hi:'टेस्ट सीरीज',href:'/dashboard/test-series'},#{id:'test-series',icon:'📚',en:'Batches \& Test Series',hi:'बैच और टेस्ट सीरीज',href:'/dashboard/test-series'},#" "$SHELL_FILE"
    echo "✅ Renamed 'Test Series' -> 'Batches & Test Series'"
  else
    echo "ℹ️  test-series nav anchor not found in expected form — please verify manually"
  fi
fi

# ══════════════════════════════════════════════════════════════════
# ✅ FINAL VERIFICATION CHECKLIST — FPR4+FPR5 BUGFIX v2
# ══════════════════════════════════════════════════════════════════
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  ✅ FPR4+FPR5 BUGFIX v2 — VERIFICATION CHECKLIST"
echo "═══════════════════════════════════════════════════════════"
...
check "1) Desktop Filter Sidebar — Hidden by Default (toggle state)"   "desktopFilterOpen" "$TSFILE"
check "2) Desktop Filter Toggle Button Present"                         "Hide Filters" "$TSFILE"
checkAbsent "3) Batches & Test Series — Custom Back-Button Header Removed" "router.back()} style={{ background:'rgba(77,159,255,0.1)',border:'1px solid rgba(77,159,255,0.2)',borderRadius:10,width:36,height:36" "$TSFILE"
checkAbsent "4) My Batches — Custom Back-Button Header Removed"            "router.back()} style={{background:'rgba(77,159,255,0.1)',border:'1px solid rgba(77,159,255,0.2)',borderRadius:10,width:36,height:36" "$MBFILE"
checkAbsent "5) DNA Replication / ATP Synthesis Facts Section Removed"     "NCERT FACTS" "$TSFILE"
checkAbsent "6) FACTS.map Render Call Removed"                              "FACTS.map" "$TSFILE"
check "7) Progress View Toggle Preserved (My Batches toolbar)"           "setProgressView" "$MBFILE"
check "8) Explore Button Preserved (My Batches toolbar)"                  "+ Explore" "$MBFILE"
check "9) Notification Bell Preserved (Batches & Test Series)"            "NotificationBell" "$TSFILE"
check "10) Sidebar — 'Batches & Test Series' Label"                        "en:'Batches & Test Series'" "$SHELL_FILE"
check "11) Sidebar — 'My Batches & Test Series' Label"                     "en:'My Batches & Test Series'" "$SHELL_FILE"

echo "═══════════════════════════════════════════════════════════"
echo "  RESULT: $PASS PASSED / $((PASS+FAIL)) TOTAL"
if [ "$FAIL" -eq 0 ]; then
  echo "  🎉 ALL BUGFIXES SUCCESSFULLY IMPLEMENTED ✅"
else
  echo "  ⚠️  $FAIL item(s) need attention — see ❌ above"
fi
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "🧹 Backups saved as *.bak_bugfixv2 next to originals."
echo "👉 Next: Restart Next.js dev server / redeploy and verify both pages open exactly like other sidebar pages."
