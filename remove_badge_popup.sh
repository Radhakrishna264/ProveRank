...
function ChecklistWidget({token,toast,lang}:{token:string;toast:(m:string,t?:'s'|'e'|'w')=>void;lang:'en'|'hi'}) {
  ...
  // Award badge silently in the background — no popup/confetti interruption on login
  React.useEffect(()=>{
    if(allDone && !hasBadge && !loading){
      fetch(API2+'/api/auth/checklist/complete',{method:'POST',headers:{Authorization:'Bearer '+token}}).catch(()=>{})
    }
  ...
  return (
    <>
      {/* Badge celebration popup removed — awarded silently in background */}
