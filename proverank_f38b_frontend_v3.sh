#!/bin/bash
# ProveRank — F38B Student 360° Profile Preview — FRONTEND v3
# 1) Row click (superadmin) now opens the 360 modal directly (not old inline panel)
# 2) Student360Preview now uses the admin panels shared theme (CRD/ACC/BOR/TS/DIM/SUC/DNG/WRN/GOLD)
#    passed via a theme prop, instead of its own guessed local color constants.
# Run from project ROOT in Replit shell: bash proverank_f38b_frontend_v3.sh
set -e
...
echo '-> Writing $ADMIN_DIR/Student360Preview.tsx'
cat > "$ADMIN_DIR/Student360Preview.tsx" << 'PRSHEOF'
'use client'
...
const [topStudents,setTopStudents]=useState<{rank:number,name:string,bestScore:number,totalExams:number}[]>([])
  ...
  const [preview360Id,setPreview360Id]=useState<string|null>(null) // F38B
...
echo ""
echo "════════════════════════════════════════════════════"
echo "  F38B FRONTEND v3 — VERIFICATION"
echo "════════════════════════════════════════════════════"
PASS=0; TOTAL=0
check() {
  TOTAL=$((TOTAL+1))
...
C="$ADMIN_DIR/Student360Preview.tsx"
P="$ADMIN_DIR/page.tsx"

echo "── 1) Row click opens 360 modal ──"
check "$P" "if (isSuper) setPreview360Id(s._id)" "Row click opens 360 modal for superadmin"
check "$P" "else setSelStudent(s)"                "Non-superadmin row click still falls back to old inline panel"

echo "── 2) Shared admin panel theme passed through ──"
check "$P" "theme={{ CRD, ACC, BOR, TS, DIM, SUC, DNG, WRN, GOLD }}" "page.tsx passes the 9 shared theme constants as a prop"
check "$C" "const ThemeCtx = createContext"     "Student360Preview defines a ThemeContext"
check "$C" "const T = { ...DEFAULT_THEME, ...(theme || {}) }" "theme prop merged over sensible defaults"
notcheck "$C" "const CARD_BG = 'rgba(0,22,40,0.78)'" "Old guessed CARD_BG constant removed"
notcheck "$C" "const BORDER = 'rgba(77,159,255,0.18)'" "Old guessed BORDER constant removed"
notcheck "$C" "const PRIM = '#4D9FFF'"          "Old guessed PRIM constant removed"
check "$C" "theme.CRD"                          "Card component reads theme.CRD"
check "$C" "theme.BOR"                          "Card/Row components read theme.BOR"
check "$C" "T.BOR"                              "Main component body uses T.BOR (from passed-in theme)"
check "$C" "T.ACC"                              "Main component body uses T.ACC (from passed-in theme)"
check "$C" "T.SUC"                              "Success color now theme-driven (T.SUC)"
check "$C" "T.DNG"                              "Danger color now theme-driven (T.DNG)"
check "$C" "T.WRN"                              "Warning color now theme-driven (T.WRN)"
check "$C" "T.GOLD"                             "Gold color now theme-driven (T.GOLD)"
check "$C" "ThemeCtx.Provider value={T}"        "Modal wraps its content in ThemeCtx.Provider"
