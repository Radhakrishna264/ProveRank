#!/bin/bash
# ProveRank — Galaxy BG Fix V5 (MINIMAL & SAFE)
# Only replaces draw() body inside ParticlesBg — nothing else touched
...
cp "$FILE" "$FILE.bak_v5"
log "Backup: page.tsx.bak_v5"

step "Writing fix_v5.js"
cat > ~/workspace/fix_v5.js << 'NODEEOF'
const fs = require('fs');
...
log "fix_v5.js written"

step "Running fix"
cd ~/workspace
node fix_v5.js
[ $? -ne 0 ] && cp "$FILE.bak_v5" "$FILE" && err "Failed — auto-restored backup"
log "Fix applied"

step "TS error check"
cd ~/workspace/frontend
ERRS=$(npx tsc --noEmit --skipLibCheck 2>&1 | grep "error TS" | wc -l)
echo "TS errors found: $ERRS"
if [ "$ERRS" -gt "10" ]; then
  cd ~/workspace/frontend/app/admin/x7k2p
  cp page.tsx.bak_v5 page.tsx
  cd ~/workspace
  git add frontend/app/admin/x7k2p/page.tsx
  git commit -m "restore: v5 auto-restore due to TS errors"
  git push origin main
  err "Too many TS errors — auto-restored! No crash will happen."
fi
log "TS check passed"
...
cd ~/workspace
git add frontend/app/admin/x7k2p/page.tsx
git commit -m "fix: Admin Dashboard — Galaxy nebula BG (V5 safe minimal)"
git push origin main
[ $? -ne 0 ] && err "Git push failed"

echo ""
echo -e "${G}══════════════════════════${N}"
echo -e "${G}  DONE! Wait 2 min → Vercel${N}"
