#!/bin/bash
# ══════════════════════════════════════════════════════════════════
# ProveRank — FPR1 BATCH MANAGEMENT ULTRA SaaS — BACKEND INSTALLER
# Run from your project ROOT on Replit:  bash fpr1_backend_install.sh
# Safe to re-run (idempotent) — will not duplicate existing patches.
# ══════════════════════════════════════════════════════════════════
set -e
echo "🚀 ProveRank FPR1 — Batch Manager Ultra SaaS — BACKEND install starting..."
...
BACKEND_ROOT=$(find . -maxdepth 4 -type f -iname "index.js" -not -path "*/node_modules/*" 2>/dev/null -exec grep -l "app.use('/api/admin/batch-controls'" {} \; | head -1 | xargs -I{} dirname {})

if [ -z "$MODELS_DIR" ]; then MODELS_DIR="./backend/models"; echo "⚠️  Batch.js not found — defaulting models dir to $MODELS_DIR"; fi
if [ -z "$ROUTES_DIR" ]; then ROUTES_DIR="./backend/routes"; echo "⚠️  adminBatchControls.js not found — defaulting routes dir to $ROUTES_DIR"; fi
if [ -z "$BACKEND_ROOT" ]; then BACKEND_ROOT="./backend"; echo "⚠️  index.js with batch-controls mount not found — defaulting backend root to $BACKEND_ROOT"; fi
...
# ── 1) Create models/BatchAuditLog.js ──
if [ -f "$MODELS_DIR/BatchAuditLog.js" ]; then echo "⏭️  BatchAuditLog.js exists — skipping"; else
cat > "$MODELS_DIR/BatchAuditLog.js" << 'PRVRNK_EOF_MARKER'
...
PRVRNK_EOF_MARKER
echo "✅ Created BatchAuditLog.js"
fi

# ── 2) Create models/BatchTemplate.js ──
if [ -f "$MODELS_DIR/BatchTemplate.js" ]; then echo "⏭️  BatchTemplate.js exists — skipping"; else
cat > "$MODELS_DIR/BatchTemplate.js" << 'PRVRNK_EOF_MARKER'
const mongoose = require('mongoose');
...
PRVRNK_EOF_MARKER
echo "✅ Created BatchTemplate.js"
fi

# ── 3) Create routes/batchManagerUltra.js ──
cat > "$ROUTES_DIR/batchManagerUltra.js" << 'PRVRNK_EOF_MARKER'
// ══════════════════════════════════════════════════════════════════
...
function computeHealthScore(batch, studentCount, examCount) {
  ...
  score += (batch.isSpotlight || batch.allowFreeTrial || batch.isBundle || batch.allowEMI ? 1 : 0.4) * 10;
  ...
  try {
    ...
    res.json({
      controls: {
        ...
        allowEMI: !!batch.allowEMI, flashSaleActive: !!(batch.flashSaleEndTime && new Date(batch.flashSaleEndTime) > new Date()),
...
PRVRNK_EOF_MARKER
echo "✅ Created/Updated routes/batchManagerUltra.js"

# ── 4) Patch models/Batch.js — add new fields (idempotent) ──
if grep -q "batchCode:" "$MODELS_DIR/Batch.js" 2>/dev/null; then
  echo "⏭️  Batch.js already patched — skipping"
else
  cp "$MODELS_DIR/Batch.js" "$MODELS_DIR/Batch.js.bak_fpr1"
  awk '
  {
    if ($0 ~ /^\},\{timestamps:true\}\);$/ && done != 1) {
      print "  batchCode:{type:String,unique:true,sparse:true},"
      print "  lifecycleStatus:{type:String,default:'"'"'active'"'"',enum:['"'"'draft'"'"','"'"'active'"'"','"'"'upcoming'"'"','"'"'paused'"'"','"'"'archived'"'"']},"
      print "  visibility:{type:String,default:'"'"'public'"'"',enum:['"'"'public'"'"','"'"'private'"'"','"'"'invite_only'"'"']},"
      print "  seatLimit:{type:Number,default:0},"
      print "  enrollmentRule:{type:String,default:'"'"'open'"'"',enum:['"'"'open'"'"','"'"'invite_only'"'"','"'"'manual_approval'"'"','"'"'auto_approval'"'"']},"
      print "  accessPolicy:{type:String,default:'"'"'open'"'"',enum:['"'"'invite_only'"'"','"'"'open'"'"','"'"'manual_approval'"'"','"'"'code_based'"'"']},"
      print "  joinCode:{type:String,default:'"'"''"'"'},"
      print "  teacherAssigned:{type:String,default:'"'"''"'"'},"
      print "  colorIcon:{type:String,default:'"'"'📦'"'"'},"
      print "  startDate:{type:Date},"
      print "  endDate:{type:Date},"
      print "  autoArchiveAfterEnd:{type:Boolean,default:false},"
      print "  priceLocked:{type:Boolean,default:false},"
      print "  settingsLocked:{type:Boolean,default:false},"
      print "  couponCode:{type:String,default:'"'"''"'"'},"
      print "  earlyBirdPrice:{type:Number},"
      print "  limitedTimePrice:{type:Number},"
      print "  priceHistory:[{oldPrice:Number,newPrice:Number,field:String,updatedBy:{type:mongoose.Schema.Types.ObjectId,ref:'"'"'User'"'"'},updatedByName:String,updatedAt:{type:Date,default:Date.now}}],"
      print "  controlSnapshot:{appliedBy:String,appliedAt:Date,state:mongoose.Schema.Types.Mixed},"
      print "  healthScoreCache:{type:Number,default:0},"
      print "  isTemplate:{type:Boolean,default:false},"
      print "  clonedFrom:{type:mongoose.Schema.Types.ObjectId,ref:'"'"'Batch'"'"'},"
      print "  lastActivityAt:{type:Date,default:Date.now},"
      print "  archivedAt:{type:Date},"
      print "  exams:[{type:mongoose.Schema.Types.ObjectId,ref:'"'"'Exam'"'"'}],"
      print "  examMeta:[{examId:{type:mongoose.Schema.Types.ObjectId,ref:'"'"'Exam'"'"'},required:Boolean,locked:Boolean,featured:Boolean,hidden:Boolean,priority:{type:Number,default:0}}],"
      print "  enrollments:[{student:{type:mongoose.Schema.Types.ObjectId,ref:'"'"'User'"'"'},status:{type:String,default:'"'"'active'"'"',enum:['"'"'active'"'"','"'"'inactive'"'"']},joinedAt:{type:Date,default:Date.now}}],"
      print "  renameHistory:[{oldName:String,newName:String,changedBy:String,changedAt:{type:Date,default:Date.now}}],"
      done = 1
    }
    print
  }' "$MODELS_DIR/Batch.js.bak_fpr1" > "$MODELS_DIR/Batch.js"
  node --check "$MODELS_DIR/Batch.js" && echo "✅ Patched Batch.js with FPR1 fields (syntax verified)" || { echo "❌ Batch.js syntax error after patch — restoring backup"; cp "$MODELS_DIR/Batch.js.bak_fpr1" "$MODELS_DIR/Batch.js"; exit 1; }
fi

# ── 5) Patch models/BatchNote.js — add pinned/expiryDate/version (idempotent) ──
if grep -q "pinned:" "$MODELS_DIR/BatchNote.js" 2>/dev/null; then
  echo "⏭️  BatchNote.js already patched — skipping"
else
  cp "$MODELS_DIR/BatchNote.js" "$MODELS_DIR/BatchNote.js.bak_fpr1"
  sed -i "s/  createdBy:{type:mongoose.Schema.Types.ObjectId,ref:'User'},/  createdBy:{type:mongoose.Schema.Types.ObjectId,ref:'User'},\n  pinned:{type:Boolean,default:false},\n  expiryDate:{type:Date,default:null},\n  version:{type:Number,default:1},/" "$MODELS_DIR/BatchNote.js"
  node --check "$MODELS_DIR/BatchNote.js" && echo "✅ Patched BatchNote.js (pinned/expiryDate/version) (syntax verified)" || { echo "❌ BatchNote.js syntax error — restoring backup"; cp "$MODELS_DIR/BatchNote.js.bak_fpr1" "$MODELS_DIR/BatchNote.js"; exit 1; }
fi

# ── 6) Patch backend index.js — mount new route (idempotent) ──
...
if [ -z "$INDEX_FILE" ] || [ ! -f "$INDEX_FILE" ]; then
  ...
  echo "   const batchManagerUltraRoutes = require('./routes/batchManagerUltra');"
  echo "   app.use('/api/admin/batch-manager', batchManagerUltraRoutes);"
else
  if grep -q "batchManagerUltra" "$INDEX_FILE"; then
    echo "⏭️  index.js already mounts batchManagerUltra — skipping"
  else
    cp "$INDEX_FILE" "$INDEX_FILE.bak_fpr1"
    if grep -q "app.use('/api/admin/batch-controls'" "$INDEX_FILE"; then
      sed -i "/app.use('\/api\/admin\/batch-controls'.*);/a const batchManagerUltraRoutes = require('./routes/batchManagerUltra');\napp.use('/api/admin/batch-manager', batchManagerUltraRoutes);" "$INDEX_FILE"
    else
      printf "\nconst batchManagerUltraRoutes = require('./routes/batchManagerUltra');\napp.use('/api/admin/batch-manager', batchManagerUltraRoutes);\n" >> "$INDEX_FILE"
    fi
    node --check "$INDEX_FILE" && echo "✅ Mounted /api/admin/batch-manager route in index.js (syntax verified)" || { echo "❌ index.js syntax error after patch — restoring backup"; cp "$INDEX_FILE.bak_fpr1" "$INDEX_FILE"; exit 1; }
  fi
fi

node --check "$ROUTES_DIR/batchManagerUltra.js" && echo "✅ batchManagerUltra.js syntax verified" || { echo "❌ batchManagerUltra.js has a syntax error"; exit 1; }
node --check "$MODELS_DIR/BatchAuditLog.js" && echo "✅ BatchAuditLog.js syntax verified"
node --check "$MODELS_DIR/BatchTemplate.js" && echo "✅ BatchTemplate.js syntax verified"


# ══════════════════════════════════════════════════════════════════
# ✅ FINAL VERIFICATION CHECKLIST — BACKEND (FPR1 Batch Manager Ultra)
# ══════════════════════════════════════════════════════════════════
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  ✅ FPR1 BATCH MANAGER — BACKEND VERIFICATION CHECKLIST"
echo "═══════════════════════════════════════════════════════════"
RFILE="$ROUTES_DIR/batchManagerUltra.js"
PASS=0; FAIL=0
check() {
...
check "1) Batch Manager Home — List/Search/Sort API"            "router.get('/', auth"                      "$RFILE"
...
check "5) Batch Template Picker / Clone / Save-as-template"      "router.post('/templates'" "$RFILE"
check "6) Create Batch — Full Config Wizard"                     "router.post('/', auth, isAdmin" "$RFILE"
check "7) Duplicate Batch Name/Code Detector"                     "warning: 'duplicate'" "$RFILE"
check "8) Batch Duplicate (Clone) Action"                         "router.post('/:id/duplicate'" "$RFILE"
check "9) Archive / Unarchive Batch"                              "router.put('/:id/archive'" "$RFILE"
check "10) Delete Batch"                                          "router.delete('/:id'," "$RFILE"
check "11) Batch Detail — Overview Aggregate (health/engagement/revenue)" "engagementMeter" "$RFILE"
check "12) Overview Alerts (capacity/end-date/no-exams)"          "alerts:" "$RFILE"
check "13) Batch Health Score Computation"                        "function computeHealthScore" "$RFILE"
...
check "16) Transfer Student by Student ID or Registered Email"   "router.post('/:id/students/transfer'" "$RFILE"
check "17) Before/After Batch Summary on Add/Transfer"            "before: { count: before }" "$RFILE"
check "18) Remove Student from Batch"                             "router.delete('/:id/students/:studentId'" "$RFILE"
check "19) Mark Student Active/Inactive"                          "students/:studentId/status" "$RFILE"
check "20) Student List CSV Export"                               "students/export" "$RFILE"
check "21) Student ID/Email Lookup + Suggestions"                 "router.get('/student-lookup'" "$RFILE"
check "22) Exams Tab — Assigned/Available List"                   "router.get('/:id/exams'" "$RFILE"
check "23) Assign Exam to Batch"                                  "router.post('/:id/exams/assign'" "$RFILE"
check "24) Exam Control Flags (required/locked/featured/hidden/priority)" "router.put('/:id/exams/:examId'" "$RFILE"
check "25) Remove Exam from Batch"                                "router.delete('/:id/exams/:examId'" "$RFILE"
check "26) Pricing Tab — Get Pricing + History + Forecast"        "router.get('/:id/pricing'" "$RFILE"
check "27) Update Pricing (base/discount/bundle/trial/EMI/coupon)" "router.put('/:id/pricing'," "$RFILE"
check "28) Price Lock / Unlock"                                   "pricing/lock" "$RFILE"
check "29) Price History Timeline Logging"                        "priceHistory.push" "$RFILE"
check "30) Price Comparison (batch vs batch)"                     "pricing/compare" "$RFILE"
check "31) Revenue Forecast Calculation"                          "forecast:" "$RFILE"
check "32) Controls Tab — Get/Update System Controls"             "router.get('/:id/controls'" "$RFILE"
check "33) Control Snapshot (applied by/at/state)"                "controlSnapshot =" "$RFILE"
check "34) One-Click Pause / Resume"                              "controls/pause" "$RFILE"
check "35) Materials Tab — List/Add/Update/Delete"                "router.get('/:id/materials'" "$RFILE"
check "36) Material Pin/Unpin + Expiry + Versioning"              "note.version = (note.version" "$RFILE"
check "37) Analytics Tab — Health/Engagement/Revenue/Churn"       "router.get('/:id/analytics'" "$RFILE"
check "38) Analytics — Conversion Funnel"                         "conversionFunnel:" "$RFILE"
check "39) Analytics — Batch Comparison"                          "analytics/compare" "$RFILE"
check "40) Announcements Tab — Send/Schedule + History"           "router.post('/:id/announcements'" "$RFILE"
check "41) Announcement Student Notifications"                    "StudentNotification.insertMany" "$RFILE"
check "42) Settings Tab — Get/Update Batch Settings"              "router.get('/:id/settings'" "$RFILE"
check "43) Settings — Rename History Tracking"                    "renameHistory.push" "$RFILE"
check "44) Settings — Lock/Unlock Batch"                          "settings/lock" "$RFILE"
check "45) Audit History Tab — Full Change Log"                   "router.get('/:id/audit'" "$RFILE"
check "46) Audit Logging Helper (every action tracked)"           "async function logAudit" "$RFILE"
check "47) Batch Comparison (general /compare)"                   "router.get('/compare'" "$RFILE"
check "48) Batch Snapshot Export (JSON)"                          "export-snapshot" "$RFILE"
check "49) Auto Banner Draft Soft-Hook on Batch Create"           "auto-generate linked banner draft" "$RFILE"
check "50) BatchAuditLog Model Created"                           "" "$MODELS_DIR/BatchAuditLog.js"
check "51) BatchTemplate Model Created"                            "" "$MODELS_DIR/BatchTemplate.js"
check "52) Batch.js Model — New SaaS Fields Patched"              "batchCode:" "$MODELS_DIR/Batch.js"
check "53) Batch.js — Exams + ExamMeta Fields"                    "examMeta:" "$MODELS_DIR/Batch.js"
check "54) Batch.js — Enrollments + Rename History"               "renameHistory:" "$MODELS_DIR/Batch.js"
check "55) BatchNote.js — Pinned/Expiry/Version Fields"           "pinned:" "$MODELS_DIR/BatchNote.js"
check "56) Route Mounted in index.js"                              "batchManagerUltra" "$INDEX_FILE"

echo "═══════════════════════════════════════════════════════════"
echo "  RESULT: $PASS PASSED / $((PASS+FAIL)) TOTAL"
if [ "$FAIL" -eq 0 ]; then
  echo "  🎉 ALL BACKEND FPR1 FEATURES SUCCESSFULLY IMPLEMENTED ✅"
else
  echo "  ⚠️  $FAIL item(s) need attention — see ❌ above"
fi
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "👉 Next: Restart your backend (Render/Replit run) then run the FRONTEND installer script."
