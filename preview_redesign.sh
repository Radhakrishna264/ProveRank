#!/bin/bash
# ╔══════════════════════════════════════════════════════════════╗
# ║  ProveRank — Preview All Questions: Complete Redesign       ║
# ║  Ultra Platinum Premium UI + All features intact            ║
# ╚══════════════════════════════════════════════════════════════╝
set -e
PAGE=/home/runner/workspace/frontend/app/admin/x7k2p/page.tsx
FE=/home/runner/workspace/frontend/app/admin/x7k2p
echo "🚀 Preview All Questions — Complete Redesign..."

# ── Step 1: Write new PreviewAllQuestions component ──────────────────────────
cat > $FE/PreviewAllQuestions.tsx << 'ENDOFFILE'
'use client'
          ...
          <div style={{display:'flex',flexDirection:'column' as any,gap:8}}>
            {pagedQs.map((q:any,qi:number)=>{
              ...
              return(
                <div key={q._id||qi}
                  style={{
                    background:isChk?'rgba(77,159,255,0.07)':CRD,
                    border:`1px solid ${isChk?`${ACC}44`:BOR}`,
                  ...
                  <div style={{display:'flex',alignItems:'flex-start',gap:8}}>
                    <input type='checkbox' checked={isChk}
                      onChange={e=>{if(e.target.checked)setBulkSel(p=>[...p,q._id]);else setBulkSel(p=>p.filter((x:string)=>x!==q._id))}}
                      style={{marginTop:3,cursor:'pointer',accentColor:ACC,flexShrink:0,width:15,height:15}}/>

                    <div style={{flex:1,minWidth:0}}>
                      {/* Badges row */}
                      <div style={{display:'flex',gap:4,marginBottom:6,flexWrap:'wrap' as any,alignItems:'center'}}>
                        <span style={{fontSize:11,color:'#fff',fontWeight:800,background:`linear-gradient(135deg,${ACC},${PRP})`,borderRadius:6,padding:'2px 9px',flexShrink:0,boxShadow:`0 1px 6px ${ACC}44`}}>#{qNum}</span>
                        <Badge label={q.subject||'General'} col={sc}/>
                        <Badge label={q.difficulty||'?'} col={dc}/>
                        <Badge label={q.type||'SCQ'} col={DIM}/>
                        {usedIn>0&&<Badge label={`📊 ${usedIn} exam${usedIn>1?'s':''}`} col={PHY}/>}
                        {q.approvalStatus&&q.approvalStatus!=='approved'&&<Badge label={q.approvalStatus} col={q.approvalStatus==='pending'?WRN:DNG}/>}
                      </div>

                      {/* Question text */}
                      <div
                        onClick={()=>{if(longPressFiredRef.current){longPressFiredRef.current=false;return}setSelQId(q._id)}}
                        onTouchStart={()=>{longPressFiredRef.current=false;longPressTimerRef.current=setTimeout(()=>{longPressFiredRef.current=true;setBulkSel(p=>p.includes(q._id)?p.filter((x:string)=>x!==q._id):[...p,q._id]);if(navigator.vibrate)navigator.vibrate(30)},500)}}
                        onTouchEnd={()=>clearTimeout(longPressTimerRef.current)}
                        onTouchMove={()=>clearTimeout(longPressTimerRef.current)}
                        onContextMenu={e=>{e.preventDefault();setBulkSel(p=>p.includes(q._id)?p.filter((x:string)=>x!==q._id):[...p,q._id])}}
                        style={{cursor:'pointer',fontSize:13,color:'#CBD5E1',lineHeight:1.6,marginBottom:4,display:'-webkit-box' as any,WebkitLineClamp:3,WebkitBoxOrient:'vertical' as any,overflow:'hidden',WebkitUserSelect:'none' as any,userSelect:'none' as any}}>
                        {qText.slice(0,120)}{qText.length>120?'…':''}
                      </div>

                      {/* Hindi pending */}
                      {qLang==='hi'&&!q.hindiText&&(
                        <div style={{fontSize:9,color:'#6366f1',marginTop:2,display:'flex',alignItems:'center',gap:3}}>
                          <span style={{display:'inline-block',width:5,height:5,borderRadius:'50%',background:'#6366f1',animation:'pulse 1.5s infinite'}}/>
                          हिंदी अनुवाद प्रतीक्षा में...
                        </div>
                      )}

                      {/* Chapter/topic */}
                      {q.chapter&&<div style={{fontSize:10,color:DIM,marginTop:2}}>📖 {q.chapter}{q.topic?` › ${q.topic}`:''}</div>}
                      ...
                      {stdPrv&&(q.options||[]).length>0&&(
                        <div style={{marginTop:8,display:'grid',gridTemplateColumns:'1fr 1fr',gap:4}}>
                          {(q.options||[]).map((opt:string,oi:number)=>{
                            ...
                            return(
                              <div key={oi} style={{padding:'4px 8px',borderRadius:6,fontSize:10,border:`1px solid ${isC?`${SUC}44`:BOR}`,background:isC?`${SUC}0A`:'rgba(255,255,255,0.02)',color:isC?SUC:'#94A3B8'}}>
                                <span style={{fontWeight:700,marginRight:4,color:isC?SUC:ACC}}>{ltr}.</span>
                                {((qLang==='hi'&&(q.hindiOptions||[])[oi])?q.hindiOptions[oi]:opt||'').replace(/^[A-Da-d][\.\)\:]\s*/,'').trim().slice(0,30)}
                                {isC&&' ✓'}
                              </div>
                            )
                          })}
                    ...
                    <div style={{display:'flex',flexDirection:'column' as any,gap:4,flexShrink:0}}>
                      {[
                        {ico:'👁️',fn:()=>setSelQId(q._id),title:'Preview'},
                        {ico:'📊',fn:()=>fetchUsageStats(q),title:'Usage Stats'},
                        {ico:'✏️',fn:()=>{const ltrs=['A','B','C','D'];const ci=Array.isArray(q.correct)&&q.correct.length>0?q.correct[0]:(q.correctAnswer?ltrs.indexOf(q.correctAnswer):0);setEditQD({...q,correctLetter:ltrs[ci>=0?ci:0]||'A'})},title:'Edit'},
                        {ico:'📋',fn:()=>copyToAddForm(q),title:'Copy'},
                      ].map(btn=>(
                        <button key={btn.title} onClick={btn.fn} title={btn.title}
                          style={{background:'rgba(255,255,255,0.04)',border:`1px solid ${BOR}`,color:DIM,borderRadius:6,width:28,height:26,cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.2s',flexShrink:0}}
                          onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background='rgba(77,159,255,0.12)';(e.currentTarget as HTMLElement).style.color=TS}}
                          onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background='rgba(255,255,255,0.04)';(e.currentTarget as HTMLElement).style.color=DIM}}>
                          {btn.ico}
                        </button>
                      ))}
                      {/* Delete — red hover */}
                      <button onClick={()=>openDeleteModal(q)} title='Delete'
                        style={{background:'rgba(255,77,77,0.05)',border:`1px solid rgba(255,77,77,0.2)`,color:DNG,borderRadius:6,width:28,height:26,cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.2s',opacity:0.5,flexShrink:0}}
                        onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background=`linear-gradient(135deg,${DNG},#aa0000)`;(e.currentTarget as HTMLElement).style.color='#fff';(e.currentTarget as HTMLElement).style.opacity='1'}}
                        onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background='rgba(255,77,77,0.05)';(e.currentTarget as HTMLElement).style.color=DNG;(e.currentTarget as HTMLElement).style.opacity='0.5'}}>
                        🗑️
                      </button>
                    </div>
                  </div>
...
ENDOFFILE
echo "✅ PreviewAllQuestions.tsx component created"

# ── Step 2: Patch page.tsx to import + use new component ─────────────────────
node << 'EOF'
const fs = require('fs');
...
// Add import
if (!c.includes('PreviewAllQuestions')) {
  c = c.replace("'use client'", "'use client'\nimport PreviewAllQuestions from './PreviewAllQuestions'");
  console.log('✅ PreviewAllQuestions imported');
} else { console.log('✅ Import present'); }

// Find qBV==='preview' block start and end
const PSTART = '{qBV===\'preview\'&&(\n                <div>';
const PEND   = '              )}\n\n              {/* ════════════════════════════════════════════════════ */}';

const startIdx = c.indexOf(PSTART);
const endIdx   = c.indexOf(PEND);

if (startIdx === -1 || endIdx === -1) {
  console.log('❌ Could not find preview section boundaries');
  console.log('Start found:', startIdx !== -1);
  console.log('End found:', endIdx !== -1);
  process.exit(1);
}

// Get token (how it's accessed in page.tsx)
const tokenExpr = "typeof window!=='undefined'?localStorage.getItem('pr_token')||'':''";

const NEW_PREVIEW = `{qBV==='preview'&&(
                <PreviewAllQuestions
                  questions={questions||[]}
                  exams={exams||[]}
                  token={${tokenExpr}}
...
c = c.substring(0, startIdx) + NEW_PREVIEW + '\n\n              ' + c.substring(endIdx);
fs.writeFileSync(file, c);
console.log('✅ page.tsx patched — preview section replaced with PreviewAllQuestions component');
EOF

# ── Verification ──────────────────────────────────────────────────────────────
echo ""
echo "══════════════════════════════════════════════════════"
echo "  Preview All Questions Redesign — Verification"
echo "══════════════════════════════════════════════════════"
F=$FE/PreviewAllQuestions.tsx
chk(){ grep -q "$2" "$1" 2>/dev/null && echo "  ✅ $3" || echo "  ❌ $3"; }

chk $F "Ultra Platinum\|Playfair\|backdropFilter\|backdrop"  "Premium UI design"
chk $F "subjectTabs"          "Subject tabs with counts"
chk $F "Smart Filters"        "Smart Filters collapsible"
chk $F "qfApproval\|qfDiff2"  "All 7 smart filter fields"
chk $F "chapOpts"             "Chapter filter"
chk $F "qBioSub"              "Biology sub-filter (Zoo/Bot)"
chk $F "Difficulty Distribution" "Difficulty distribution bar"
chk $F "bulkSel.length>0"     "Bulk selection bar"
chk $F "bulkApproveFn"        "Bulk approve (custom modal)"
chk $F "bulkDeleteFn"         "Bulk delete (custom modal)"
chk $F "bulkArchiveFn"        "Bulk archive (custom modal)"
chk $F "openDeleteModal"      "Single delete → modal"
chk $F "openA2E"              "Add to Exam"
chk $F "stdPrv"               "Student preview mode"
chk $F "qLang.*hi.*hindiText" "Hindi/English toggle"
chk $F "longPressTimerRef"    "Long press bulk select (mobile)"
chk $F "ConfirmModal"         "Custom confirm modal (no browser confirm)"
chk $F "DeleteConfirmModal"   "Delete modal"
chk $F "RecycleBinModal"      "Recycle Bin modal"
chk $F "ArchivedModal"        "Archived modal"
chk $F "UndoToast"            "Undo toast"
chk $F "fetchUsageStats"      "Usage stats button"
chk $F "setEditQD"            "Edit button"
chk $F "copyToAddForm"        "Copy to form button"
chk $F "expQB\|expQBPdf"      "Export CSV/PDF"
chk $F "_qPg.*_qTP"           "Pagination"
chk $F "Add to Exam →"        "Floating bottom bar"
# Bulk Edit REMOVED check
grep -q "Bulk Edit\|openBulkEdit\|showBulkEdit" $F && echo "  ❌ Bulk Edit still present (should be removed)" || echo "  ✅ Bulk Edit REMOVED"
# Browser confirm REMOVED
grep -q "window.confirm\|confirm('" $F && echo "  ❌ browser confirm still present" || echo "  ✅ No browser confirm dialogs"
chk $PAGE "PreviewAllQuestions"  "Component used in page.tsx"

echo ""
echo "══════════════════════════════════════════════════════"
echo "🎉 git add . && git commit -m 'feat: Preview All Qs redesign' && git push"
echo "══════════════════════════════════════════════════════"
