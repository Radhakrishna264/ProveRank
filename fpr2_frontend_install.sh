#!/bin/bash
# ══════════════════════════════════════════════════════════════════
# ProveRank — FPR2 TEST SERIES MANAGEMENT ULTRA SaaS — FRONTEND INSTALLER
# Run from your project ROOT on Replit:  bash fpr2_frontend_install.sh
# Safe to re-run (idempotent). Independent of FPR1 — works before/after it.
# ══════════════════════════════════════════════════════════════════
set -e
echo "🚀 ProveRank FPR2 — Test Series Management Ultra SaaS — FRONTEND install starting..."

# ── Auto-detect admin panel directory ──
ADMIN_DIR=$(grep -rl "NAV_TABS\|const NAV=" --include="page.tsx" . 2>/dev/null | grep -v node_modules | head -1 | xargs -I{} dirname {})
if [ -z "$ADMIN_DIR" ]; then
...
# ── 1) Create TestSeriesManagerUltra.tsx (component) ──
...
PRVRNK_EOF_MARKER
echo "✅ Created/Updated TestSeriesManagerUltra.tsx"

cp "$PAGE_FILE" "$PAGE_FILE.bak_fpr2"

# ── 2) Add import (idempotent) ──
if grep -q "import TestSeriesManagerUltra from './TestSeriesManagerUltra'" "$PAGE_FILE"; then
  echo "⏭️  Import already present — skipping"
else
  if grep -q "^import BatchManagerUltra from '.\/BatchManagerUltra'$" "$PAGE_FILE"; then
    sed -i "0,/^import BatchManagerUltra from '.\/BatchManagerUltra'$/s//import BatchManagerUltra from '.\/BatchManagerUltra'\nimport TestSeriesManagerUltra from '.\/TestSeriesManagerUltra'/" "$PAGE_FILE"
  elif grep -q "^'use client'$" "$PAGE_FILE"; then
    sed -i "0,/^'use client'$/s//'use client'\nimport TestSeriesManagerUltra from '.\/TestSeriesManagerUltra'/" "$PAGE_FILE"
  else
    sed -i "1i import TestSeriesManagerUltra from './TestSeriesManagerUltra'" "$PAGE_FILE"
  fi
  echo "✅ Added TestSeriesManagerUltra import"
fi

# ── 3) Quick-actions NAV_TABS entry (idempotent) ──
if grep -q "tab:'test_series'" "$PAGE_FILE"; then
  echo "⏭️  Quick-action nav entry already present — skipping"
else
  if grep -q "{label:'Batch Manager',tab:'batches',icon:'🗂️'}," "$PAGE_FILE"; then
    sed -i "s/{label:'Batch Manager',tab:'batches',icon:'🗂️'},/{label:'Batch Manager',tab:'batches',icon:'🗂️'},{label:'Test Series Management',tab:'test_series',icon:'📚'},/" "$PAGE_FILE"
    echo "✅ Added Test Series Management to quick-action nav list"
  else
    echo "⚠️  Could not find quick-action nav anchor — please add {label:'Test Series Management',tab:'test_series',icon:'📚'} manually to your NAV_TABS array"
  fi
fi

# ── 4) Sidebar NAV entry (idempotent) ──
if grep -q "id:'test_series'" "$PAGE_FILE"; then
  echo "⏭️  Sidebar nav entry already present — skipping"
else
  if grep -q "{id:'batches',ico:'📦',lbl:'Batches',grp:'Students'}," "$PAGE_FILE"; then
    sed -i "s/{id:'batches',ico:'📦',lbl:'Batches',grp:'Students'},/{id:'batches',ico:'📦',lbl:'Batches',grp:'Students'},\n    {id:'test_series',ico:'📚',lbl:'Test Series Management',grp:'Students'},/" "$PAGE_FILE"
    echo "✅ Added Test Series Management to sidebar NAV"
  else
    echo "⚠️  Could not find sidebar NAV anchor — please add {id:'test_series',ico:'📚',lbl:'Test Series Management',grp:'Students'} manually to your NAV array"
  fi
fi

# ── 5) Permission mapping (idempotent) — reuse batch_transfer permission ──
if grep -q "batch_transfer:\['batches','test_series'\]" "$PAGE_FILE"; then
  echo "⏭️  Permission mapping already updated — skipping"
else
  if grep -q "batch_transfer:\['batches'\]," "$PAGE_FILE"; then
    sed -i "s/batch_transfer:\['batches'\],/batch_transfer:['batches','test_series'],/" "$PAGE_FILE"
    echo "✅ Mapped Test Series Management tab to batch_transfer permission"
  else
    echo "ℹ️  Permission mapping anchor not found — nav item will be always-visible (no permission gating applied). This is safe but not blocking."
  fi
fi

# ── 6) Render block — wire tab='test_series' to component (idempotent) ──
if grep -q "tab==='test_series'&&(<TestSeriesManagerUltra" "$PAGE_FILE"; then
  echo "⏭️  Render block already wired — skipping"
else
  if grep -q "{tab==='batches'&&(<BatchManagerUltra token={token} API={API}/>)}" "$PAGE_FILE"; then
    sed -i "/{tab==='batches'&&(<BatchManagerUltra token={token} API={API}\/>)}/a\\          {tab==='test_series'\&\&(<TestSeriesManagerUltra token={token} API={API}\/>)}" "$PAGE_FILE"
    echo "✅ Wired tab==='test_series' to <TestSeriesManagerUltra/>"
  else
    echo "⚠️  Could not find FPR1 Batch Manager render anchor. Appending render block near end of file body as fallback — please verify placement manually."
    printf "\n          {tab==='test_series'&&(<TestSeriesManagerUltra token={token} API={API}/>)}\n" >> "$PAGE_FILE"
  fi
fi

# ══════════════════════════════════════════════════════════════════
# ✅ FINAL VERIFICATION CHECKLIST — FRONTEND (FPR2 Test Series Manager Ultra)
# ══════════════════════════════════════════════════════════════════
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  ✅ FPR2 TEST SERIES MANAGER — FRONTEND VERIFICATION CHECKLIST"
echo "═══════════════════════════════════════════════════════════"
CFILE="$ADMIN_DIR/TestSeriesManagerUltra.tsx"
PASS=0; FAIL=0
check() {
...
check "1) Test Series Manager Home (cards + entry point)"          "function TestSeriesManagerUltra" "$CFILE"
check "2) Status Summary Strip (active/paused/archived/draft)"    "Status Summary Strip" "$CFILE"
check "3) Smart Search Bar"                                       "Search series, code, exam" "$CFILE"
check "4) Smart Filter Bar (status/exam/price/students/dates)"    "Filters {showFilters" "$CFILE"
check "5) Offer Filters (Spotlight/Trial/Bundle/EMI/FlashSale)"   "'spotlight', 'trial', 'bundle', 'emi', 'flashsale'" "$CFILE"
check "6) Save / Load Filter Presets"                             "savePreset" "$CFILE"
check "7) Sort (newest/oldest/students/revenue/active/name)"      "most_active" "$CFILE"
check "8) Create Test Series Button"                              "Create Series" "$CFILE"
check "9) Bulk Actions (Archive/Delete Selected)"                 "bulkAction" "$CFILE"
check "10) Series Card Grid (premium cards)"                       "function TestSeriesManagerUltra" "$CFILE"
check "11) Card Actions (Open/Duplicate/Archive/Delete)"          "duplicateSeries" "$CFILE"
check "12) Create Series Wizard — Multi-Step"                      "function CreateSeriesWizard" "$CFILE"
check "13) Wizard — Series Template Picker"                        "Series Template Picker" "$CFILE"
check "14) Wizard — Lifecycle Mode"                                 "Lifecycle Mode" "$CFILE"
check "15) Wizard — Enrollment Rule Builder"                       "Enrollment Rule Builder" "$CFILE"
check "16) Wizard — Pricing Setup Wizard"                           "step === 3" "$CFILE"
check "17) Wizard — Default Controls (Spotlight/Auto-Archive)"     "step === 4" "$CFILE"
check "18) Wizard — Preview Before Create"                          "step === 5" "$CFILE"
check "19) Wizard — Duplicate Detector Warning"                     "dupWarn" "$CFILE"
check "20) Test Series Detail Page — Tabbed Workspace"              "function TestSeriesDetail" "$CFILE"
check "21) Overview Tab (health/engagement/revenue/quick actions)" "function OverviewTab" "$CFILE"
check "22) Students Tab"                                            "function StudentsTab" "$CFILE"
check "23) Add Student Modal (by ID + Email dual selector)"        "function StudentAddModal" "$CFILE"
check "24) Before/After Summary in Add Modal"                       "beforeAfter" "$CFILE"
if grep -q "students/transfer" "$CFILE" 2>/dev/null; then
  echo "❌ 25) NO Transfer Flow present (Add-only per FPR2 spec)"; FAIL=$((FAIL+1))
else
  echo "✅ 25) NO Transfer Flow present (Add-only per FPR2 spec)"; PASS=$((PASS+1))
fi
check "26) Tests Tab (assign/remove/priority/flags)"                "function TestsTab" "$CFILE"
check "27) Pricing Tab (base/discount/bundle/trial/EMI/coupon)"     "function PricingTab" "$CFILE"
check "28) Pricing — Lock/Unlock"                                    "toggleLock" "$CFILE"
check "29) Pricing — History Timeline"                              "Price History Timeline" "$CFILE"
check "30) Pricing — Revenue Forecast Display"                      "Revenue Forecast" "$CFILE"
check "31) Controls Tab (spotlight/bundle/trial/emi/status)"        "function ControlsTab" "$CFILE"
check "32) Controls — One-Click Pause"                              "One-Click Pause" "$CFILE"
check "33) Controls — Enrollment Lock / Access Policy"              "Access Policy" "$CFILE"
check "34) Materials Tab (upload/pin/expiry/category)"              "function MaterialsTab" "$CFILE"
check "35) Analytics Tab (health/engagement/revenue/churn)"         "function AnalyticsTab" "$CFILE"
check "36) Analytics — Conversion Funnel Display"                    "Conversion Funnel" "$CFILE"
check "37) Analytics — Series Comparison"                             "Series Comparison" "$CFILE"
check "38) Announcements Tab (send/schedule/history)"               "function AnnouncementsTab" "$CFILE"
check "39) Settings Tab (rename/seat/teacher/lock/auto-archive)"    "function SettingsTab" "$CFILE"
check "40) Settings — Rename History Display"                       "Rename History" "$CFILE"
check "41) Audit History Tab (full change log)"                     "function AuditTab" "$CFILE"
check "42) Mobile Responsive Detection Hook"                        "function useIsMobile" "$CFILE"
check "43) Mobile — Single Column Card Stack"                       "isMobile ? '1fr'" "$CFILE"
check "44) Toast / Confirmation Feedback System"                     "showToast" "$CFILE"
check "45) Admin Theme Matched (glassmorphism dark palette)"        "const ACC  = '#4D9FFF'" "$CFILE"
check "46) Correct Backend Endpoint (test-series-manager)"          "/api/admin/test-series-manager" "$CFILE"
check "47) Isolated localStorage Key (no clash with Batch Manager)" "pr_tsm_detailId" "$CFILE"
check "48) Detail Page Persists on Refresh"                          "pr_tsm_detailId" "$CFILE"
check "49) Stale/Deleted Series Detail Fallback"                     "notFound" "$CFILE"
check "50) Naming Consistency ('Test Series Management' everywhere)" "Back to Test Series Management" "$CFILE"
check "51) page.tsx — TestSeriesManagerUltra Imported"               "import TestSeriesManagerUltra from './TestSeriesManagerUltra'" "$PAGE_FILE"
check "52) page.tsx — New Tab Wired to Component"                    "<TestSeriesManagerUltra token={token} API={API}" "$PAGE_FILE"
check "53) page.tsx — Sidebar Nav Entry Added"                       "id:'test_series'" "$PAGE_FILE"
check "54) page.tsx — Quick-Action Nav Entry Added"                  "tab:'test_series'" "$PAGE_FILE"

echo "═══════════════════════════════════════════════════════════"
echo "  RESULT: $PASS PASSED / $((PASS+FAIL)) TOTAL"
if [ "$FAIL" -eq 0 ]; then
  echo "  🎉 ALL FRONTEND FPR2 FEATURES SUCCESSFULLY IMPLEMENTED ✅"
else
  echo "  ⚠️  $FAIL item(s) need attention — see ❌ above"
fi
echo "═══════════════════════════════════════════════════════════"
echo ""
echo "🧹 Backups saved as *.bak_fpr2 next to originals (safe to delete once verified working)."
echo "👉 Next: Restart your Next.js dev server (or redeploy) and open Admin Panel → Test Series Management tab to test live."
