#!/bin/bash
# ProveRank — Admin Dashboard Fix V3
# Fix: Backtick template literals → string concat (no heredoc conflict)
...
step "Verifying file"
[ ! -f "$FILE" ] && err "page.tsx not found — run Part 1 first!"
log "File found"
...
step "Writing Node fix script"

cat > ~/workspace/fix_dashboard_v3.js << 'NODEEOF'
const fs = require('fs');
...
// NOTE: All dynamic colors use string concat NOT template literals
const OLD_START = '// ══════════════════════════════════════════════════════════════\n// PARTICLES BACKGROUND — Same as Login page';
const OLD_END = "  return <canvas ref={canvasRef} style={{position:'fixed',inset:0,pointerEvents:'none',zIndex:0}}/>\n}";

const si = content.indexOf(OLD_START);
const ei = content.indexOf(OLD_END, si);
if(si===-1||ei===-1){console.error('ERROR: ParticlesBg not found');process.exit(1);}

// Galaxy BG — NO backtick template literals, all string concat
const GALAXY = [
"// ══════════════════════════════════════════════════════════════",
...
content = content.slice(0,si) + GALAXY + content.slice(ei + OLD_END.length);
console.log('Fix 1 DONE: GalaxyBg installed');

// ══ FIX 2+3: Dashboard rich content ══
const DS = "          {/* ══ DASHBOARD ══ */}\n          {tab==='dashboard'&&(";
const DE = "          {/* ══ GLOBAL SEARCH ══ */}";
const dsi = content.indexOf(DS);
const dei = content.indexOf(DE);
if(dsi===-1||dei===-1){console.error('ERROR: Dashboard section not found');process.exit(1);}
...
content = content.slice(0,dsi) + DASH + content.slice(dei);
console.log('Fix 2+3 DONE: Dashboard SVG + rich content');

fs.writeFileSync(FILE, content, 'utf8');
console.log('File saved.');
NODEEOF

log "Node script written"

step "Running fix"
cd ~/workspace
node fix_dashboard_v3.js
[ $? -ne 0 ] && err "Node script failed"
log "Node script success"
...
git commit -m "fix: Admin Dashboard V3 — Galaxy BG + SVG Illustrations + Rich Content"
git push origin main
[ $? -ne 0 ] && err "Git push failed"

echo ""
echo -e "${G}══════════════════════════════${N}"
echo -e "${G}  DONE! Wait 2-3 min → Vercel${N}"
echo -e "${G}  https://prove-rank.vercel.app/admin/x7k2p${N}"
echo -e "${G}══════════════════════════════${N}"
