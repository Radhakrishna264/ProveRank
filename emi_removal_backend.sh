#!/bin/bash
# ══════════════════════════════════════════════════════════════════
# ProveRank — EMI FEATURE COMPLETE REMOVAL — BACKEND
# Removes all EMI-related code from: models/Batch.js, models/TestSeries.js,
# routes/studentBatches.js, routes/adminBatchControls.js,
# routes/batchManagerUltra.js, routes/testSeriesManagerUltra.js
# Run from your project ROOT on Replit:  bash emi_removal_backend.sh
# Safe to re-run (idempotent).
# ══════════════════════════════════════════════════════════════════
set -e
echo "🚀 ProveRank — EMI Feature Removal — BACKEND install starting..."
...
# ── 1) models/Batch.js ──
cp "$MODELS_DIR/Batch.js" "$MODELS_DIR/Batch.js.bak_emirm" 2>/dev/null || true
cat > "$MODELS_DIR/Batch.js" << 'PRVRNK_EOF_MARKER'
...
PRVRNK_EOF_MARKER
node --check "$MODELS_DIR/Batch.js" && echo "✅ Batch.js updated (EMI removed, syntax verified)" || { echo "❌ syntax error — restoring backup"; cp "$MODELS_DIR/Batch.js.bak_emirm" "$MODELS_DIR/Batch.js" 2>/dev/null; exit 1; }

# ── 2) models/TestSeries.js (only if file exists — from FPR2) ──
if [ -f "$MODELS_DIR/TestSeries.js" ]; then
  cp "$MODELS_DIR/TestSeries.js" "$MODELS_DIR/TestSeries.js.bak_emirm"
...
PRVRNK_EOF_MARKER
  node --check "$MODELS_DIR/TestSeries.js" && echo "✅ TestSeries.js updated (EMI removed, syntax verified)" || { echo "❌ syntax error — restoring backup"; cp "$MODELS_DIR/TestSeries.js.bak_emirm" "$MODELS_DIR/TestSeries.js"; exit 1; }
else echo "ℹ️  TestSeries.js not found — skipping (FPR2 may not be installed)"; fi

# ── 3) routes/studentBatches.js ──
cp "$ROUTES_DIR/studentBatches.js" "$ROUTES_DIR/studentBatches.js.bak_emirm" 2>/dev/null || true
...
PRVRNK_EOF_MARKER
node --check "$ROUTES_DIR/studentBatches.js" && echo "✅ studentBatches.js updated (EMI removed, syntax verified)" || { echo "❌ syntax error — restoring backup"; cp "$ROUTES_DIR/studentBatches.js.bak_emirm" "$ROUTES_DIR/studentBatches.js" 2>/dev/null; exit 1; }

# ── 4) routes/adminBatchControls.js ──
if [ -f "$ROUTES_DIR/adminBatchControls.js" ]; then
  cp "$ROUTES_DIR/adminBatchControls.js" "$ROUTES_DIR/adminBatchControls.js.bak_emirm"
  cat > "$ROUTES_DIR/adminBatchControls.js" << 'PRVRNK_EOF_MARKER'
const express  = require('express');
const router   = express.Router();
const mongoose = require('mongoose');
const jwt      = require('jsonwebtoken');
...
module.exports = router;
PRVRNK_EOF_MARKER
  node --check "$ROUTES_DIR/adminBatchControls.js" && echo "✅ adminBatchControls.js updated (EMI route removed, syntax verified)" || { echo "❌ syntax error — restoring backup"; cp "$ROUTES_DIR/adminBatchControls.js.bak_emirm" "$ROUTES_DIR/adminBatchControls.js"; exit 1; }
else echo "ℹ️  adminBatchControls.js not found — skipping"; fi

# ── 5) routes/batchManagerUltra.js (FPR1) ──
if [ -f "$ROUTES_DIR/batchManagerUltra.js" ]; then
  cp "$ROUTES_DIR/batchManagerUltra.js" "$ROUTES_DIR/batchManagerUltra.js.bak_emirm"
...
module.exports = router;
PRVRNK_EOF_MARKER
  node --check "$ROUTES_DIR/batchManagerUltra.js" && echo "✅ batchManagerUltra.js updated (EMI removed, syntax verified)" || { echo "❌ syntax error — restoring backup"; cp "$ROUTES_DIR/batchManagerUltra.js.bak_emirm" "$ROUTES_DIR/batchManagerUltra.js"; exit 1; }
else echo "ℹ️  batchManagerUltra.js not found — skipping (FPR1 may not be installed)"; fi

# ── 6) routes/testSeriesManagerUltra.js (FPR2) ──
if [ -f "$ROUTES_DIR/testSeriesManagerUltra.js" ]; then
  cp "$ROUTES_DIR/testSeriesManagerUltra.js" "$ROUTES_DIR/testSeriesManagerUltra.js.bak_emirm"
...
module.exports = router;
PRVRNK_EOF_MARKER
  node --check "$ROUTES_DIR/testSeriesManagerUltra.js" && echo "✅ testSeriesManagerUltra.js updated (EMI removed, syntax verified)" || { echo "❌ syntax error — restoring backup"; cp "$ROUTES_DIR/testSeriesManagerUltra.js.bak_emirm" "$ROUTES_DIR/testSeriesManagerUltra.js"; exit 1; }
else echo "ℹ️  testSeriesManagerUltra.js not found — skipping (FPR2 may not be installed)"; fi

# ══════════════════════════════════════════════════════════════════
# ✅ FINAL VERIFICATION CHECKLIST — EMI REMOVAL (BACKEND)
# ══════════════════════════════════════════════════════════════════
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  ✅ EMI REMOVAL — BACKEND VERIFICATION CHECKLIST"
echo "═══════════════════════════════════════════════════════════"
PASS=0; FAIL=0
checkAbsent() {
...
checkAbsent "1) Batch.js — allowEMI field removed"                    "EMI" "$MODELS_DIR/Batch.js"
checkAbsent "2) TestSeries.js — allowEMI field removed"                "EMI" "$MODELS_DIR/TestSeries.js"
checkAbsent "3) studentBatches.js — EMI filter logic removed"          "EMI" "$ROUTES_DIR/studentBatches.js"
checkAbsent "4) adminBatchControls.js — PUT /:id/emi route removed"    "EMI" "$ROUTES_DIR/adminBatchControls.js"
checkAbsent "5) batchManagerUltra.js — all EMI references removed"     "EMI" "$ROUTES_DIR/batchManagerUltra.js"
checkAbsent "6) testSeriesManagerUltra.js — all EMI references removed" "EMI" "$ROUTES_DIR/testSeriesManagerUltra.js"

echo "═══════════════════════════════════════════════════════════"
echo "  RESULT: $PASS PASSED / $((PASS+FAIL)) TOTAL"
if [ "$FAIL" -eq 0 ]; then
  echo "  🎉 EMI FEATURE FULLY REMOVED FROM BACKEND ✅"
else
...
echo "👉 Next: Restart your backend, then run emi_removal_frontend.sh"
