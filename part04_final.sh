#!/bin/bash
echo "=== Part-04 Comparison Feature ==="

# ── STEP 1: Fix StudentShell href ──
node << 'EOF1'
const fs=require('fs')
const fp=process.env.HOME+'/workspace/frontend/src/components/StudentShell.tsx'
let c=fs.readFileSync(fp,'utf8')
if(c.includes("href:'/compare'")){
  c=c.replace("href:'/compare'","href:'/dashboard/compare'")
  fs.writeFileSync(fp,c)
  console.log('✅ Compare href fixed')
}else{console.log('✅ Already correct or not found')}
EOF1

# ── STEP 2: Add compareList to test-series page (surgical) ──
node << 'EOF2'
const fs=require('fs')
const fp=process.env.HOME+'/workspace/frontend/app/dashboard/test-series/page.tsx'
let c=fs.readFileSync(fp,'utf8')

if(c.includes('compareList')){console.log('✅ compareList already exists');process.exit(0)}

// 1. Add state after qIdx state (line ~272)
c=c.replace(
...
// 2. Add ⚖️ checkbox button inside BatchCard — after wishlist button
// Find the wishlist button and add compare button after it
c=c.replace(
...
// 3. Pass compareList and toggleCompare to BatchCard calls
c=c.replace(
...
// 4. Update BatchCard function signature to accept compareList and toggleCompare
c=c.replace(
...
// 5. Add tray — insert before the LAST </div>\n    </div>\n  )\n}
const CLOSING = '\n      </div>\n    </div>\n  )\n}'
...
c = c.substring(0, lastIdx) + TRAY + CLOSING + c.substring(lastIdx + CLOSING.length)

fs.writeFileSync(fp, c)
console.log('✅ compareList state, checkbox, tray all added safely')
EOF2

# ── STEP 3: Create Compare Page ──
mkdir -p ~/workspace/frontend/app/dashboard/compare

cat > ~/workspace/frontend/app/dashboard/compare/page.tsx << 'PAGEOF'
'use client'
...
function CompareInner() {
  ...
  const shareLink = () => {
    const url = `${window.location.origin}/dashboard/compare?ids=${selected.map(b => b._id).join(',')}`
...
# STEP 4: Git Push
cd ~/workspace && git add -A && git commit -m "feat: Part-04 Comparison — compare page, floating tray on test-series, side-by-side table, best value, share link, direct enroll" && git push origin main
echo "=== DONE ==="
