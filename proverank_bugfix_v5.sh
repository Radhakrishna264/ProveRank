...
export default function AdminPanel() {
  ...
  const sendAnnounce = async()=>{
    const announceText=announceRef.current?.value||'';const announceText=announceRef.current?.value||'';if(!announceText){showToast('Write announcement first','error');return}
    try{
