#!/bin/bash
# ProveRank — Dashboard Fix V4 (SAFE)
# Strategy: Node.js reads file, does STRING replacement, writes back
# NO template literal conflicts — backticks inside JS strings are fine in Node
...
cp "$FILE" "$FILE.bak_v4"
log "Backup: page.tsx.bak_v4"

# ── Write Node.js script ──
# KEY INSIGHT: Inside a JS file (not heredoc), backticks work fine!
# The NODEEOF heredoc just writes the .js file — no TS execution inside heredoc

step "Writing fix_v4.js"
cat > ~/workspace/fix_v4.js << 'NODEEOF'
const fs = require('fs');
...
log "fix_v4.js written"

step "Running fix"
cd ~/workspace
node fix_v4.js
[ $? -ne 0 ] && err "Node script failed — restoring backup" && cp "${FILE}.bak_v4" "$FILE"
log "Node script success"

step "TypeScript check"
cd ~/workspace/frontend
npx tsc --noEmit --skipLibCheck 2>&1 | grep -E "error TS" | head -10
echo "(No output above = zero TS errors ✅)"

step "Git push"
cd ~/workspace
git add frontend/app/admin/x7k2p/page.tsx
git commit -m "fix: Admin Dashboard V4 — Galaxy BG + SVG cards + rich content (safe)"
git push origin main
[ $? -ne 0 ] && err "Git push failed"

echo ""
echo -e "${G}══════════════════════════════════════${N}"
echo -e "${G}  ALL DONE! Vercel deploy: 2-3 min${N}"
echo -e "${G}  prove-rank.vercel.app/admin/x7k2p${N}"
echo -e "${G}══════════════════════════════════════${N}"
